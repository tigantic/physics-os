...
| SDK Server | `sdk/server/main.py` | HTTP REST (FastAPI) |
...
| `fluidelite-zk/` | ZK proof system | ~31K | Halo2 + Groth16 backends |
...
| `scripts/` | Dev/ops tooling | ~10K | `security_scan.py`, `release_check.py` |
...
| `deployment/` | Container + config | ~1K | `Containerfile`, `deployment.toml` |
...
| **SEC-04** | **Arbitrary code execution via torch.load(weights_only=False)** | [tensornet/platform/checkpoint.py lines 181,189](tensornet/platform/checkpoint.py#L181): explicit `weights_only=False`; 17+ additional instances without `weights_only=True` across `tensornet/`, `demos/`, `scripts/`, `fluidelite/` | Attacker provides malicious `.pt` checkpoint file → `torch.load` deserializes arbitrary objects → RCE |
...
| **SEC-07** | **Grafana admin password "admin"** | [sdk/docker/docker-compose.yml line 110](sdk/docker/docker-compose.yml#L110): `GF_SECURITY_ADMIN_PASSWORD=admin` | Default Grafana accessible with `admin:admin` credentials → full dashboard access, potential pivot |
...
| **SEC-15** | Security scan exclusions too broad | [scripts/security_scan.py](scripts/security_scan.py): excludes `results/*.json`, `evidence/*.json` — could hide real secrets |
...
| Benchmark suite | ⚠️ Partial | 15 benchmark scripts in `benchmarks/`; no CI-integrated regression detection |
...
| **Metrics** | ✅ | Prometheus on port 9090; `prometheus_client==0.21.0` in lockfile; `per_solver_metrics = true` in [deployment.toml](deployment/config/deployment.toml) |
...
| Check | Evidence |
|-------|----------|
| Container HEALTHCHECK | [deployment/Containerfile](deployment/Containerfile): `curl -sf http://localhost:${TRUSTLESS_API_PORT}/health` (30s interval, 3 retries) |
| Standalone script | [deployment/scripts/health_check.sh](deployment/scripts/health_check.sh) (324 lines): API health, solver endpoints, metrics, response times (<100ms pass, <500ms warn), system resources |
...
| **Capacity planning** | ⚠️ Partial | Memory check (≥8GB) and disk check (≥10GB) in [start.sh](deployment/scripts/start.sh); no load testing results |
...
| **Disk full** | Certificate storage has `max_total_size = "10GB"` in config but no enforcement code found | [deployment/config/deployment.toml](deployment/config/deployment.toml) |
...
| **Deployment guide** | `deployment/` exists but no end-to-end deployment walkthrough | Medium |
...
| ST-8 | **Add Prometheus alerting rules and Grafana dashboards** | `deployment/monitoring/` | Alertmanager config with rules for: error rate > 1%, latency p99 > 500ms, OOM, disk > 80%; 3+ Grafana dashboard JSONs | P2 |
