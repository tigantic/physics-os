...
class StreamStats:
    ...
    def update(self, bytes_sent: int, frame_time_ms: float):
        ...
        self.avg_frame_time_ms = (
            alpha * frame_time_ms + (1 - alpha) * self.avg_frame_time_ms
        )

        # Bandwidth in Mbps
        if frame_time_ms > 0:
            self.bandwidth_mbps = bytes_sent * 8 / 1e6 / (frame_time_ms / 1000)
...
class FrameBuffer:
    ...
    def __init__(self, size: int = 3):
        self.size = size
        self.buffers: list[dict[str, Any] | None] = [None] * size
        self.write_idx = 0
        self.read_idx = 0
