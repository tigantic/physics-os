...
class SliceResult:
    ...
    def to_image(self, colormap: 'ColorMap' = None) -> np.ndarray:
        ...
        from .colormaps import apply_colormap, VIRIDIS
...
class SliceEngine:
    ...
    def multi_slice(
        ...
        """
        return [
            self.slice(plane=plane, depth=d, resolution=resolution)
            for d in depths
        ]
...
class VolumeRenderer:
    ...
    def render(
        ...
        # Generate rays
        rays_o, rays_d = self._generate_rays(
            width, height, camera_pos, look_at, fov
        )
        
        # Ray march
        image, depth = self._ray_march(
            rays_o, rays_d, samples_per_ray
        )
    ...
    def _ray_march(
        ...
        for i, t in enumerate(t_vals):
            ...
            color = color + transmittance[:, :, None] * sample_color * sample_alpha[:, :, None]
