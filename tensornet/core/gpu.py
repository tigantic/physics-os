    ...
    def allocate(self, shape: tuple[int, ...], dtype: torch.dtype) -> torch.Tensor:
        ...
        if self.pool is None:
            ...
            self.pool = torch.empty(
                self.pool_size, dtype=torch.uint8, device=self.device
            )
...
def batched_tt_matvec(
    cores: list[torch.Tensor], vectors: torch.Tensor, device: torch.device = None
) -> torch.Tensor:
    """
...
def optimized_einsum(
    equation: str, *operands: torch.Tensor, optimize: str = "optimal"
) -> torch.Tensor:
    """
...
def roe_flux_gpu(
    ...
    lambda_1 = torch.where(
        lambda_1 < epsilon, (lambda_1**2 + epsilon**2) / (2 * epsilon), lambda_1
    )
    lambda_3 = torch.where(
        lambda_3 < epsilon, (lambda_3**2 + epsilon**2) / (2 * epsilon), lambda_3
    )
    ...
    if u_hat.dim() > rho_L.dim():
        F_rhou = rho_L * u_L[0] * u_n_L + (
            p_L if normal == 0 else torch.zeros_like(p_L)
        )
        F_rhov = rho_L * u_L[1] * u_n_L + (
            p_L if normal == 1 else torch.zeros_like(p_L)
        )
    else:
...
def compute_strain_rate_gpu(
    ...
    if is_3d:
        ...
        S_mag = torch.sqrt(
            2.0 * (S_11**2 + S_22**2 + S_33**2 + 2.0 * (S_12**2 + S_13**2 + S_23**2))
        )
    else:
        # 2D case
        ...
        S_mag = torch.sqrt(2.0 * (S_11**2 + S_22**2 + 2.0 * S_12**2))
...
def benchmark_kernel(
    kernel_fn, *args, n_warmup: int = 3, n_runs: int = 10, device: torch.device = None
) -> KernelStats:
    """
