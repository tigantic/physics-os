...
class StableFluidSolver:
    ...
    def _build_preconditioner(self):
        ...
        self.preconditioner = torch.full(
            self.shape, -6.0 / (self.dx * self.dx), device=self.device
        )
    ...
    def _pressure_clamp(self, p_new: torch.Tensor, p_old: torch.Tensor) -> torch.Tensor:
        ...
        delta_clamped = torch.clamp(
            delta, -self.max_pressure_delta, self.max_pressure_delta
        )
    ...
    def project(
        ...
        dp_dx = (
            torch.roll(self.pressure, -1, dims=0) - torch.roll(self.pressure, 1, dims=0)
        ) / (2 * self.dx)
        dp_dy = (
            torch.roll(self.pressure, -1, dims=1) - torch.roll(self.pressure, 1, dims=1)
        ) / (2 * self.dx)
        dp_dz = (
            torch.roll(self.pressure, -1, dims=2) - torch.roll(self.pressure, 1, dims=2)
        ) / (2 * self.dx)
    ...
    def advect(
        self, field: torch.Tensor, u: torch.Tensor, v: torch.Tensor, w: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        advected = F.grid_sample(
            field_5d, grid, mode="bilinear", padding_mode="border", align_corners=True
        )
    ...
    def diffuse(self, field: torch.Tensor, iterations: int = 10) -> torch.Tensor:
        ...
        for _ in range(iterations):
            neighbors = (
                torch.roll(result, 1, dims=0)
                + torch.roll(result, -1, dims=0)
                + torch.roll(result, 1, dims=1)
                + torch.roll(result, -1, dims=1)
                + torch.roll(result, 1, dims=2)
                + torch.roll(result, -1, dims=2)
            )
    ...
    def step(
        self,
        external_force: tuple[torch.Tensor, torch.Tensor, torch.Tensor] | None = None,
    ):
        """
        ...
        if (
            torch.isnan(self.u).any()
            or torch.isnan(self.v).any()
            or torch.isnan(self.w).any()
        ):
    ...
    def get_vorticity(self) -> torch.Tensor:
        ...
        """
        # Curl components
        dw_dy = (torch.roll(self.w, -1, dims=1) - torch.roll(self.w, 1, dims=1)) / (
            2 * self.dx
        )
        dv_dz = (torch.roll(self.v, -1, dims=2) - torch.roll(self.v, 1, dims=2)) / (
            2 * self.dx
        )

        du_dz = (torch.roll(self.u, -1, dims=2) - torch.roll(self.u, 1, dims=2)) / (
            2 * self.dx
        )
        dw_dx = (torch.roll(self.w, -1, dims=0) - torch.roll(self.w, 1, dims=0)) / (
            2 * self.dx
        )

        dv_dx = (torch.roll(self.v, -1, dims=0) - torch.roll(self.v, 1, dims=0)) / (
            2 * self.dx
        )
        du_dy = (torch.roll(self.u, -1, dims=1) - torch.roll(self.u, 1, dims=1)) / (
            2 * self.dx
        )
