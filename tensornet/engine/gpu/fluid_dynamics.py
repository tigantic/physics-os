...
class FluidDynamicsSolver:
    ...
    def project(
        self, u: torch.Tensor, v: torch.Tensor, w: torch.Tensor, iterations: int = 50
    ...
    def get_vorticity(self) -> torch.Tensor:
        ...
        dwdy = (torch.roll(self.w, -1, dims=1) - torch.roll(self.w, 1, dims=1)) / (
            2 * self.dx
        )
        dvdz = (torch.roll(self.v, -1, dims=2) - torch.roll(self.v, 1, dims=2)) / (
            2 * self.dx
        )
        omega_x = dwdy - dvdz

        dudz = (torch.roll(self.u, -1, dims=2) - torch.roll(self.u, 1, dims=2)) / (
            2 * self.dx
        )
        dwdx = (torch.roll(self.w, -1, dims=0) - torch.roll(self.w, 1, dims=0)) / (
            2 * self.dx
        )
        omega_y = dudz - dwdx

        dvdx = (torch.roll(self.v, -1, dims=0) - torch.roll(self.v, 1, dims=0)) / (
            2 * self.dx
        )
        dudy = (torch.roll(self.u, -1, dims=1) - torch.roll(self.u, 1, dims=1)) / (
            2 * self.dx
        )
        omega_z = dvdx - dudy
...
def benchmark_fluid_dynamics(size: int = 64, steps: int = 10):
    ...
    # Initialize solver
    solver = FluidDynamicsSolver(shape=(size, size, size), viscosity=1e-5, dt=0.01)
