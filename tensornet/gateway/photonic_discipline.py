...
class ColorRGB:
    ...
    def darken(self, factor: float) -> "ColorRGB":
        ...
        return ColorRGB(r=self.r * factor, g=self.g * factor, b=self.b * factor)
...
class PhotonicPalette:
    ...
    def apply_opacity_mapping(
        value: torch.Tensor, min_opacity: float = 0.1, max_opacity: float = 0.9
    ) -> torch.Tensor:
        """
...
def demo_palette():
    ...
    print(
        f"  Golden Ratio:   {GutterSystem.GOLDEN_RATIO_PRIMARY:.0%} / {GutterSystem.GOLDEN_RATIO_ANALYTICAL:.0%}"
    )
