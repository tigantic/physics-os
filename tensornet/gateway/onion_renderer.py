...
class RenderLayer:
    ...
    def __repr__(self) -> str:
        return (
            f"RenderLayer({self.type.name}, z={self.z_depth}, {self.blend_mode.value})"
        )
...
class OnionRenderer:
    ...
    def __init__(self, width: int = 1920, height: int = 1080, device: str = "cuda:0"):
        ...
        self.layers: list[RenderLayer] = [
            ...
            RenderLayer(
                type=LayerType.HUD, blend_mode=BlendMode.OVER, z_depth=4, opacity=1.0
            ),
        ]
    ...
    def render_tensor_field(
        self, scalar_field: torch.Tensor, plasma_gradient: np.ndarray
    ):
        """
        ...
        rgb = colors[indices.flatten()].view(
            scalar_field.shape[0], scalar_field.shape[1], 3
        )
    ...
    def composite(self) -> torch.Tensor:
        ...
        enabled = [
            l
            for l in sorted(self.layers, key=lambda x: x.z_depth)
        ...
        self.final_buffer.copy_(
            base_layer.buffer.to(dtype=torch.float16, non_blocking=True)
        )
            ...
            if layer.blend_mode == BlendMode.ADDITIVE:
                ...
                if layer.dirty_rect:
                    ...
                    torch.addcmul(
                        ...
                        src_region[:, :, :3],
                        alpha_src,
                        out=self.final_buffer[:, :, :3],
                    )
                    self.final_buffer[:, :, :3].clamp_(0, 1)

            elif (
                layer.blend_mode == BlendMode.ALPHA
                or layer.blend_mode == BlendMode.PREMULTIPLIED
            ):
                # Fused alpha blend (in-place, no temporary allocations)
                alpha = src_region[:, :, 3:4]
                one_minus_alpha = 1 - alpha
                if layer.dirty_rect:
                    dst_region[:, :, :3].mul_(one_minus_alpha).add_(
                        src_region[:, :, :3] * alpha
                    )
                    if layer.blend_mode == BlendMode.ALPHA:
                        torch.maximum(
                            dst_region[:, :, 3:4], alpha, out=dst_region[:, :, 3:4]
                        )
                    self.final_buffer[y0:y1, x0:x1] = dst_region
                else:
                    self.final_buffer[:, :, :3].mul_(one_minus_alpha).add_(
                        src_region[:, :, :3] * alpha
                    )
                    if layer.blend_mode == BlendMode.ALPHA:
                        torch.maximum(
                            self.final_buffer[:, :, 3:4],
                            alpha,
                            out=self.final_buffer[:, :, 3:4],
                        )
                ...
                if layer.dirty_rect:
                    ...
                    self.final_buffer[:, :, :3] = (
                        src_region[:, :, :3] * src_a
                        + self.final_buffer[:, :, :3] * dst_a * (1 - src_a)
                    ) / denom
    ...
    def print_config(self):
        ...
        for layer in sorted(self.layers, key=lambda l: l.z_depth):
            ...
            print(
                f"  [{layer.z_depth}] {status} {layer.type.name:12s} | "
        ...
        total_vram += (
            self.final_buffer.nelement() * self.final_buffer.element_size() / 1024**2
        )
