...
class ModernGLRenderer:
    ...
    def __init__(
        self, width: int = 1920, height: int = 1080, enable_vsync: bool = True
    ):
        ...
        self.fbo = self.ctx.framebuffer(
            color_attachments=[self.ctx.texture((width, height), 4, dtype="f4")],
    ...
    def _load_shaders(self):
        ...
        try:
            with open(shader_dir / "geological.vert") as f:
                geo_vert = f.read()
            with open(shader_dir / "geological.frag") as f:
                geo_frag = f.read()
            self.programs["geological"] = self.ctx.program(
                vertex_shader=geo_vert, fragment_shader=geo_frag
            )
        ...
        try:
            with open(shader_dir / "tensor_field.vert") as f:
                tensor_vert = f.read()
            with open(shader_dir / "tensor_field.frag") as f:
                tensor_frag = f.read()
            self.programs["tensor"] = self.ctx.program(
                vertex_shader=tensor_vert, fragment_shader=tensor_frag
            )
        except FileNotFoundError:
        ...
        try:
            with open(shader_dir / "geometry_grid.vert") as f:
                grid_vert = f.read()
            with open(shader_dir / "geometry_grid.frag") as f:
                grid_frag = f.read()
            self.programs["grid"] = self.ctx.program(
                vertex_shader=grid_vert, fragment_shader=grid_frag
            )
        except FileNotFoundError:
    ...
    def _generate_sphere(
        self, radius: float = 1.0, rings: int = 32, sectors: int = 32
...
def demo_renderer():
    ...
    print(
        f"✓ Shaders loaded: {len([p for p in renderer.programs.values() if p is not None])}"
    )
