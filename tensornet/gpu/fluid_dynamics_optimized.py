...
class FluidDynamicsSolverOptimized:
    ...
    def diffuse_implicit(
        self, field: torch.Tensor, iterations: int = 10
    ) -> torch.Tensor:
        """
    ...
    def _project_cuda(
        ...
        self.pressure = pressure_solver_cuda.solve_pressure_poisson(
            div, self.pressure, float(self.dx), 10, 1e-4
        )

        # Apply pressure gradient to velocity (custom CUDA kernel)
        pressure_solver_cuda.apply_pressure_gradient(
            u, v, w, self.pressure, float(self.dx)
        )

        return u, v, w

    def _project_pytorch(
        self, u: torch.Tensor, v: torch.Tensor, w: torch.Tensor, iterations: int = 20
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        for _ in range(iterations):
            p_neighbors = (
                torch.roll(p, 1, dims=0)
                + torch.roll(p, -1, dims=0)
                + torch.roll(p, 1, dims=1)
                + torch.roll(p, -1, dims=1)
