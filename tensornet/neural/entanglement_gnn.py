...
class MessagePassingLayer(nn.Module):
    ...
    def forward(
        ...
        if self.aggregation == "mean":
            ...
            counts.scatter_add_(0, target_idx, torch.ones_like(target_idx, dtype=torch.float))
...
class EntanglementGNN(nn.Module):
    ...
    def forward(
        ...
        edge_features = self.edge_encoder(graph.edge_tensor) if graph.num_edges > 0 else None
...
def build_entanglement_graph(
    ...
    correlations: Optional[torch.Tensor] = None,
    local_dims: int = 2,
    ...
    if connect_all and num_sites <= 20:
        ...
        for i in range(num_sites):
            for j in range(i + 1, num_sites):
                ...
                error = truncation_errors[i] if truncation_errors and i < len(truncation_errors) else 1e-10
...
def predict_entanglement_structure(
    graph: EntanglementGraph,
    model: Optional[EntanglementGNN] = None,
...
def train_entanglement_gnn(
    ...
    for epoch in range(epochs):
        ...
        for graph, chi_targets, entropy_targets in zip(
            ...
            entropy_loss = F.mse_loss(entropy_pred[:len(entropy_target)], entropy_target)
