...
class AeroTableConfig:
    ...
    fill_value: Optional[float] = None  # None = extrapolate
...
class AeroTable:
    ...
    def populate_from_cfd(
        self,
        cfd_solver: Callable[[float, float, float], Dict[str, float]]
    ):
        """
    ...
    def _build_interpolators(self):
        ...
        method = self.config.method.value if self.config.method != InterpolationMethod.RBF else 'linear'
        
        # Ensure fill_value is numeric
        fill_value = self.config.fill_value if self.config.fill_value is not None else 0.0
        
        self._interpolators = {
            'CL': RegularGridInterpolator(grids, self.CL_table, method=method, 
                                          bounds_error=self.config.bounds_error, 
                                          fill_value=fill_value),
            'CD': RegularGridInterpolator(grids, self.CD_table, method=method,
                                          bounds_error=self.config.bounds_error,
                                          fill_value=fill_value),
            'CY': RegularGridInterpolator(grids, self.CY_table, method=method,
                                          bounds_error=self.config.bounds_error,
                                          fill_value=fill_value),
            'Cl': RegularGridInterpolator(grids, self.Cl_table, method=method,
                                          bounds_error=self.config.bounds_error,
                                          fill_value=fill_value),
            'Cm': RegularGridInterpolator(grids, self.Cm_table, method=method,
                                          bounds_error=self.config.bounds_error,
                                          fill_value=fill_value),
            'Cn': RegularGridInterpolator(grids, self.Cn_table, method=method,
                                          bounds_error=self.config.bounds_error,
                                          fill_value=fill_value),
        }
    ...
    @classmethod
    def load(cls, filepath: Union[str, Path]) -> 'AeroTable':
...
class RealTimeCFD:
    ...
    def get_aero(
        ...
        return {
            ...
            'Cl': Cl, 'Cm': Cm, 'Cn': Cn,
            'L': L, 'D': D, 'Y': Y,
            'l_moment': l_moment, 'm_moment': m_moment, 'n_moment': n_moment,
            'L_D': LD,
    ...
    def update_correction(
        ...
        self.CL_correction += alpha * (measured_CL - (self.table.lookup(1.0, 0.0).CL + self.CL_correction))
        self.CD_correction += alpha * (measured_CD - (self.table.lookup(1.0, 0.0).CD + self.CD_correction))
        self.Cm_correction += alpha * (measured_Cm - (self.table.lookup(1.0, 0.0).Cm + self.Cm_correction))
...
def create_hypersonic_waverider_model() -> Callable:
    ...
    Returns:
        CFD solver function
    """
    def waverider_aero(mach: float, alpha_deg: float, beta_deg: float) -> Dict[str, float]:
...
def validate_realtime_cfd_module():
    ...
    print(f"Mach=5, Alpha=10°, Beta=0°:")
    ...
    # Do many lookups
    for _ in range(1000):
        rt_cfd.get_aero({'mach': np.random.uniform(2, 7), 
                        'alpha_deg': np.random.uniform(0, 15),
                        'beta_deg': 0, 'q_bar': 50000, 'V': 1500})
    ...
    assert metrics['CL_mae'] < 0.01  # Linear interpolation should be exact at grid points
    print("✓ PASS")
