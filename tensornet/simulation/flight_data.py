...
class TelemetryFrame:
    ...
    air_data: Optional[Dict[str, float]] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'accelerations': self.accelerations.tolist() if self.accelerations is not None else None,
    ...
    def from_dict(cls, d: Dict[str, Any]) -> 'TelemetryFrame':
        ...
        return cls(
            ...
            accelerations=np.array(d['accelerations']) if d.get('accelerations') else None,
...
class TrajectoryReconstruction:
    ...
    # Measurement noise
    R_gps: Optional[np.ndarray] = None
    R_imu: Optional[np.ndarray] = None
    ...
    def __post_init__(self):
        ...
        if self.P is None:
            self.P = np.diag([
                ...
                10, 10, 10,         # Velocity (m²/s²)
                1, 1, 1,            # Acceleration (m²/s⁴)
                0.1, 0.1, 0.1       # Attitude (rad²)
            ])
        ...
        if self.Q is None:
            self.Q = np.diag([
                ...
                1.0, 1.0, 1.0,      # Acceleration process noise
        ...
        if self.R_imu is None:
            self.R_imu = np.diag([0.1, 0.1, 0.1, 0.01, 0.01, 0.01])  # IMU measurement noise
    ...
    def process_frame(self, frame: TelemetryFrame, x_prev: np.ndarray, dt: float) -> np.ndarray:
        """
...
def parse_telemetry(
    ...
    format: TelemetryFormat = TelemetryFormat.CSV
) -> FlightRecord:
    """
...
def _parse_binary(source: Union[str, Path, bytes]) -> FlightRecord:
...
def validate_against_flight(
    ...
    for i in range(3):
        model_pos_interp[:, i] = np.interp(flight_times, model_times, model_trajectory[:, i])
        model_vel_interp[:, i] = np.interp(flight_times, model_times, model_trajectory[:, i + 3])
    ...
    crossrange_error_mean = np.mean(np.sqrt(pos_error[:, 1]**2 + pos_error[:, 2]**2))
...
def compute_reconstruction_error(
    ...
    common_times = np.intersect1d(
        np.round(flight_times, 2),
        np.round(recon_times, 2)
    )
    ...
    # Average uncertainty
    avg_uncertainty = np.mean([reconstruction.get_uncertainty(i)[0:3].mean() 
