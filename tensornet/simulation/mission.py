...
class MissionResult:
    ...
    uncertainty_sample: Optional[Dict] = None
...
class MissionSimulator:
    ...
    def _xyz_to_latlon(self, x: float, y: float, z: float) -> Tuple[float, float, float]:
        """Convert local XYZ to lat/lon/alt."""
        R_earth = 6371000.0
        lat = self.config.launch_lat + np.degrees(y / R_earth)
        lon = self.config.launch_lon + np.degrees(x / (R_earth * np.cos(np.radians(lat))))
        return lat, lon, z
    
    def _compute_atmosphere(self, alt: float, uncertainty: Dict = None) -> Dict[str, float]:
    ...
    def _check_constraints(self, g_load: float, q_bar: float, heating: float) -> FailureMode:
    ...
    def _determine_phase(self, range_to_target_km: float) -> MissionPhase:
        ...
        elif alt < self.config.pullout_complete_alt and self.current_phase == MissionPhase.BOOST:
    ...
    def run(self, uncertainty_sample: Dict = None) -> MissionResult:
        ...
        pos = self._latlon_to_xyz(
            self.config.launch_lat,
            self.config.launch_lon,
            self.config.launch_alt
        )
        ...
        while self.time < self.config.max_time_s:
            ...
            if self.current_phase == MissionPhase.BOOST:
                ...
                thrust = self.config.boost_thrust_N * uncertainty_sample.get('thrust_factor', 1.0)
                if V > 0:
                    thrust_dir = vel / V
                else:
                    thrust_dir = np.array([np.cos(heading_rad), np.sin(heading_rad), 0.5])
                ...
                accel = lift_dir * L / mass - vel_dir * D / mass - np.array([0, 0, 9.81])
        ...
        return MissionResult(
            ...
            trajectory=np.array(self.trajectory_history) if self.trajectory_history else None,
...
def analyze_dispersion(results: List[MissionResult]) -> Dict[str, Any]:
    ...
    miss_distances = [r.miss_distance_m for r in results if r.success or r.failure_mode == FailureMode.NONE]
    downrange_errors = [r.downrange_error_m for r in results if r.success or r.failure_mode == FailureMode.NONE]
    crossrange_errors = [r.crossrange_error_m for r in results if r.success or r.failure_mode == FailureMode.NONE]
...
def validate_mission_module():
    ...
    success_count = sum(1 for r in results if r.success or r.failure_mode == FailureMode.NONE)
    ...
    high_uncertainty = UncertaintyModel(
        density_sigma_pct=20.0,
        thrust_sigma_pct=10.0
    )
