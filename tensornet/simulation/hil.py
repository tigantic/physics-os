...
class GPSSensor(SensorModel):
    ...
    def measure(self, true_state: Dict[str, float], t: float) -> Dict[str, float]:
        ...
        # Position
        lat = self.apply_noise(
            true_state.get('lat', 0.0),
            self.position_noise, dt
        ) * self.hdop
        
        lon = self.apply_noise(
            true_state.get('lon', 0.0),
            self.position_noise, dt
        ) * self.hdop
        
        alt = self.apply_noise(
            true_state.get('alt', 0.0),
            self.position_noise, dt
        ) * self.vdop
...
class AirDataSensor(SensorModel):
    ...
    def measure(self, true_state: Dict[str, float], t: float) -> Dict[str, float]:
        ...
        p_static = self.apply_noise(
            true_state.get('p_static', 101325.0),
            self.pressure_noise, dt
        )
        
        q_dynamic = self.apply_noise(
            true_state.get('q_dynamic', 0.0),
            self.pressure_noise, dt
        )
        
        T_total = self.apply_noise(
            true_state.get('T_total', 288.15),
            self.temperature_noise, dt
        )
        
        alpha = self.apply_noise(
            true_state.get('alpha_deg', 0.0),
            self.angle_noise, dt
        )
        
        beta = self.apply_noise(
            true_state.get('beta_deg', 0.0),
            self.angle_noise, dt
        )
...
class ControlSurface(ActuatorModel):
    ...
    def update(self, command: float, dt: float) -> float:
        ...
        accel = self.omega_n**2 * error - 2 * self.damping * self.omega_n * self.velocity
        ...
        self.current_position = np.clip(
            ...
            self.position_limit_min,
            self.position_limit_max
        )
...
def create_vehicle_actuators(vehicle_class: str = "hypersonic") -> Dict[str, ActuatorModel]:
    """
    ...
    if vehicle_class == "hypersonic":
        actuators['elevator'] = ControlSurface(
            name='elevator',
            rate_limit_deg_s=60,
            position_limit_deg=30,
            bandwidth_hz=10
        )
        actuators['aileron'] = ControlSurface(
            name='aileron',
            rate_limit_deg_s=100,
            position_limit_deg=25,
            bandwidth_hz=15
        )
...
def validate_hil_module():
    ...
    print(f"True position: (34.0, -118.0, 10000)")
    ...
    print(f"True p_static: 50000 Pa")
    ...
    print(f"Command: 80% throttle")
    print(f"Max thrust: 100000 N")
    ...
    true_state = {'ax': 0, 'ay': 0, 'az': -9.81, 'p': 0, 'q': 0, 'r': 0, 'lat': 0, 'lon': 0, 'alt': 1000, 'vn': 100, 've': 0, 'vd': 0}
