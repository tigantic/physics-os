            ...
            def hook(module, input, output):
                if isinstance(output, torch.Tensor):
                    ...
                    if name in activation_ranges:
                        ...
                        activation_ranges[name] = (min(old_min, min_val), max(old_max, max_val))
                    else:
        ...
        for name, (min_val, max_val) in activation_ranges.items():
            if self.config.symmetric:
                ...
                if self.config.precision == Precision.INT8:
                    ...
                    self.zero_points[name] = torch.tensor(-min_val / self.scale_factors[name] - 128)
                elif self.config.precision == Precision.INT4:
                    self.scale_factors[name] = torch.tensor(range_val / 15.0)
                    self.zero_points[name] = torch.tensor(-min_val / self.scale_factors[name] - 8)
    ...
    def suggest_optimizations(self, profile: MemoryProfile) -> List[str]:
        ...
        if not profile.fits_in_memory(self.target.memory_mb):
            ...
            if overage < profile.parameter_bytes * 0.5:
                suggestions.append("Consider INT8 quantization (50% parameter reduction)")
            else:
                suggestions.append("Consider INT4 quantization (75% parameter reduction)")
...
def deploy_to_hardware(
    ...
    if precision in [Precision.INT8, Precision.INT4, Precision.FP16, Precision.BF16]:
        ...
        if calibration_data is not None and precision in [Precision.INT8, Precision.INT4]:
...
def estimate_inference_time(
    ...
    return {
        ...
        'estimated_throughput': 1e6 / estimated_time_us if estimated_time_us > 0 else float('inf')
    }
...
if __name__ == "__main__":
    ...
    print(f"  Estimated throughput: {estimate['estimated_throughput']:.0f} inferences/sec")
