...
Scientifically identifies the slowest functions in the tensornet
...
Usage:
    python -m tensornet.benchmarks.profile_bottlenecks
"""
...
def time_function(fn, iterations: int = 100, warmup: int = 10, name: str = "Operation") -> TimingResult:
    """
...
def profile_advection_gpu(grid_size: int = 512) -> TimingResult:
    ...
    u_vel = torch.rand((grid_size, grid_size), dtype=torch.float32, device=device) * 2.0 - 1.0
    v_vel = torch.rand((grid_size, grid_size), dtype=torch.float32, device=device) * 2.0 - 1.0
    dt = 0.01
...
def profile_gradient_cpu(grid_size: int = 512) -> TimingResult:
    ...
    def gradient_step():
        # Central differences
        du_dx = (torch.roll(field, -1, dims=1) - torch.roll(field, 1, dims=1)) / (2 * dx)
        du_dy = (torch.roll(field, -1, dims=0) - torch.roll(field, 1, dims=0)) / (2 * dy)
        return du_dx, du_dy
...
def profile_laplacian_cpu(grid_size: int = 512) -> TimingResult:
    ...
    def laplacian_step():
        # 5-point stencil
        lap = (
            (torch.roll(field, -1, dims=1) - 2*field + torch.roll(field, 1, dims=1)) / (dx**2) +
            (torch.roll(field, -1, dims=0) - 2*field + torch.roll(field, 1, dims=0)) / (dy**2)
        )
        return lap
...
def profile_tensor_contraction_cpu(bond_dim: int = 32, grid_size: int = 512) -> TimingResult:
...
def profile_full_ns_step(grid_size: int = 256) -> TimingResult:
    ...
    def ns_step():
        ...
        div = (torch.roll(u_star, -1, dims=1) - torch.roll(u_star, 1, dims=1)) / (2 * dx) + \
...
def run_profiling(output_file: str = "profile_results.json"):
    ...
    for r in results:
        if r.avg_ms > 0:
            print(f"{r.operation:<30} {r.avg_ms:>10.2f} {r.min_ms:>10.2f} {r.max_ms:>10.2f}")
