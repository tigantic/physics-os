...
class PrecisionBenchmark:
    ...
    def run(
        ...
        for precision in self.config.precision_modes:
            ...
            input_tensor = torch.randn(input_shape, dtype=dtype, device=self.config.device)
    ...
    def run_full_suite(
        ...
        for precision in self.config.precision_modes:
            ...
            for batch_size in self.config.batch_sizes:
                ...
                latency = latency_bench.run(model, input_tensor)
                memory = memory_bench.run(model, input_tensor)
                throughput = throughput_bench.run(model, input_tensor, duration_seconds=5.0)
    ...
    def _get_device_info(self) -> Dict[str, Any]:
        ...
        if torch.cuda.is_available():
            info.update({
                ...
                'device_capability': torch.cuda.get_device_capability(self.config.gpu_id),
...
def compare_precision_modes(
    ...
    comparison = {
        'fp32_avg_latency_ms': sum(r.latency.mean_ms for r in fp32_results) / len(fp32_results) if fp32_results else 0,
        'fp16_avg_latency_ms': sum(r.latency.mean_ms for r in fp16_results) / len(fp16_results) if fp16_results else 0,
        'speedup': 0,
        'results': [r.to_dict() for r in results],
    }
    
    if comparison['fp16_avg_latency_ms'] > 0:
        comparison['speedup'] = comparison['fp32_avg_latency_ms'] / comparison['fp16_avg_latency_ms']
