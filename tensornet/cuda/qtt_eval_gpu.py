...
class GPUQTTEvaluator:
    ...
    def load_qtt(self, qtt_cores):
        ...
        self.core_offsets = torch.tensor(
    ...
    def eval_grid_gpu(self, grid_size: int) -> torch.Tensor:
        ...
        if CUDA_AVAILABLE and self.cores_gpu is not None:
            ...
            points = torch.stack(
                [
                    torch.arange(grid_size, device=self.device).repeat_interleave(
                        grid_size
                    ),
            ...
            values = _cuda_module.qtt_eval_batch(
                self.cores_gpu, self.core_offsets, self.core_shapes, points, grid_size
            )
