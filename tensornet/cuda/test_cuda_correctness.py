...
sys.path.insert(0, '/home/brad/TiganticLabz/Main_Projects/Project HyperTensor/tensornet/cuda')
import tensornet_cuda


def python_advect_2d(density: torch.Tensor, velocity: torch.Tensor, dt: float) -> torch.Tensor:
    """
...
def test_correctness_2d(tolerance: float = 1e-5) -> bool:
    ...
    for size in sizes:
        ...
        velocity = (torch.rand((2, size, size), dtype=torch.float32, device=device) - 0.5) * 10.0
        dt = 0.1
        ...
        print(f"  {size}x{size}: max_diff={max_diff:.2e}, mean_diff={mean_diff:.2e} {status}")
        
        if not passed:
            all_passed = False
            # Debug info
            print(f"    Python max: {result_python.max().item():.6f}, min: {result_python.min().item():.6f}")
            print(f"    CUDA max:   {result_cuda.max().item():.6f}, min: {result_cuda.min().item():.6f}")
...
def test_performance():
    ...
    for size in sizes:
        density = torch.rand((size, size), dtype=torch.float32, device=device)
        velocity = torch.rand((2, size, size), dtype=torch.float32, device=device) * 2.0 - 1.0
        dt = 0.01
...
def test_edge_cases():
    ...
    print(f"  Zero velocity: max_diff={diff:.2e} {'[PASS]' if diff < 1e-3 else '[FAIL]'}")
    
    # Test 2: Large velocity (tests clamping)
    velocity_large = torch.ones((2, size, size), dtype=torch.float32, device=device) * 1000.0
    ...
    velocity_neg = torch.ones((2, size, size), dtype=torch.float32, device=device) * -1000.0
...
def main():
    ...
    test_vel = test_correctness_velocity_2d(tolerance=1e-3)  # Velocity has compounded error
    test_edge_cases()
    test_performance()
