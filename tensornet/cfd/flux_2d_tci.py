...
def qtt2d_eval_batch(qtt: QTT2DState, indices: Tensor) -> Tensor:
    ...
    for k in range(1, n_cores):
        ...
        result = torch.bmm(result.unsqueeze(1), core_slices).squeeze(1)  # [batch, r_right]
...
class Flux2DSampler:
    ...
    def __init__(self, rho: QTT2DState, rhou: QTT2DState, rhov: QTT2DState, 
                 E: QTT2DState, config: Flux2DConfig, axis: str = 'x'):
        """
    ...
    def _compute_rusanov_flux(self, indices: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """
...
def compute_flux_2d_tci(
    rho: QTT2DState, rhou: QTT2DState, rhov: QTT2DState, E: QTT2DState,
    axis: str, config: Flux2DConfig
...
def compute_flux_difference_2d(
    ...
    shift_mpo = make_interleaved_shift_mpo(
        n_cores, shift=-1, axis=axis,
        dtype=flux.cores[0].dtype, device=flux.cores[0].device
    )
