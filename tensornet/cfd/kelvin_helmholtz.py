...
from tensornet.cfd.qtt_2d import QTT2DState, morton_encode, morton_decode
...
def decode_morton_vectorized(indices: torch.Tensor, n_bits_per_dim: int) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
class KelvinHelmholtzSampler:
    ...
    def sample_rho(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        rho = self.config.rho_bottom + (self.config.rho_top - self.config.rho_bottom) * step
        return rho
    ...
    def sample_P(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        return self.config.P * torch.ones(len(indices), dtype=self.config.dtype, device=self.config.device)
...
def build_kh_via_dense(n_bits_per_dim: int = 7, config: KHConfig = None) -> Tuple[QTT2DState, ...]:
    """
    ...
    v = config.pert_amplitude * torch.sin(config.pert_wavenumber * math.pi * X) * \
...
def analyze_kh_ranks(n_bits_per_dim: int = 7, config: KHConfig = None):
    ...
    print(f"Kelvin-Helmholtz IC Rank Analysis ({2**n_bits_per_dim}x{2**n_bits_per_dim} grid)")
    print("=" * 60)
    ...
    for qtt, name in [(rho, 'rho'), (rhou, 'rho*u'), (rhov, 'rho*v'), (E, 'E')]:
        ...
        print(f"{info['name']:8s}: max_rank={info['max_rank']:3d}, compression={info['compression']:.1f}x")
...
def build_kh_via_tci(n_bits_per_dim: int = 10, config: KHConfig = None):
    ...
    rho_qtt, _ = qtt_from_function(
        sampler.sample_rho, total_qubits, 
        max_rank=config.max_rank, tolerance=config.tci_tolerance
    )
    
    print("  Sampling rho*u...")
    rhou_qtt, _ = qtt_from_function(
        sampler.sample_rhou, total_qubits,
        max_rank=config.max_rank, tolerance=config.tci_tolerance
    )
    
    print("  Sampling rho*v...")
    rhov_qtt, _ = qtt_from_function(
        sampler.sample_rhov, total_qubits,
        max_rank=config.max_rank, tolerance=config.tci_tolerance
    )
    
    print("  Sampling E...")
    E_qtt, _ = qtt_from_function(
        sampler.sample_E, total_qubits,
        max_rank=config.max_rank, tolerance=config.tci_tolerance
    )
...
if __name__ == "__main__":
    ...
    for z in test_indices:
        ...
        print(f"  z={z:4d} -> ({x.item():.3f}, {y.item():.3f}): rho={rho.item():.3f}, u={u.item():.3f}, v={v.item():.4f}")
