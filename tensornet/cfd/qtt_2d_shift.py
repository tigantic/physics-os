...
def apply_mpo_to_qtt2d(
    ...
    for k in range(len(state.cores)):
        ...
        assert (
            d_in == d_in_m
        ), f"Physical dimension mismatch at core {k}: {d_in} vs {d_in_m}"
...
def test_native_shift_x():
    ...
    print("Original square at x=[4,8), y=[4,8)")
    ...
    X, Y = torch.meshgrid(
        torch.arange(Nx).float(), torch.arange(Ny).float(), indexing="ij"
    )
    cx = (weights * X).sum().item()
    cy = (weights * Y).sum().item()

    print(f"Center of mass: ({cx:.1f}, {cy:.1f})")
    print("Expected: (6.5, 5.5) → (7.5, 5.5) after +X shift")
