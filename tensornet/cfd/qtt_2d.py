...
def dense_to_qtt_2d(
    field: torch.Tensor, max_bond: int = 64, tol: float = 1e-10
) -> QTT2DState:
    """
...
def shift_mpo_x_2d(
    n_qubits: int, device: torch.device = None, dtype: torch.dtype = torch.float32
) -> list[torch.Tensor]:
    """
    ...
    for k, core in enumerate(mpo):
        if k % 2 == 1:
            ...
            new_core = torch.zeros(
                r_left, 2, 2, r_right, dtype=core.dtype, device=core.device
            )
...
def truncate_qtt_2d(state: QTT2DState, max_rank: int) -> QTT2DState:
    ...
    for k in range(n - 1):
        ...
        cores[k + 1] = torch.einsum(
            ...
            S,
            Vh,
            cores[k + 1].reshape(r_right, -1, cores[k + 1].shape[2]),
        )
...
def test_2d_riemann_quadrant():
    ...
    print(
        f"  ρ: rank={rho_qtt.max_rank}, ratio={rho_qtt.dense_memory_bytes()/rho_qtt.memory_bytes:.1f}×"
    )
    print(
        f"  u: rank={u_qtt.max_rank}, ratio={u_qtt.dense_memory_bytes()/u_qtt.memory_bytes:.1f}×"
    )
    print(
        f"  v: rank={v_qtt.max_rank}, ratio={v_qtt.dense_memory_bytes()/v_qtt.memory_bytes:.1f}×"
    )
    print(
        f"  P: rank={P_qtt.max_rank}, ratio={P_qtt.dense_memory_bytes()/P_qtt.memory_bytes:.1f}×"
    )
