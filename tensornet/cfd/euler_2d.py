...
class Euler2DState:
    ...
    def to_conservative(self) -> Tensor:
        ...
        """
        return torch.stack(
            [self.rho, self.rho * self.u, self.rho * self.v, self.E], dim=0
        )

    @classmethod
    def from_conservative(
        cls, U: Tensor, gamma: float = gamma_default
    ) -> Euler2DState:
        """
...
class Euler2D:
    ...
    def __init__(
        ...
        self.x = torch.linspace(
            self.dx / 2, Lx - self.dx / 2, Nx, dtype=dtype, device=self.device
        )
        self.y = torch.linspace(
            self.dy / 2, Ly - self.dy / 2, Ny, dtype=dtype, device=self.device
        )
        ...
        # State
        self.state: Euler2DState | None = None
        ...
        self.inflow_state: Euler2DState | None = None
    ...
    def set_boundary_conditions(
        ...
        inflow_state: Euler2DState | None = None,
    ) -> None:
    ...
    def _apply_bc_x(self, U: Tensor, n_ghost: int = 2) -> Tensor:
        ...
        _, Ny, Nx = U.shape
        U_ext = torch.zeros(
            4, Ny, Nx + 2 * n_ghost, dtype=self.dtype, device=self.device
        )
        ...
        if (
            self.bc_right == BCType.OUTFLOW
            or self.bc_right == BCType.SUPERSONIC_OUTFLOW
        ):
    ...
    def _apply_bc_y(self, U: Tensor, n_ghost: int = 2) -> Tensor:
        ...
        _, Ny, Nx = U.shape
        U_ext = torch.zeros(
            4, Ny + 2 * n_ghost, Nx, dtype=self.dtype, device=self.device
        )
    ...
    def _hllc_2d(
        ...
        # Physical flux
        FL = torch.stack([rhoL * uL, rhoL * uL**2 + pL, rhoL * uL * vL, uL * (EL + pL)])
        FR = torch.stack([rhoR * uR, rhoR * uR**2 + pR, rhoR * uR * vR, uR * (ER + pR)])
        ...
        if SL >= 0:
            ...
            UL_star = torch.stack(
                [
                    ...
                    rhoL_star * vL,
                    rhoL_star
                    * (EL / rhoL + (SM - uL) * (SM + pL / (rhoL * (SL - uL)))),
                ]
            )
            ...
            UR_star = torch.stack(
                [
                    ...
                    rhoR_star * vR,
                    rhoR_star
                    * (ER / rhoR + (SM - uR) * (SM + pR / (rhoR * (SR - uR)))),
                ]
            )
    ...
    def step(self, dt: float | None = None, cfl: float = 0.4) -> float:
        """
    ...
    def solve(
        ...
        return {"time": self.time, "steps": steps, "final_dt": dt}
...
def oblique_shock_exact(M1: float, theta: float, gamma: float = gamma_default) -> dict:
    """
