...
from tensornet.cfd.nd_shift_mpo import (
    make_nd_shift_mpo, apply_nd_shift_mpo, truncate_cores
)
from tensornet.cfd.flux_2d_tci import (
    Flux2DConfig, compute_flux_2d_tci, qtt2d_eval_batch
)
...
class EulerND_Native:
    ...
    def __init__(self, config: EulerNDConfig):
        ...
        print(f"EulerND_Native initialized: {config.num_dims}D, {config.grid_size}^{config.num_dims} grid")
    ...
    def _evolve_axis_2d(self, state: EulerNDState, dt: float, axis: int) -> EulerNDState:
        """
        ...
        F_rho, F_rhou, F_rhov, F_E = compute_flux_2d_tci(
            state.rho, state.momentum[0], state.momentum[1], state.E,
            axis=axis_name, config=flux_config
        )
...
def create_kh_initial_condition_2d(config: EulerNDConfig) -> EulerNDState:
    ...
    # Shear layer
    rho = torch.where(torch.abs(Y - 0.5) < 0.25, 
                      torch.tensor(2.0, dtype=config.dtype),
                      torch.tensor(1.0, dtype=config.dtype))
    u = torch.where(torch.abs(Y - 0.5) < 0.25,
...
if __name__ == "__main__":
    ...
    # 2D test at 64x64
    config = EulerNDConfig(
        num_dims=2,
        qubits_per_dim=6,  # 64x64
        max_rank=48,
        cfl=0.4
    )
    ...
    for i in range(n_steps):
        ...
        t += dt
        print(f"  Step {i+1}: t={t:.5f}, dt={dt:.5f}, rank={state.max_rank()}, time={step_time:.2f}s")
