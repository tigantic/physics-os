...
class Euler1D:
    ...
    def __init__(
        ...
        # State
        self.state: EulerState | None = None
        self.t = 0.0
    ...
    def solve(
        self,
        t_final: float,
        callback: Callable[["Euler1D", float], None] | None = None,
        ...
        while self.t < t_final:
            ...
            if callback is not None:
                if (
                    callback_interval <= 0
                    or self.t - last_callback_t >= callback_interval
                ):
...
def sod_shock_tube_ic(
    ...
    dx = (x_max - x_min) / N
    x = torch.linspace(
        x_min + 0.5 * dx, x_max - 0.5 * dx, N, dtype=dtype, device=device
    )

    # Primitive variables
    rho = torch.where(
        x < x_discontinuity, torch.ones_like(x), 0.125 * torch.ones_like(x)
    )
    u = torch.zeros_like(x)
    p = torch.where(x < x_discontinuity, torch.ones_like(x), 0.1 * torch.ones_like(x))
...
def lax_shock_tube_ic(
    ...
    rho = torch.where(
        x < x_discontinuity, 0.445 * torch.ones_like(x), 0.5 * torch.ones_like(x)
    )
    u = torch.where(
        x < x_discontinuity, 0.698 * torch.ones_like(x), torch.zeros_like(x)
    )
    p = torch.where(
        x < x_discontinuity, 3.528 * torch.ones_like(x), 0.571 * torch.ones_like(x)
    )
