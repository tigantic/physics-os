...
class FlowState:
    ...
    def to_tensor(self, dtype=torch.float64, device="cpu") -> Tensor:
        ...
        return torch.tensor(
            [self.rho, self.u, self.v, self.p], dtype=dtype, device=device
        )


def apply_extrapolation_bc(
    U: Tensor, n_ghost: int, side: Literal["left", "right", "bottom", "top"]
) -> None:
    """
    ...
    if U.dim() == 2:
        ...
        if side == "left":
            ...
            U[:, :, -n_ghost:] = U[:, :, -n_ghost - 1 : -n_ghost].expand(
                -1, -1, n_ghost
            )
        elif side == "bottom":
...
def apply_periodic_bc(U: Tensor, n_ghost: int, axis: int) -> None:
    """
...
class BoundaryManager:
    ...
    def __init__(
        ...
        inflow_state: FlowState | None = None,
    ...
    def _apply_single(
        ...
        if bc_type == BCType.EXTRAPOLATION:
            ...
            apply_subsonic_outflow_bc(U, n_ghost, self.back_pressure, side, self.gamma)
