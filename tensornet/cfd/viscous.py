...
def sutherland_viscosity(
    T: Tensor, mu_ref: float = MU_REF, T_ref: float = T_REF, S: float = S_MU
) -> Tensor:
    """
...
def thermal_conductivity(mu: Tensor, cp: float = CP_AIR, Pr: float = PR_AIR) -> Tensor:
    """
...
def compute_transport_properties(
    T: Tensor, cp: float = CP_AIR, Pr: float = PR_AIR
) -> TransportProperties:
    """
...
def velocity_gradients_2d(
    u: Tensor, v: Tensor, dx: float, dy: float
) -> dict[str, Tensor]:
    """
...
def temperature_gradient_2d(T: Tensor, dx: float, dy: float) -> tuple[Tensor, Tensor]:
    """
...
def stress_tensor_2d(grads: dict[str, Tensor], mu: Tensor) -> dict[str, Tensor]:
    """
...
def heat_flux_2d(dTdx: Tensor, dTdy: Tensor, k: Tensor) -> tuple[Tensor, Tensor]:
    """
...
def viscous_flux_x_2d(
    u: Tensor, v: Tensor, tau: dict[str, Tensor], qx: Tensor
) -> Tensor:
    """
...
def viscous_flux_y_2d(
    u: Tensor, v: Tensor, tau: dict[str, Tensor], qy: Tensor
) -> Tensor:
    """
...
def viscous_flux_divergence_2d(Fv: Tensor, Gv: Tensor, dx: float, dy: float) -> Tensor:
    """
...
def viscous_timestep_limit(
    dx: float, dy: float, mu_max: float, rho_min: float, safety: float = 0.25
) -> float:
    """
...
def recovery_temperature(
    T_inf: float, M: float, r: float = 0.85, gamma: float = 1.4
) -> float:
    """
...
def heat_transfer_coefficient(q_wall: float, T_wall: float, T_recovery: float) -> float:
    """
