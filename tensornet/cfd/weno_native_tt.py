...
def _truncate_mpo(mpo: MPO, max_rank: int) -> MPO:
    ...
    for i in range(n - 1):
        ...
        new_cores[i + 1] = (SVh @ next_reshaped).reshape(
            rank, d_o_next, d_i_next, r_r_next
        )
...
def stencil_coefficient_mpo(
    ...
    result = (
        shift_mpo_cached(n_qubits, offset) if offset != 0 else identity_mpo(n_qubits)
    )
    result = _scale_mpo(result, weight)

    for offset, weight in coefficients[1:]:
        shift = (
            shift_mpo_cached(n_qubits, offset)
            if offset != 0
            else identity_mpo(n_qubits)
        )
    ...
    for i in range(n):
        ...
        if i == 0:
            ...
            new_core = torch.zeros(
                1, d_o, d_i, r1_r + r2_r, dtype=c1.dtype, device=c1.device
            )
            ...
            # (r1+r2, d_o, d_i, 1)
            new_core = torch.zeros(
                r1_l + r2_l, d_o, d_i, 1, dtype=c1.dtype, device=c1.device
            )
            ...
            # (r1+r2, d_o, d_i, r1+r2)
            new_core = torch.zeros(
                r1_l + r2_l, d_o, d_i, r1_r + r2_r, dtype=c1.dtype, device=c1.device
            )
...
def compute_smoothness_indicators_tt(
    ...
    # Get difference MPOs
    (d1_0_mpo, d2_0_mpo), (d1_1_mpo, d2_1_mpo), (d1_2_mpo, d2_2_mpo) = (
        smoothness_indicator_mpos(n_qubits)
    )
...
def weno_reconstruct_native_tt(
    ...
    config: WENONativeTTConfig | None = None,
) -> QTTState:
    """
