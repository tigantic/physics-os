...
def apply_shift_mpo(state: QTT2DState, mpo: List[torch.Tensor], 
...
def benchmark_native_vs_dense():
    ...
    print("-" * 60)
    for r in results:
        print(f"{r['grid']:<12} {r['points']:<10,} {r['native_ms']:<12.2f} {r['dense_ms']:<12.2f} {r['speedup']:<8.1f}×")
