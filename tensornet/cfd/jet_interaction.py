...
def pressure_matched_jet_diameter(
    ...
    # Exit area ratio
    M_e = exit_mach
    Ae_Astar = ((gp1/2)**(-(gp1)/(2*gm1)) * 
                (1 + gm1/2 * M_e**2)**((gp1)/(2*gm1)) / M_e)
    ...
    Aj_Astar = ((gp1/2)**(-(gp1)/(2*gm1)) * 
                (1 + gm1/2 * M_j**2)**((gp1)/(2*gm1)) / M_j)
...
def prandtl_meyer_expansion(mach: float, gamma: float = 1.4) -> float:
    ...
    nu = (math.sqrt(gp1/gm1) * math.atan(math.sqrt(gm1/gp1 * (mach**2 - 1))) - 
...
class UnderexpandedJet:
    ...
    def compute_plume(self, ambient_pressure: float) -> JetPlume:
        ...
        penetration = pressure_matched_jet_diameter(
            cfg.exit_diameter, cfg.exit_mach,
            cfg.exit_pressure, ambient_pressure, cfg.gamma
        )
...
class JetInteractionCorrector:
    ...
    def compute_interaction_forces(
        ...
        active_jets: Optional[List[int]] = None
    ) -> InteractionForces:
        """
    ...
    def correct_thrust(
        ...
        forces = self.compute_interaction_forces(
            freestream_mach, freestream_pressure,
            freestream_density, freestream_velocity,
            reference_area, active_jets
        )
