...
from tensornet.cfd.qtt_2d_shift_native import make_interleaved_shift_mpo, apply_shift_mpo
...
class Euler2DState:
    ...
    def __init__(self, rho: QTT2DState, rhou: QTT2DState, rhov: QTT2DState, E: QTT2DState):
        self.rho = rho
        self.rhou = rhou
        self.rhov = rhov
        self.E = E
...
class Euler2D_Strang:
    ...
    def _shift_field(self, field: QTT2DState, direction: str, amount: int = 1) -> QTT2DState:
    ...
    def evolve(self, state: Euler2DState, t_final: float, 
               callback: Callable = None, callback_interval: int = 10) -> Euler2DState:
        """
...
def create_kelvin_helmholtz_ic(nx: int = 7, ny: int = 7, 
                               config: Euler2DConfig = None) -> Euler2DState:
    """
    ...
    sigma = 0.1
    pert_amp = 0.1
    v = pert_amp * torch.sin(4 * math.pi * X) * torch.exp(-((Y - 0.5)**2) / (sigma**2))
...
if __name__ == "__main__":
    ...
    t_final = 0.1
    state = solver.evolve(state, t_final, callback=progress_callback, callback_interval=5)
