...
class DifferentiableRoe:
    ...
    def __call__(
        ...
        # Roe dissipation
        diss_rho = (
            abs_lambda_1 * alpha_1 + abs_lambda_2 * alpha_2 + abs_lambda_3 * alpha_3
        )
        diss_rhou = (
            abs_lambda_1 * alpha_1 * (u_roe - c_roe)
            + abs_lambda_2 * alpha_2 * u_roe
            + abs_lambda_3 * alpha_3 * (u_roe + c_roe)
        )
        diss_E = (
            abs_lambda_1 * alpha_1 * (H_roe - u_roe * c_roe)
...
class DifferentiableHLLC:
    ...
    def __call__(
        ...
        E_L_star = rho_L_star * (
            E_L / rho_L
            + (S_star - u_L) * (S_star + p_L / (rho_L * (S_L - u_L) + 1e-10))
        )
        E_R_star = rho_R_star * (
            E_R / rho_R
            + (S_star - u_R) * (S_star + p_R / (rho_R * (S_R - u_R) + 1e-10))
        )
        ...
        F_rho = (1 - w_L) * F_rho_L + w_L * (
            (1 - w_star) * F_rho_L_star
            + w_star * ((1 - w_R) * F_rho_R_star + w_R * F_rho_R)
        )
        F_rhou = (1 - w_L) * F_rhou_L + w_L * (
            (1 - w_star) * F_rhou_L_star
            + w_star * ((1 - w_R) * F_rhou_R_star + w_R * F_rhou_R)
        )
        F_E = (1 - w_L) * F_E_L + w_L * (
            (1 - w_star) * F_E_L_star + w_star * ((1 - w_R) * F_E_R_star + w_R * F_E_R)
        )
...
class DifferentiableEuler1D(nn.Module):
    ...
    def conservative_to_primitive(
        self, rho: Tensor, rhou: Tensor, E: Tensor
    ) -> tuple[Tensor, Tensor, Tensor]:
    ...
    def compute_dt(self, rho: Tensor, u: Tensor, p: Tensor) -> Tensor:
    ...
    def step(
        self, rho: Tensor, rhou: Tensor, E: Tensor, dt: Tensor | None = None
    ) -> tuple[Tensor, Tensor, Tensor]:
        """
...
class DifferentiableEuler2D(nn.Module):
    ...
    def primitive_to_conservative(
        self, rho: Tensor, u: Tensor, v: Tensor, p: Tensor
    ...
    def conservative_to_primitive(
        self, rho: Tensor, rhou: Tensor, rhov: Tensor, E: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        """Convert conservative to primitive variables."""
        u = rhou / rho
        v = rhov / rho
    ...
    def compute_dt(self, rho: Tensor, u: Tensor, v: Tensor, p: Tensor) -> Tensor:
    ...
    def x_sweep(
        self, rho: Tensor, rhou: Tensor, rhov: Tensor, E: Tensor, dt: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        """X-direction sweep."""
        ny, nx = rho.shape
        ...
        for j in range(ny):
            ...
            for i in range(nx + 1):
                ...
                if i > 0:
                    ...
                    rhov[j, i - 1] = rhov[j, i - 1] - dtdx * (
                        rho_row[i] * v_row[i] * u_row[i]
                    )
                ...
                if i < nx:
                    ...
                    rhov[j, i] = rhov[j, i] + dtdx * (
                        rho_row[i + 1] * v_row[i + 1] * u_row[i + 1]
                    )
...
def euler_residual_loss(
    rho: Tensor, u: Tensor, p: Tensor, x: Tensor, t: Tensor, gamma: float = 1.4
) -> Tensor:
    """
