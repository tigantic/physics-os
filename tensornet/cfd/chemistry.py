...
class Reaction:
    ...
    third_body_efficiencies: Optional[Dict[Species, float]] = None
...
REACTIONS = [
    ...
    Reaction(
        ...
        third_body_efficiencies={
            Species.N2: 1.0, Species.O2: 1.0, Species.NO: 1.0,
            Species.N: 5.0, Species.O: 5.0  # Atoms more efficient
        }
    ),
    # Reaction 2: N₂ + M ⇌ 2N + M
    Reaction(
    ...
    Reaction(
        ...
        third_body=True,
        third_body_efficiencies={
            Species.N2: 1.0, Species.O2: 1.0, Species.NO: 22.0,
            Species.N: 22.0, Species.O: 22.0
        }
    ),
...
def third_body_concentration(
    reaction: Reaction,
    concentrations: Dict[Species, torch.Tensor]
) -> torch.Tensor:
    """
...
def advance_chemistry_explicit(
    state: ChemistryState,
    dt: float
) -> ChemistryState:
    """
...
def post_shock_composition(
    ...
    T2_T1 = (1 + (gamma-1)/2 * M**2) * (2*gamma/(gamma-1) * M**2 - 1) / \
