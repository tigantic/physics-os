...
class NS3DSolver:
    ...
    def compute_rhs(
        ...
        adv_u, adv_v, adv_w = compute_advection_3d(
            state.u, state.v, state.w, self.dx, self.dy, self.dz, method="spectral"
        )
    ...
    def step_forward_euler(
        ...
        proj = project_velocity_3d(
            u_star, v_star, w_star, self.dx, self.dy, self.dz, dt=1.0, method="spectral"
        )
    ...
    def compute_diagnostics(
        ...
        state: NSState3D,
        dt: float,
        initial_max_vorticity: float | None = None,
    ) -> NSDiagnostics3D:
    ...
    def compute_stable_dt(
        ...
        # Viscous stability
        dt_visc = (
            cfl_target * min(self.dx**2, self.dy**2, self.dz**2) / (6 * self.nu + 1e-10)
        )
    ...
    def solve(
        ...
        if verbose:
            print(
                f"NS3D Solver ({method.upper()}): t_final={t_final:.4f}, dt={dt:.2e}, ν={self.nu:.2e}"
            )
        ...
        for step_idx in range(max_steps):
            ...
            if step_idx % diag_interval == 0 or state.t >= t_final:
                ...
                if verbose and step_idx % (diag_interval * 5) == 0:
                    print(
                        f"  t={state.t:.4f}: KE={diag.kinetic_energy:.4e}, "
        ...
        if verbose:
            ...
            print(
                f"  Final: KE={final_diag.kinetic_energy:.4e}, "
...
def test_taylor_green_3d():
    ...
    div_init = compute_divergence_3d(
        state.u, state.v, state.w, solver.dx, solver.dy, solver.dz, method="spectral"
    )
    ...
    print(
        f"Decay rate: numerical={decay_rate_numerical:.4f}, exact={decay_rate_exact:.4f}"
    )
...
def test_3d_projection():
    ...
    div_before = compute_divergence_3d(
        u_star, v_star, w_star, dx, dy, dz, method="spectral"
    )
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(
        f"3D Operators: Gradient err={ops_result['gradient_error']:.2e}, "
        f"Laplacian err={ops_result['laplacian_error']:.2e}"
    )
    print(f"3D Projection: div_after={proj_result['divergence_after']:.2e}")
    print(
        f"3D Taylor-Green: decay_err={tg_result['decay_error_pct']:.2f}%, "
        f"max_div={tg_result['max_divergence']:.2e}"
    )

    all_pass = (
        ops_result["gradient_error"] < 1e-10
        and ops_result["laplacian_error"] < 1e-10
        and proj_result["gate"]
        and tg_result["all_pass"]
    )
