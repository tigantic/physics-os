...
class PathPlanner:
    ...
    def _plan_rrt(
        ...
        for _ in range(self.config.max_iterations):
            ...
            if (
                math.sqrt((rx - goal[0]) ** 2 + (ry - goal[1]) ** 2)
    ...
    def _reconstruct_path(
        ...
        waypoints = []
        current: GridNode | None = goal_node

        while current:
            waypoints.append(Waypoint(position=(float(current.x), float(current.y))))
            current = current.parent
...
def smooth_path(
    ...
    for _ in range(iterations):
        for i in range(1, len(smoothed) - 1):
            ...
            smoothed[i] = smoothed[i] * (1 - smoothing_factor) + mid * smoothing_factor
