...
class Obstacle:
    ...
    def predicted_position(self, dt: float) -> tuple[float, ...]:
        ...
        Returns:
            Predicted position
        """
        return tuple(p + v * dt for p, v in zip(self.position, self.velocity))
    ...
    def distance_to(
        ...
        center_dist = math.sqrt(sum((a - b) ** 2 for a, b in zip(point, pred_pos)))
...
class ObstacleAvoidance:
    ...
    def prune_expired(self) -> int:
        ...
        Returns:
            Number removed
        """
        to_remove = [oid for oid, obs in self.obstacles.items() if obs.is_expired]
    ...
    def _potential_field_avoidance(
        ...
        if magnitude > self.config.max_avoidance:
            total_force = tuple(
                f / magnitude * self.config.max_avoidance for f in total_force
            )
    ...
    def is_path_clear(
        ...
        num_samples = max(
            10, int(math.sqrt(sum((a - b) ** 2 for a, b in zip(start, end))))
        )
