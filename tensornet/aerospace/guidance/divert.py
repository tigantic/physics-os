...
class VehicleState:
    ...
    angular_rate: tuple[float, float, float] | None = None
...
def closing_velocity(
    kv_velocity: tuple[float, float, float], target_velocity: tuple[float, float, float]
) -> float:
...
def line_of_sight_rate(
    kv_state: VehicleState, target_state: TargetState
...
def time_to_go(kv_state: VehicleState, target_state: TargetState) -> float:
    """
...
def zero_effort_miss(
    kv_state: VehicleState, target_state: TargetState, tgo: float
) -> tuple[float, float, float]:
    """
...
def proportional_navigation(
    kv_state: VehicleState, target_state: TargetState, N: float = 3.0
) -> tuple[float, float, float]:
    """
...
class DivertThruster:
    ...
    def compute_pulse_width(
        self, acceleration_component: float, vehicle_mass: float, dt: float
    ) -> float:
        """
...
class DivertGuidance:
    ...
    def compute_guidance(
        self, kv_state: VehicleState, target_state: TargetState, dt: float
    ) -> GuidanceCommand:
        """
        ...
        if self.guidance_law == "pn":
            ...
            a_cmd = augmented_proportional_navigation(
                kv_state, target_state, self.nav_constant
            )
        else:  # optimal
    ...
    def _allocate_thrusters(
        self, a_cmd: tuple[float, float, float], mass: float, dt: float
    ) -> dict[int, float]:
        """
        ...
        # Required force
        F_cmd = torch.tensor(
            [a_cmd[0] * mass, a_cmd[1] * mass, a_cmd[2] * mass], dtype=torch.float64
        )
