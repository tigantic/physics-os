...
from tensornet.physics.hypersonic import HazardField, calculate_hazard_field
...
def sample_cost_along_path(
    ...
    if ndim == 2:
        ...
        sampled = F.grid_sample(cost_4d, grid, mode='bilinear', 
        ...
        sampled = F.grid_sample(cost_5d, grid, mode='trilinear',
                                padding_mode='border', align_corners=True)
        return sampled.view(-1)
...
def optimize_trajectory_fast_marching(
    ...
    if len(path_indices) > 2:
        ...
        path_resampled = torch.tensor(path_resampled_np, device=device, dtype=torch.float32)
    else:
    ...
    with torch.no_grad():
        ...
        path_length = torch.sum(torch.norm(path_resampled[1:] - path_resampled[:-1], dim=-1))
...
def find_optimal_trajectory(
    ...
    **kwargs,
    ...
    # Convert to tensors
    ndim = len(start)
    start_t = torch.tensor(start[:cost_field.dim()], device=device, dtype=torch.float32)
    end_t = torch.tensor(end[:cost_field.dim()], device=device, dtype=torch.float32)
    
    if method == 'gradient':
        return optimize_trajectory_gradient(
            start_t, end_t, cost_field, bounds[:cost_field.dim()],
            num_waypoints=num_waypoints, **kwargs
        )
...
def demo_trajectory_optimizer():
    ...
    traj_grad = find_optimal_trajectory(
        hazard, start + (0.0,), end + (0.0,), bounds + ((0, 1),),
        method='gradient', num_waypoints=50, max_iterations=200, verbose=True
    )
    ...
    traj_fm = find_optimal_trajectory(
        hazard, start + (0.0,), end + (0.0,), bounds + ((0, 1),),
        method='fast_marching', num_waypoints=50
    )
