...
from tensornet.physics.hypersonic import (
    calculate_hazard_field,
    HazardField,
    VehicleConfig,
)
from tensornet.physics.trajectory_optimizer import (
...
def generate_synthetic_weather(
    ...
    storm1 = 0.2 * torch.exp(-((lon - 0.3)**2 + (lat + 0.2)**2 + (alt - 0.3)**2) / 0.05)
    storm2 = 0.15 * torch.exp(-((lon + 0.4)**2 + (lat - 0.3)**2 + (alt - 0.4)**2) / 0.08)
...
def run_integration_test():
    ...
    print(f"  Density Range: {weather['density'].min():.4f} - {weather['density'].max():.4f} kg/m³")
    print(f"  Wind U Range: {weather['wind_u'].min():.1f} - {weather['wind_u'].max():.1f} m/s")
    print(f"  Time: {t_weather:.2f} ms")
    
    test1_pass = weather['density'].numel() > 0
    results['tests'].append({'name': 'Weather Generation', 'passed': test1_pass, 'time_ms': t_weather})
    ...
    print(f"  Dynamic Pressure: {hazard.dynamic_pressure.min():.0f} - {hazard.dynamic_pressure.max():.0f} Pa")
    print(f"  Wall Temperature: {hazard.stagnation_temp.min():.0f} - {hazard.stagnation_temp.max():.0f} K")
    print(f"  Wind Shear: {hazard.wind_shear.min():.4f} - {hazard.wind_shear.max():.4f} 1/s")
    finite_costs = hazard.total_cost[torch.isfinite(hazard.total_cost)]
    if finite_costs.numel() > 0:
        print(f"  Total Cost Range: {finite_costs.min():.2f} - {finite_costs.max():.2f}")
    else:
        print(f"  Total Cost: All infinite (no safe regions at Mach 10)")
    print(f"  Time: {t_hazard:.2f} ms")
    
    test2_pass = t_hazard < 100.0 and hazard.total_cost.numel() > 0
    results['tests'].append({'name': 'Hazard Field', 'passed': test2_pass, 'time_ms': t_hazard})
    ...
    test3_pass = (
        len(trajectory.waypoints) == 100 and
        t_optim < 5000  # 5 seconds max (convergence optional)
    )
    results['tests'].append({'name': 'Trajectory Optimization', 'passed': test3_pass, 'time_ms': t_optim})
    results['passed' if test3_pass else 'failed'] += 1
    print(f"  Status: {'✓ PASSED' if test3_pass else '✗ FAILED'} (target: <5s, 100 waypoints)")
    ...
    print(f"  First Waypoint: ({waypoints_list[0]['lat']:.4f}, {waypoints_list[0]['lon']:.4f})")
    print(f"  Last Waypoint: ({waypoints_list[-1]['lat']:.4f}, {waypoints_list[-1]['lon']:.4f})")
    
    # Save output
    output_path = '/home/brad/TiganticLabz/Main_Projects/Project HyperTensor/tensornet/physics/trajectory_output.json'
    with open(output_path, 'w') as f:
        f.write(json_output)
    print(f"  Saved: {output_path}")
    
    test4_pass = json_size > 1000 and len(waypoints_list) == 100
    results['tests'].append({'name': 'Waypoint Output', 'passed': test4_pass, 'size_bytes': json_size})
