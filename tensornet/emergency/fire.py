...
class FireSim:
    ...
    def __init__(
        ...
        print(f"[FIRE] Simulation initialized")
    ...
    def step(self) -> None:
        ...
        advected_heat = torch.roll(
            torch.roll(self.heat, wind_shift_x, dims=0),
            wind_shift_y, dims=1
        )
        ...
        if self.wind_speed > 3.0 and torch.rand(1).item() < self.spotting_probability:
            ...
            if len(burning_indices) > 0:
                ...
                if 0 <= spot_x < self.size and 0 <= spot_y < self.size:
                    if self.fuel[spot_x, spot_y] > 0 and not self.burned[spot_x, spot_y]:
...
def run_wildfire_prediction(steps: int = 100) -> FireReport:
    ...
    for t in range(steps):
        ...
        if t % 25 == 0:
            ...
            print(f"Hour {t}: Active={report.active_cells} acres, "
