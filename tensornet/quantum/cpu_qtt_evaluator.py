...
@jit(nopython=True, cache=True)
def qtt_contract_single(morton_idx: int, cores_flat: np.ndarray, 
                       core_shapes: np.ndarray, core_offsets: np.ndarray,
                       n_cores: int) -> float:
    """
...
@jit(nopython=True, parallel=True, cache=True)
def qtt_eval_batch_numba(morton_indices: np.ndarray, cores_flat: np.ndarray,
                        core_shapes: np.ndarray, core_offsets: np.ndarray,
    ...
    for i in prange(n_samples):
        values[i] = qtt_contract_single(
            morton_indices[i], cores_flat, core_shapes, 
            core_offsets, n_cores
        )
...
class CPUQTTEvaluator:
    ...
    def load_qtt(self, qtt: QTT2DState):
        ...
        for k, core in enumerate(qtt.cores):
            if core.ndim != 3 or core.shape[1] != 2:
                raise ValueError(f"Core {k} has invalid shape {core.shape}, expected [r_left, 2, r_right]")
    ...
    def eval_sparse_grid(self, grid_size: int) -> Tuple[np.ndarray, float]:
        ...
        values_flat = qtt_eval_batch_numba(
            morton_flat, self.cores_flat, self.core_shapes,
            self.core_offsets, self.n_cores
        )
        t1 = time.perf_counter()
