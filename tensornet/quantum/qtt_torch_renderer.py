...
from tensornet.cfd.qtt_2d import QTT2DState, morton_encode
...
class QTTTorchRenderer:
    ...
    def _generate_morton_grid(self, width: int, height: int, n_bits: int) -> torch.Tensor:
        """
        ...
        y_coords = torch.arange(height, dtype=torch.long, device=self.device).unsqueeze(1)
        x_coords = torch.arange(width, dtype=torch.long, device=self.device).unsqueeze(0)
    ...
    def benchmark_synthesis(self, qtt: QTT2DState, width: int, height: int, n_trials: int = 100):
        """
...
def test_qtt_torch_renderer():
    ...
    for width, height, name in test_cases:
        ...
        print(f"  Estimated full eval: {estimated_eval_time:.0f} ms ({1000/estimated_eval_time:.1f} FPS)")
        print(f"  CONCLUSION: {'VIABLE' if estimated_eval_time < 16.67 else 'TOO SLOW'} for 60 FPS")
