...
class HybridQTTRenderer:
    ...
    def render_qtt_hybrid(
        ...
        output_width: int,
        output_height: int,
        colormap: Optional[torch.Tensor] = None,
        ...
        try:
            from tensornet.cuda.qtt_eval_gpu import hybrid_qtt_eval
            
            start_eval = time.perf_counter()
            sparse_tensor, eval_time = hybrid_qtt_eval(qtt.cores, sparse_size, self.device)
        ...
        sparse_4d = sparse_tensor.unsqueeze(0).unsqueeze(0)  # [1, 1, sparse_size, sparse_size]
    ...
    def benchmark_hybrid(
        ...
        print("=" * 70)
        print(f"Output: {output_width}×{output_height} ({output_width*output_height/1e6:.1f}M pixels)")
        ...
        for sparse_size in sparse_sizes:
            print(f"\n{'='*70}")
            print(f"Sparse Grid: {sparse_size}×{sparse_size} ({sparse_size**2/1000:.0f}K samples)")
            print(f"Upsampling: {output_width/sparse_size:.1f}× horizontal, {output_height/sparse_size:.1f}× vertical")
            ...
            # Statistics
            keys = ['cpu_eval_ms', 'upload_ms', 'interpolate_ms', 'colormap_ms', 'total_ms']
            stats = {}
            for key in keys:
            ...
            print(f"  CPU Evaluation:  {stats['cpu_eval_ms']['mean']:6.2f} ms (±{stats['cpu_eval_ms']['std']:.2f})")
            print(f"  GPU Upload:      {stats['upload_ms']['mean']:6.2f} ms (±{stats['upload_ms']['std']:.2f})")
            print(f"  GPU Interpolate: {stats['interpolate_ms']['mean']:6.2f} ms (±{stats['interpolate_ms']['std']:.2f})")
            print(f"  GPU Colormap:    {stats['colormap_ms']['mean']:6.2f} ms (±{stats['colormap_ms']['std']:.2f})")
            print(f"  ─────────────────────────")
            print(f"  TOTAL:           {stats['total_ms']['mean']:6.2f} ms (±{stats['total_ms']['std']:.2f})")
            print(f"  Min/Max:         {stats['total_ms']['min']:.2f} / {stats['total_ms']['max']:.2f} ms")
