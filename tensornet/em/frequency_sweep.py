...
Dependencies
------------
- ``tensornet.em.s_parameters``: ``Port``, ``port_source_tt``,
  ``extract_mode_coefficients_lsq``, ``compute_s11``,
  ``SParameterResult``, ``compute_s_matrix_1d``,
  ``solve_and_extract_s11``, ``s_to_db``
- ``tensornet.em.boundaries``: ``Geometry1D``, ``helmholtz_mpo_with_bc``,
  ``PMLConfig``
- ``tensornet.em.qtt_helmholtz``: ``tt_amen_solve``, ``reconstruct_1d``,
  ``array_to_tt``
"""
...
class FrequencySweepResult:
    ...
    def validate_passivity(self, tol: float = 0.01) -> NDArray:
        ...
        """
        from tensornet.em.s_parameters import validate_s_matrix_passivity
        return np.array([
