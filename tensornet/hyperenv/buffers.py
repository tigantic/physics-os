...
class Trajectory:
    ...
    def compute_gae(
        ...
        for t in reversed(range(len(self.rewards))):
            ...
            delta = (
                self.rewards[t] + gamma * next_value * (1 - self.dones[t]) - values[t]
            )
...
class Batch:
    ...
    values: np.ndarray | None = None
    log_probs: np.ndarray | None = None
    weights: np.ndarray | None = None  # For prioritized replay
    indices: np.ndarray | None = None  # For priority updates
    ...
    def to_torch(
        ...
        result = {
            ...
            "next_observations": torch.from_numpy(self.next_observations)
            .float()
            .to(device),
...
class ReplayBuffer:
    ...
    def _init_storage(
        ...
        self._next_observations = np.zeros(
            (self.capacity, *obs_shape), dtype=np.float32
        )
...
class RolloutBuffer:
    ...
    def compute_returns_and_advantages(
        ...
        for t in reversed(range(self._ptr)):
            ...
            running_gae = (
                delta + self.gamma * self.gae_lambda * next_non_terminal * running_gae
            )
            self._advantages[t] = running_gae

        # Returns = advantages + values
        self._returns[: self._ptr] = (
            self._advantages[: self._ptr] + self._values[: self._ptr]
        )
