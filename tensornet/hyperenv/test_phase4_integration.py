...
def test_hypersonic_env():
    ...
    from tensornet.hyperenv.hypersonic_env import HypersonicEnv, make_hypersonic_env
    ...
    print(f"  ✓ Reward function: R = Vel*0.1 - Heat*2.0 - TubeDist*5.0")
...
def test_swarm_command():
    ...
    test_commands = [
        ("Swarm Alpha, intercept trajectory vector 3-5-0 at Mach 8", SwarmCommandType.INTERCEPT),
    ...
    for text, expected_type in test_commands:
        ...
        if actual_type == expected_type.value:
            ...
            print(f"  ✗ '{text[:40]}...' -> {actual_type} (expected {expected_type.value})")
