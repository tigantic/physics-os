    ...
    def _get_observations(
        ...
        for agent_id in self._agent_ids:
            ...
            if base_obs is not None:
                ...
                elif hasattr(self, "_base_env") and hasattr(
                    self._base_env, "observation_space"
                ):
                    obs = np.zeros(
                        self._base_env.observation_space.shape, dtype=np.float32
                    )
                else:
...
class SelfPlayEnv(MultiAgentEnv):
        ...
        if config is None:
            config = MultiAgentConfig(
                teams=[
                    ...
                    TeamConfig(
                        "opponent", size=num_opponents, role=AgentRole.ADVERSARY
                    ),
                ]
            )
    ...
    def sample_opponent(self) -> AgentState | None:
