...
class HypersonicEnv(gym.Env):
    ...
    def _apply_action(self, action: np.ndarray):
        ...
        self.aircraft.thrust = np.clip(
            self.aircraft.thrust + thrust_change,
            0.0, 1.0
        )
    ...
    def _physics_step(self):
        ...
        heat_flux = k_sutton * np.sqrt(stag_rho / self.vehicle.nose_radius_m) * stag_v**3
    ...
    def _calculate_reward(self, terminated: bool) -> float:
        ...
        # Crash penalty
        crash_penalty = self.config.crash_penalty if terminated and not self.aircraft.alive else 0.0
