...
class GoldenValueStore:
    ...
    def save(
        ...
        **metadata,
        ...
        if isinstance(golden.value, (np.ndarray, torch.Tensor)):
            ...
            arr = (
                golden.value.cpu().numpy()
                if isinstance(golden.value, torch.Tensor)
                else golden.value
            )
            ...
            with open(value_path, "w") as f:
                json.dump(
                    {"value": golden.value, "type": type(golden.value).__name__}, f
                )
    ...
    def load(self, name: str) -> GoldenValue | None:
        """
        Load a golden value.
...
class RegressionTest:
    ...
    golden_name: str
    comparator: ArrayComparator | TensorComparator | StateComparator = field(
