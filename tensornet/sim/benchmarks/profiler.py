...
class ProfileResult:
    ...
    # Operation profiles
    operation_profiles: dict[OperationType, OperationProfile] = field(
    ...
    def get_top_layers(self, n: int = 10) -> list[LayerProfile]:
        ...
        return sorted(self.layer_profiles, key=lambda x: x.total_time_ms, reverse=True)[
            :n
        ]
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "operations": {
                op.name: p.to_dict() for op, p in self.operation_profiles.items()
            },
...
class TensorRTProfiler:
    ...
    def profile(
        ...
        operation_profiles = self._build_operation_profiles(
            layer_profiles, avg_total_time
        )
    ...
    def _build_layer_profiles(self, total_time_ms: float) -> list[LayerProfile]:
        ...
        for name, timer in self._layer_timers.items():
            ...
            profile = LayerProfile(
                ...
                percentage=(
                    100 * stats["avg"] / total_time_ms if total_time_ms > 0 else 0
                ),
            ...
            if module is not None:
                profile.flops = self._estimate_flops(
                    module, info.get("input_shapes", [])
                )
    ...
    def _build_operation_profiles(
        ...
        for op, stats in op_stats.items():
            profiles[op] = OperationProfile(
                ...
                percentage=(
                    100 * stats["time"] / total_time_ms if total_time_ms > 0 else 0
                ),
    ...
    def _classify_operation(self, layer_type: str) -> OperationType:
        ...
        if "conv" in layer_type:
            ...
            act in layer_type for act in ["relu", "gelu", "silu", "sigmoid", "tanh"]
        ):
            ...
            reshape in layer_type
            for reshape in ["reshape", "view", "flatten", "squeeze"]
        ):
