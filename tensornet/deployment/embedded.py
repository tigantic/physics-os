...
class MemoryProfile:
    ...
    def available_mb(self) -> int:
        ...
        return (self.total_system_mb - self.reserved_system_mb - 
                self.model_weights_mb - self.inference_buffer_mb -
                self.io_buffer_mb - self.safety_margin_mb)
    
    @property
    def utilization_pct(self) -> float:
        """Memory utilization percentage."""
        used = (self.reserved_system_mb + self.model_weights_mb + 
                self.inference_buffer_mb + self.io_buffer_mb)
    ...
    def allocate(self, name: str, shape: Tuple[int, ...]) -> torch.Tensor:
        ...
        with self._lock:
            if self.offset + size > len(self.pool):
                raise MemoryError(f"Pool exhausted. Need {size}, have {len(self.pool) - self.offset}")
    ...
    def infer(
        ...
        try:
            if model_info['type'] == 'onnxruntime':
                ...
                np_inputs = {model_info['input_name']: next(iter(inputs.values())).cpu().numpy()}
        ...
        if len(self._inference_times) > 0:
            self.metrics.throughput_hz = 1000.0 / (sum(self._inference_times) / len(self._inference_times))
