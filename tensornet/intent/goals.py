    ...
    def progress(
        ...
        **context,
        ...
        if self.type == GoalType.OPTIMIZE:
            ...
            initial = context.get(
                "initial_value", float("inf") if self.minimize else float("-inf")
            )
            ...
            initial_error = context.get(
                "initial_error", np.max(np.abs(self.target_field))
            )
...
class ActionPlan:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "completed_steps": sum(
                1 for s in self.steps if s.status == GoalStatus.COMPLETED
            ),
...
class GoalCoordinator:
    ...
    def __init__(self, director: GoalDirector | None = None):
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            goal.name: {
                ...
                "type": goal.type.name,
                "plan_status": self._active_plans.get(
                    goal.name, ActionPlan(goal=goal)
                ).status.value,
            }
            for goal in self._goals
        }
