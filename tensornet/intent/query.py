...
class Selector:
    ...
    @classmethod
    def gradient(cls, axis: Optional[int] = None) -> 'Selector':
...
class Aggregator:
    ...
    axis: Optional[int] = None  # None = all axes
    ...
    def apply(
        ...
        if self.agg_type == AggregatorType.NONE:
            ...
            if self.weights is not None:
                return np.nansum(values * self.weights, axis=self.axis) / np.nansum(self.weights)
    ...
    @classmethod
    def sum(cls, axis: Optional[int] = None) -> 'Aggregator':
        return cls(agg_type=AggregatorType.SUM, axis=axis)
    
    @classmethod
    def mean(cls, axis: Optional[int] = None) -> 'Aggregator':
        return cls(agg_type=AggregatorType.MEAN, axis=axis)
    
    @classmethod
    def max(cls, axis: Optional[int] = None) -> 'Aggregator':
        return cls(agg_type=AggregatorType.MAX, axis=axis)
    
    @classmethod
    def min(cls, axis: Optional[int] = None) -> 'Aggregator':
...
class FieldQuery:
    ...
    def where(
        self,
        predicate: Optional[Union[Predicate, Callable]] = None,
        **kwargs,
    ) -> 'FieldQuery':
        """
    ...
    def aggregate(
        self,
        aggregator: Optional[Union[Aggregator, str]] = None,
    ) -> 'FieldQuery':
        """
        Set aggregation.
    ...
    def maximize(self, field: Optional[str] = None) -> 'FieldQuery':
    ...
    def minimize(self, field: Optional[str] = None) -> 'FieldQuery':
    ...
    def average(self, field: Optional[str] = None) -> 'FieldQuery':
