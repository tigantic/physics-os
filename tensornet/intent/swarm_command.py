...
from tensornet.intent.parser import IntentParser, ParseResult, IntentType, Entity
from tensornet.intent.engine import IntentEngine, IntentResult, ExecutionContext
...
class SwarmCommand:
    ...
    mach: Optional[float] = None     # target Mach number
    altitude: Optional[float] = None # target altitude (meters)
    position: Optional[Tuple[float, float, float]] = None  # lat, lon, alt
    
    # Formation
    formation: Optional[str] = None  # "wedge", "line", "echelon"
    spacing: Optional[float] = None  # meters
...
class SwarmCommandParser:
    ...
    # Altitude patterns
    ALTITUDE_PATTERNS = [
        (r"angels?\s*(\d+)", lambda m: float(m.group(1)) * 1000 * 0.3048),  # Angels = 1000ft
        (r"altitude\s*(\d+)\s*(?:m|meters?)", lambda m: float(m.group(1))),
        (r"altitude\s*(\d+)\s*(?:ft|feet)", lambda m: float(m.group(1)) * 0.3048),
        (r"(\d+)\s*(?:km|kilometers?)\s*(?:altitude|alt)?", lambda m: float(m.group(1)) * 1000),
    ]
...
class SwarmCommander:
    ...
    def _execute_status(self, targets: List[str]) -> Dict[str, Any]:
        ...
        return {
            "status": "success",
            "command": {"command": "status", "target_swarm": targets[0] if len(targets) == 1 else "all"},
