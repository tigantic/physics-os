...
class DomainRandomizer:
    ...
    def sample_subset(self, names: List[str]) -> Dict[str, float]:
        ...
        return {name: self.ranges[name].sample(self._rng) 
                for name in names if name in self.ranges}
...
class AdaptiveRandomizer(DomainRandomizer):
    ...
    def _success_rate_near(
        ...
        near_episodes = [
            success for v, success in history
...
def make_fluid_curriculum(difficulty: str = 'standard') -> Curriculum:
    ...
    if difficulty == 'easy':
        stages = [
            CurriculumStage(
                name="beginner",
                viscosity=0.1, dt=0.02, max_steps=50,
                success_threshold=0.7, min_episodes=50,
            ),
            CurriculumStage(
                name="intermediate",
                viscosity=0.05, dt=0.01, max_steps=100,
                success_threshold=0.7, min_episodes=100,
            ),
        ]
        ...
        stages = [
            CurriculumStage(
                name="warmup",
                viscosity=0.1, dt=0.02, max_steps=50,
                success_threshold=0.8, min_episodes=100,
            ),
            CurriculumStage(
                name="beginner",
                viscosity=0.05, dt=0.01, max_steps=100,
                noise_scale=0.01,
                success_threshold=0.8, min_episodes=200,
            ),
            CurriculumStage(
                name="intermediate",
                viscosity=0.02, dt=0.01, max_steps=150,
                noise_scale=0.02, perturbation_freq=0.1,
                success_threshold=0.75, min_episodes=300,
            ),
            CurriculumStage(
                name="advanced",
                viscosity=0.01, dt=0.005, max_steps=200,
                noise_scale=0.05, perturbation_freq=0.2,
                obstacle_count=2,
                success_threshold=0.7, min_episodes=500,
            ),
            CurriculumStage(
                name="expert",
                viscosity=0.005, dt=0.005, max_steps=300,
                noise_scale=0.1, perturbation_freq=0.3,
                obstacle_count=4,
                viscosity_range=(0.001, 0.01),
                dt_range=(0.002, 0.01),
                success_threshold=0.6, min_episodes=1000,
            ),
        ]
    else:  # standard
        stages = [
            CurriculumStage(
                name="beginner",
                viscosity=0.1, dt=0.02, max_steps=50,
                success_threshold=0.75, min_episodes=100,
            ),
            CurriculumStage(
                name="intermediate",
                viscosity=0.05, dt=0.01, max_steps=100,
                noise_scale=0.01,
                success_threshold=0.75, min_episodes=200,
            ),
            CurriculumStage(
                name="advanced",
                viscosity=0.01, dt=0.01, max_steps=200,
                noise_scale=0.02,
