...
class DownsampleObservation(ObservationWrapper):
    ...
    def _average_pool(self, obs: np.ndarray) -> np.ndarray:
        ...
        # Reshape for pooling
        obs_reshaped = obs.reshape(c, new_h, self.factor, new_w, self.factor)
        ...
        episodes_data = [
            {
                ...
                "actions": [
                    act.tolist() if hasattr(act, "tolist") else act
    ...
    def load(self, path: str):
        ...
        episodes = [
            EpisodeRecord(
                ...
                actions=[
                    np.array(act) if isinstance(act, list) else act
                    for act in ep["actions"]
                ],
                rewards=ep["rewards"],
                total_reward=ep["total_reward"],
...
class LogMetrics(Wrapper):
    ...
    def __init__(
        ...
        logger: Any | None = None,  # tensorboard, wandb, etc.
    ):
        super().__init__(env)
