...
class BenchmarkResult:
    ...
    max_time: float
    n_runs: int
    raw_timings: list[float] | None = None
...
class ScalabilityTest(ABC):
    ...
    def analyze_scaling(self) -> dict:
        ...
        return {
            ...
            "times": times,
            "efficiency": [
                times[0] / (t * scales[i] / scales[0]) for i, t in enumerate(times)
            ],
        }
    ...
    def run_all(self, verbose: bool = True) -> dict[str, BenchmarkResult]:
        ...
        for name, (fn, metadata) in self.benchmarks.items():
            ...
            try:
                ...
                if verbose:
                    print(
                        f"  {result.mean_time*1000:.2f} ms ± {result.std_time*1000:.2f} ms"
                    )
...
def compare_benchmarks(
    ...
    for name in set(baseline.keys()) & set(current.keys()):
        ...
        speedup = (
            base.mean_time / curr.mean_time if curr.mean_time > 0 else float("inf")
        )
    ...
    for name in set(current.keys()) - set(baseline.keys()):
        comparison[name] = {
            "status": "added",
            "current_ms": current[name].mean_time * 1000,
        }
