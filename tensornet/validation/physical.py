    ...
    def validate(
        ...
        **kwargs,
        ...
        if passed:
            ...
            message = f"Conservation maintained within tolerance"
        elif error_value < 10 * self.tolerance:
            severity = ValidationSeverity.WARNING
            message = f"Conservation marginally violated"
        else:
            severity = ValidationSeverity.FAIL
            message = f"Conservation law violated"
...
class AnalyticalValidator(ABC):
    ...
    def validate(
        ...
        **kwargs,
        ...
        if passed:
            severity = ValidationSeverity.PASS
            message = f"Error within tolerance"
        elif error < 10 * self.tolerance:
            severity = ValidationSeverity.WARNING
            message = f"Error marginally exceeds tolerance"
        else:
            severity = ValidationSeverity.FAIL
            message = f"Error significantly exceeds tolerance"
...
class SodShockValidator(AnalyticalValidator):
            ...
            if xi_val < head_L:
                ...
                if p_star <= self.p_L:
                    ...
                    if xi_val < tail_L:
                        # Inside rarefaction
                        rho[i] = self.rho_L * (
                            (2 / (gamma + 1) + (gamma - 1) / ((gamma + 1) * a_L) * (self.u_L - xi_val))
                            ** (2 / (gamma - 1))
                        )
                        u[i] = 2 / (gamma + 1) * (a_L + (gamma - 1) / 2 * self.u_L + xi_val)
                        p[i] = self.p_L * (
                            (2 / (gamma + 1) + (gamma - 1) / ((gamma + 1) * a_L) * (self.u_L - xi_val))
                            ** (2 * gamma / (gamma - 1))
                        )
    ...
    def _solve_for_pstar(self) -> float:
        ...
        for _ in range(100):
            ...
            df = self._df_pressure(p, self.rho_L, self.p_L, gamma) + \
                 self._df_pressure(p, self.rho_R, self.p_R, gamma)
...
def run_physical_validation(
    validation_tests: List[Union[ConservationValidator, AnalyticalValidator]],
    **kwargs,
) -> ValidationReport:
    """
