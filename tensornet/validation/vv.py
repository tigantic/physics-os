...
class VVPlan:
    ...
    def run(
        self,
        categories: list[VVCategory] | None = None,
        levels: list[VVLevel] | None = None,
        verbose: bool = True,
    ) -> list[VVTestResult]:
        """
        ...
        return {
            ...
            "by_category": {
                cat.name: {
                    ...
                    "passed": sum(
                        1 for r in self.results if r.category == cat and r.passed
                    ),
                }
...
class VVReport:
    ...
    def to_markdown(self) -> str:
        ...
        for cat_name, stats in summary["by_category"].items():
            if stats["total"] > 0:
                ...
                lines.append(
                    f"| {cat_name} | {stats['passed']} | {stats['total']} | {rate:.0f}% |"
                )
        ...
        for result in self.plan.results:
            ...
            if result.criteria_results:
                ...
                for name, info in result.criteria_results.items():
                    ...
                    lines.append(
                        f"- {c_status} {name}: {info['value']:.6e} (threshold: {info['threshold']:.6e})"
                    )
                lines.append("")
    ...
    def save(self, filepath: str | Path, format: str = "markdown"):
        """
        Save report to file.
...
class ValidationCase:
    ...
    name: str
    description: str
    experimental_data: Any | None = None
...
class ExperimentalValidation:
    ...
    def validate(self) -> dict[str, Any]:
        ...
        for metric in self.metrics:
            if hasattr(sim_result, metric) and hasattr(
                self.case.experimental_data, metric
            ):
                sim_val = getattr(sim_result, metric)
                exp_val = getattr(self.case.experimental_data, metric)

                if isinstance(sim_val, (int, float)) and isinstance(
                    exp_val, (int, float)
                ):
...
class ValidationUncertainty:
    ...
    def __init__(
        ...
        numerical_uncertainty: UncertaintyBand | None = None,
    ):
        """
