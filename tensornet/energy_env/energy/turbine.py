...
class WindFarm:
    ...
    def calculate_capacity_factor(
        self, actual_power: float, wind_speed: float = 12.0
    ) -> float:
        """
    ...
    def get_wake_centerline(
        self, turbine_idx: int, max_distance: float = 1000.0, resolution: float = 10.0
    ) -> list[dict]:
        """
        ...
        while distance <= max_distance:
            ...
            points.append(
                {"x": t["x"], "y": t["y"], "z": t["z"] + distance, "radius": r_wake}
            )
            distance += resolution
    ...
    def annual_energy_production(
        self, power_mw: float, capacity_factor: float = 0.45
    ) -> float:
        """
...
class WakeOptimizer:
    ...
    def __init__(
        self, farm: WindFarm, bounds: dict[str, tuple], grid_resolution: float = 10.0
    ):
        """
        Args:
    ...
    def optimize(
        ...
        for i in range(iterations):
            ...
            if verbose and i % 20 == 0:
                print(
                    f"  Iteration {i}: Power = {power:.3f} MW (best: {best_power:.3f})"
                )
