...
class GrayscaleBridgeWriter:
    ...
    def write_frame(
        ...
        if intensity.shape != (self.height, self.width):
            raise ValueError(f"Expected {(self.height, self.width)}, got {intensity.shape}")
...
class CUDAHeatmapGeneratorV2:
    ...
    def __init__(
        ...
        print(f"✓ CUDA Heatmap Generator v2 (grayscale)")
    ...
    def generate_convergence(self, time: float) -> None:
        ...
        self.intensity += torch.exp(-jet_dist * 5.0) * 0.4 * (
    ...
    def run_loop(self, target_fps: float = 165.0, duration: Optional[float] = None) -> None:
        ...
        print(f"   Bandwidth: {self.width * self.height * target_fps / 1024 / 1024:.1f} MB/s")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CUDA Heatmap Generator v2 (Grayscale)")
