...
class QTTBridgeStreamer:
    ...
    def __init__(
        ...
        print(f"✓ QTT Bridge Streamer initialized")
    ...
    def _init_qtt_cores(self) -> None:
        ...
        self._work_matrices = torch.zeros(12, 2, 4, device=self.device, dtype=torch.float32)
    ...
    def _update_qtt_cores_gpu(self, time: float) -> None:
        ...
        self._work_matrices[:, :, 2] = 0.1 * self._work_t * (1 - self.matrix_idx_grid)  # c
    ...
    def run_loop(self, target_fps: float = 500.0, duration: Optional[float] = None) -> None:
...
def main():
    ...
    parser.add_argument("--colormap", type=str, default="plasma", choices=["plasma", "viridis"])
