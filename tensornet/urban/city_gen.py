...
class VoxelCity:
    ...
    def generate_manhattan(
        ...
        print(f"[CITY] Max height: {self.stats.max_height} voxels "
    ...
    def generate_grid(
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                building = BuildingSpec(
                    x=x, z=z, width=bw, depth=bd, height=h,
    ...
    def add_building(self, spec: BuildingSpec) -> bool:
        ...
        """
        # Bounds check
        if (spec.x < 0 or spec.x + spec.width > self.width or
            spec.z < 0 or spec.z + spec.depth > self.depth or
...
def demo_city_generation():
    ...
    print(f"   Domain: {city1.physical_size[0]:.0f}m x "
