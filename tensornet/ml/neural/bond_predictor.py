...
class EntropyFeatures:
    ...
    def from_entropies(
        ...
        boundary_entropy = (
            float((entropies[0] + entropies[-1]) / 2)
            if len(entropies) > 1
            else float(entropies[0])
        )
...
class TemporalFeatures:
    ...
    def from_histories(
        ...
        return cls(
            ...
            trend_error=compute_trend(
                [math.log10(max(e, 1e-16)) for e in error_history]
            ),
        )
    ...
    def to_tensor(self, max_history: int = 20) -> torch.Tensor:
        ...
        while len(entropy_recent) < max_history:
            entropy_recent = [
                entropy_recent[0] if entropy_recent else 0.0
            ] + entropy_recent
        ...
        features.extend(
            [-math.log10(max(e, 1e-16)) / 16.0 for e in error_recent]
        )  # Log error
...
class PredictorConfig:
    ...
    input_dim: int = (
        71  # 5 (entropy) + 3*20 (temporal histories) + 3 (trends) + 3 (additional)
    )
    hidden_dim: int = 256
    num_layers: int = 3
    dropout: float = 0.1
...
class BondDimensionPredictor:
    ...
    def load(
        ...
        predictor = cls(
            ...
            chi_min=checkpoint.get("chi_min", 4),
...
def predict_optimal_chi(
    ...
    target_error: float,
    current_chi: int,
    predictor: BondDimensionPredictor | None = None,
) -> int:
    ...
    if predictor is None:
        ...
        max_entropy = (
            float(torch.max(site_entropies)) if site_entropies.numel() > 0 else 1.0
        )
