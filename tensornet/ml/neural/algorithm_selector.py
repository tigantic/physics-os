...
def _generate_reasoning(
    ...
    if problem.is_ground_state and algorithm == AlgorithmType.DMRG:
        parts.append(
            "DMRG is optimal for ground state problems with short-range interactions."
        )
    elif problem.is_dynamics and algorithm == AlgorithmType.TEBD:
        parts.append(
            "TEBD is efficient for real-time dynamics with nearest-neighbor terms."
        )
    elif problem.is_dynamics and algorithm in (
        AlgorithmType.TDVP_1,
        AlgorithmType.TDVP_2,
    ):
...
class AlgorithmSelector:
    ...
    def select(
        ...
        weights = torch.tensor(criteria.get_weights(), dtype=torch.float32).to(
            self.device
        )

        with torch.no_grad():
            scores = self.network(features.unsqueeze(0), weights.unsqueeze(0)).squeeze(
                0
            )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            history["loss"].append(
                epoch_loss / max(1, len(self.training_data) // batch_size)
            )
...
def benchmark_algorithms(
    ...
    run_fn: Callable[[AlgorithmType, ProblemFeatures], BenchmarkResult] | None = None,
