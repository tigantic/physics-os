...
class PolicyState:
    ...
    def to_tensor(self) -> torch.Tensor:
        ...
        chi_normalized = (self.current_chi - self.chi_min) / max(
            1, self.chi_max - self.chi_min
        )
...
class RLTruncationAgent:
    ...
    def compute_reward(
        ...
        if truncation_error <= target_error:
            accuracy_reward = 1.0 + math.log10(
                target_error / max(truncation_error, 1e-16)
            )
        else:
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            new_truncation_error = (
                truncation_error * (state.current_chi / max(new_chi, 1)) ** 2
            )
