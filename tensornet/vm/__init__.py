...
inside compilers loaded via ``tensornet.vm.compilers.*`` resolve to the
same canonical ``tensornet.engine.vm.*`` module objects, preventing
...
_SHIM_PREFIX = "tensornet.vm"
_REAL_PREFIX = "tensornet.engine.vm"
...
# ``tensornet.vm.compilers.navier_stokes``) see the *same* module object.
...
class _ShimFinder:
    """Redirect ``tensornet.vm.*`` imports to ``tensornet.engine.vm.*``."""
