...
class CascadeReport:
    ...
    def __str__(self) -> str:
        cascade_status = (
            "⛔ CASCADE FAILURE" if self.cascade_occurred else "✅ CONTAINED"
        )
        return (
            f"[CASCADE REPORT]\n"
...
class CyberGrid:
    ...
    def __init__(
        ...
        self.edge_tensor = torch.tensor(
            [[e[0], e[1]] for e in self.edges], device=self.device, dtype=torch.long
        )
        ...
        np.random.seed(42)
        self.capacity = (
            torch.ones(num_nodes, device=self.device, dtype=torch.float64)
            * base_capacity
        )
        self.capacity += torch.randn(
            num_nodes, device=self.device, dtype=torch.float64
        ) * (base_capacity * 0.2)
    ...
    def simulate_attack(
        ...
        for t in range(steps):
            ...
            if verbose and t % 10 == 0:
                ...
                print(
                    f"   T={t:3d}: Max Load {max_p:6.0f}% at Node {victim:2d} | "
        ...
        if cascade_occurred:
            recommendation = (
                "CRITICAL: Implement rate limiting at edge nodes. "
                "Add redundant paths. Consider BGP blackholing."
            )
        elif failed_count > 0:
            recommendation = (
                f"PARTIAL DAMAGE: {failed_count} nodes failed. "
                "Increase capacity or add load balancers."
            )
        else:
            recommendation = (
                "ATTACK MITIGATED: Network absorbed the attack. " "Continue monitoring."
            )
