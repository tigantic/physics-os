...
def calculate_equilibrium_wall_temperature(
    ...
    # T_w = (q / (ε * σ))^0.25
    temp_wall = torch.pow(heat_flux / (emissivity * STEFAN_BOLTZMANN), 0.25)
...
def demo_hazard_field():
    ...
    print(
        f"  Dynamic Pressure Range: {hazard.dynamic_pressure.min():.0f} - {hazard.dynamic_pressure.max():.0f} Pa"
    )
    print(
        f"  Wall Temperature Range: {hazard.stagnation_temp.min():.0f} - {hazard.stagnation_temp.max():.0f} K"
    )
    print(
        f"  Wind Shear Range: {hazard.wind_shear.min():.4f} - {hazard.wind_shear.max():.4f} 1/s"
    )
