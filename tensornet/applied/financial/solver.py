...
Usage:
    >>> from tensornet.applied.financial.solver import LiquiditySolver
...
class LiquiditySolver:
    ...
    def compute_flow(
        self, density_field: torch.Tensor, current_price_idx: int
    ) -> FlowSignal:
        """
        ...
        confidence = min(
            1.0, (imbalance * 0.4 + permeability * 0.3 + force_magnitude * 0.3)
        )
    ...
    def _find_resistance(
        self, density: torch.Tensor, current_idx: int
    ) -> tuple[float, int]:
        """
    ...
    def detect_breakout(
        ...
        # Breakout score
        score = (
            asymmetry * 0.4 + momentum * 10.0 * 0.3 + (1.0 if thin_wall else 0.0) * 0.3
        )
    ...
    def step_simulation(
        self, density: torch.Tensor, velocity: torch.Tensor, dt: float = 0.01
    ) -> torch.Tensor:
        """
...
def solve_price_flow(
    density_field: torch.Tensor, current_price_idx: int, lookahead: int = 50
) -> float:
    """
...
def run_solver_demo():
    ...
    print(
        f"   Resistance: {signal.resistance_strength:.2f} (dist: {signal.nearest_resistance})"
    )
    print()
