...
class AerodynamicData:
    ...
    # Surface data
    cp_distribution: Optional[np.ndarray] = None
    heat_flux_distribution: Optional[np.ndarray] = None
...
class FlightRecord:
    ...
    def get_condition_at_time(self, t: float) -> Optional[FlightCondition]:
        ...
        for i in range(len(self.conditions) - 1):
            ...
            if c1.timestamp <= t <= c2.timestamp:
                ...
                return FlightCondition(
                    ...
                    mach_number=c1.mach_number + alpha * (c2.mach_number - c1.mach_number),
                    velocity_m_s=c1.velocity_m_s + alpha * (c2.velocity_m_s - c1.velocity_m_s),
                    pressure_pa=c1.pressure_pa + alpha * (c2.pressure_pa - c1.pressure_pa),
                    temperature_k=c1.temperature_k + alpha * (c2.temperature_k - c1.temperature_k),
                    density_kg_m3=c1.density_kg_m3 + alpha * (c2.density_kg_m3 - c1.density_kg_m3),
                    angle_of_attack_deg=c1.angle_of_attack_deg + alpha * (c2.angle_of_attack_deg - c1.angle_of_attack_deg),
                    sideslip_angle_deg=c1.sideslip_angle_deg + alpha * (c2.sideslip_angle_deg - c1.sideslip_angle_deg),
                )
        
        return None
    
    def get_aero_at_time(self, t: float) -> Optional[AerodynamicData]:
...
class FlightDataLoader:
    ...
    def load(
        self,
        path: Union[str, Path],
        format: Optional[FlightDataFormat] = None,
