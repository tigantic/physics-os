...
class MeasurementUncertainty:
    ...
    def _recalculate(self):
        ...
        if abs(self.measured_value) > 1e-10:
            self.percent_uncertainty = 100 * self.expanded_uncertainty / abs(self.measured_value)
...
class ValidationUncertainty:
    ...
    measurement_uncertainty: Optional[MeasurementUncertainty] = None
    ...
    def _recalculate(self):
        ...
        if self.combined_uncertainty > 1e-10:
            self.validation_metric = abs(self.comparison_error) / self.combined_uncertainty
...
class UncertaintyBudget:
    ...
    # Measurement uncertainties
    measurement_uncertainties: Dict[str, MeasurementUncertainty] = field(default_factory=dict)
    
    # Model uncertainties
    model_uncertainties: Dict[str, ModelUncertainty] = field(default_factory=dict)
    
    # Validation uncertainties
    validation_uncertainties: Dict[str, ValidationUncertainty] = field(default_factory=dict)
...
class GridConvergenceIndex:
    ...
    def calculate_gci(
        ...
        p = self.target_order
        for _ in range(20):
            p_new = abs(math.log(abs(epsilon_21 / epsilon_32)) + 
                       math.log((r**p - s) / (r**p - s))) / math.log(r)
            if abs(p_new - p) < 1e-6:
                break
            p = 0.5 * (p + p_new)
