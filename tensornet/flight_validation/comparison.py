...
from .data_loader import FlightRecord
...
class ComparisonResult:
    ...
    def _update_overall_status(self):
        ...
        self.validation_passed = worst_idx <= status_order.index(
            ComparisonStatus.MARGINAL
        )
...
class FlightDataValidator:
            ...
            if field_name in self.tolerances:
                ...
                if comparison.mean_percent_error > tolerance:
                    comparison.status = max(
                        comparison.status, ComparisonStatus.MARGINAL
                    )
    ...
    def _extract_flight_arrays(
        ...
        if record.conditions:
            ...
            data["condition_timestamp"] = np.array(
                [c.timestamp for c in record.conditions]
            )
    ...
    def validate_temporal(
        ...
        for field_name, (sim_times, sim_values) in simulation_timeseries.items():
            ...
            if field_name in ["cl", "cd", "cy", "cm"]:
                flight_times = flight_data.get(
                    "timestamp", np.arange(len(flight_values))
                )
            else:
                flight_times = flight_data.get(
                    "condition_timestamp", np.arange(len(flight_values))
                )
