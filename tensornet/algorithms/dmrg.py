...
def _apply_heff_two_site(
    ...
    # L @ theta -> temp1
    temp1 = torch.einsum('abc,ade->bcde', L, theta.reshape(theta.shape[0], -1, theta.shape[-1]))
    ...
    t1 = torch.einsum('awa,asb->wsab', L.reshape(L.shape[0], -1, L.shape[2]), 
    ...
    if direction == 'right':
        ...
        for i in range(L - 1):
            ...
            energy, theta_opt = _two_site_eigensolve(
                L_envs[i], H.tensors[i], H.tensors[i+1], R_envs[i+2],
                theta
            )
        ...
        for i in range(L - 2, -1, -1):
            ...
            R_envs[i+1] = _contract_right_env(R_envs[i+2], psi.tensors[i+1], H.tensors[i+1])
...
def dmrg(
    ...
    # Initialize MPS
    if psi0 is None:
        psi = MPS.random(L, d, min(chi_max, 4), dtype=H.tensors[0].dtype, 
                         device=H.tensors[0].device, normalize=True)
    else:
        psi = psi0
    ...
    for sweep in range(num_sweeps):
        # Right sweep
        E1, S1, err1 = dmrg_sweep(psi, H, chi_max, L_envs, R_envs, 
                                   direction='right', svd_cutoff=svd_cutoff)
        
        # Left sweep
        E2, S2, err2 = dmrg_sweep(psi, H, chi_max, L_envs, R_envs,
                                   direction='left', svd_cutoff=svd_cutoff)
