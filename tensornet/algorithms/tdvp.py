...
def tdvp_step(
    ...
    for i in sites:
        ...
        psi = _two_site_tdvp_update(
            psi, H, left_envs, right_envs, i, dt / 2,
            chi_max, lanczos_dim, tol, direction
        )
...
def _two_site_tdvp_update(
    ...
    theta_evolved = lanczos_expm(
        H_eff, theta_flat, -1j * dt, 
...
def tdvp(
    ...
    lanczos_dim: int = 20,
    tol: float = 1e-12,
    callback: Optional[Callable[[MPS, float], None]] = None,
    verbose: bool = False
) -> TDVPResult:
    """
    ...
    for step in range(num_steps):
        ...
        if verbose and (step + 1) % max(1, num_steps // 10) == 0:
            ...
            print(f"  t={t:.3f}: E = {E:.10f}, ΔE/E = {dE:.2e}, χ_max = {chi_max_current}")
    ...
    return TDVPResult(psi=psi, times=times, energies=energies, entropies=entropies, info=info)
...
def imaginary_time_tdvp(
    ...
    if verbose:
        print(f"Imaginary Time TDVP: seeking ground state")
