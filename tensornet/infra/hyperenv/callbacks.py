...
class Callback(ABC):
    ...
    def on_step(self, trainer: Trainer) -> bool | None:
        """
...
class CheckpointCallback(Callback):
    ...
    def _save(self, trainer: Trainer, prefix: str | None = None):
        """Save checkpoint."""
        prefix = prefix or self.name_prefix
        path = os.path.join(self.save_path, f"{prefix}_{trainer.state.timestep}")
...
class LoggingCallback(Callback):
    ...
    def _log(self, trainer: Trainer):
        ...
        if self.verbose >= 1:
            print(
                f"Step: {trainer.state.timestep:>8} | "
        ...
        if self._writer:
            self._writer.add_scalar(
                "train/mean_reward", mean_reward, trainer.state.timestep
            )
            self._writer.add_scalar(
                "train/episode", trainer.state.episode, trainer.state.timestep
            )
...
class EarlyStoppingCallback(Callback):
    ...
    def _check(self, trainer: Trainer) -> bool | None:
...
class LambdaCallback(Callback):
    ...
    def __init__(
        self,
        on_training_start: Callable[[Trainer], None] | None = None,
        on_training_end: Callable[[Trainer], None] | None = None,
        on_step: Callable[[Trainer], bool | None] | None = None,
        on_episode_end: Callable[[Trainer, float, int], None] | None = None,
        on_evaluation: Callable[[Trainer, float, float], None] | None = None,
    ):
