...
class Trainer:
    ...
    def __init__(
        ...
        buffer: ReplayBuffer | RolloutBuffer | None = None,
    ):
        self.agent = agent
        self.env = env
    ...
    def _checkpoint(self, callbacks: list[Callback]):
        ...
        if self.config.save_path:
            path = os.path.join(
                self.config.save_path, f"checkpoint_{self.state.timestep}"
            )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mean_episode_reward": (
                np.mean(self.state.episode_rewards[-100:])
                if self.state.episode_rewards
                else 0
            ),
            "mean_episode_length": (
                np.mean(self.state.episode_lengths[-100:])
