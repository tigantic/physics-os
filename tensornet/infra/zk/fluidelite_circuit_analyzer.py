...
High-performance ZK circuit vulnerability analysis using the full tensornet
...
Usage:
    from tensornet.infra.zk.fluidelite_circuit_analyzer import FEZKAnalyzer
...
# FEZK v2.0: Additional tensornet imports
try:
    from tensornet.cfd.pure_qtt_ops import QTTState, MPO, apply_mpo, truncate_qtt, qtt_add
    HAS_MPO = True
except ImportError:
    HAS_MPO = False

try:
    from tensornet.cfd.qtt_tci import qtt_from_function_tci_python, RUST_AVAILABLE as TCI_RUST
    HAS_TCI = True
except ImportError:
    HAS_TCI = False
    TCI_RUST = False
...
try:
    from tensornet.cfd.qtt_tci_gpu import qtt_from_function_gpu, qtt_eval_gpu
...
class QTTConstraintMatrix:
    ...
    def from_r1cs(self, r1cs: R1CS) -> 'QTTConstraintMatrix':
        ...
        if not HAS_TORCH or not HAS_QTT:
            raise RuntimeError("QTT compression requires PyTorch and tensornet.cfd.qtt")
...
class MPOConstraintOps:
    ...
    def witness_to_qtt(
        ...
        FEZK v2.0: Uses tensornet QTT infrastructure.
        """
        if not HAS_TORCH or not HAS_MPO:
            return None
...
class IntervalPropagator:
    ...
    interval arithmetic from tensornet.numerics.interval.
    ...
    def _propagate_rigorous(
        ...
        """
        Rigorous interval propagation using tensornet Interval class.
...
class FEZKAnalyzer:
    ...
    def analyze(self, path: Path) -> List[Finding]:
        ...
        if path.suffix == '.circom':
            ...
            try:
                from tensornet.infra.zk.halo2_analyzer import Halo2Analyzer
