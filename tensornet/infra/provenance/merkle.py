...
def compute_hash(data: bytes | str | np.ndarray | torch.Tensor | dict) -> str:
    """
...
class MerkleDAG:
    ...
    def __contains__(self, hash_or_node: str | MerkleNode) -> bool:
    ...
    def get(self, hash: str) -> MerkleNode | None:
        """Get node by hash."""
    ...
    def common_ancestor(self, hash1: str, hash2: str) -> MerkleNode | None:
        """
    ...
    def path(self, from_hash: str, to_hash: str) -> list[MerkleNode] | None:
        """
...
def generate_proof(
    dag: MerkleDAG, target_hash: str, root_hash: str
) -> MerkleProof | None:
    """
    ...
    siblings = []
    for node in path:
        sibling_hashes = [h for h in node.parent_hashes if h not in path_hashes]
