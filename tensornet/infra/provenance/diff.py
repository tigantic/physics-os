...
class FieldDiff:
    ...
    change_mask: np.ndarray | None = None  # Boolean mask of changed elements

    # Hotspots (regions of largest change)
    hotspots: list[tuple[tuple[int, ...], float]] = field(default_factory=list)

    # Metadata
    commit1_hash: str | None = None
    commit2_hash: str | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "hotspots": [
                {"index": list(idx), "value": val} for idx, val in self.hotspots
            ],
...
class DiffEngine:
    ...
    def compute(
        self,
        field1: np.ndarray | torch.Tensor,
        field2: np.ndarray | torch.Tensor,
        commit1_hash: str | None = None,
        commit2_hash: str | None = None,
    ) -> FieldDiff:
        """
