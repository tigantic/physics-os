...
class ObservationConfig:
    ...
    probe_locations: np.ndarray | None = None
...
class FieldObservation:
    ...
    def shape(self) -> tuple[int, ...]:
        ...
        if self.config.obs_type == ObservationType.SLICE_2D:
            ...
            n_probes = (
                len(self.config.probe_locations)
                if self.config.probe_locations is not None
                else 16
            )
    ...
    def extract(self, field: Field) -> np.ndarray:
        ...
        if self.config.obs_type == ObservationType.SLICE_2D:
            ...
            slices = [
                self._extract_slice(field, pos) for pos in self.config.slice_positions
            ]
...
class FieldAction:
    ...
    @property
    def n_discrete(self) -> int | list[int]:
        """Number of discrete actions."""
        if self.config.action_type == ActionType.DISCRETE:
            return np.prod(
                [a.discrete_bins**a.force_dims for a in self.config.actuators]
            )
        elif self.config.action_type == ActionType.MULTI_DISCRETE:
            return [a.discrete_bins**a.force_dims for a in self.config.actuators]
        return 1
...
class ActionMask:
    ...
    def compute(self, field: Field) -> np.ndarray:
        ...
        if self.action_config.action_type == ActionType.CONTINUOUS:
            ...
            mask = np.ones(
                self.action_config.actuators[0].force_dims
                * len(self.action_config.actuators),
                dtype=np.float32,
            )
        else:
