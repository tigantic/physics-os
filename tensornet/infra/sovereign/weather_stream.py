...
Usage:
    from tensornet.infra.sovereign.weather_stream import WeatherStreamWriter
...
from .protocol import PROTOCOL_MAGIC, PROTOCOL_VERSION, SHM_PATH, SHM_SIZE, WeatherHeader
...
class WeatherStreamWriter:
    ...
    def open(self):
        ...
        # Memory map
        self._mmap = mmap.mmap(self._file.fileno(), self.size, access=mmap.ACCESS_WRITE)
    ...
    def write_frame(
        ...
        if u_tensor.shape != v_tensor.shape:
            raise ValueError(
                f"Tensor shape mismatch: {u_tensor.shape} vs {v_tensor.shape}"
            )
        ...
        print(
            f"[BRIDGE] Frame {self.frame_number - 1}: {w}x{h}, max_speed={max_speed:.1f} m/s"
        )
...
_global_writer: WeatherStreamWriter | None = None
...
def write_to_bridge(u_tensor: np.ndarray, v_tensor: np.ndarray, timestamp: int):
    """
