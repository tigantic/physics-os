...
class TerrainMap:
    ...
    def get_elevation(self, lat: float, lon: float) -> float:
        ...
        z = (
            (1 - di) * (1 - dj) * self.elevation[i0, j0]
...
def compute_terrain_gradient(
    terrain: TerrainMap, lat: float, lon: float, radius: float = 1000.0
) -> tuple[float, float]:
    """
...
def compute_aero_signature(
    ...
    dz_dx[1:-1, :] = (elevations[2:, :] - elevations[:-2, :]) / (
        2 * window_size / n_samples
    )
    dz_dy[:, 1:-1] = (elevations[:, 2:] - elevations[:, :-2]) / (
        2 * window_size / n_samples
    )
...
def cross_correlate_signatures(
    measured: AeroSignature, candidate: AeroSignature
) -> float:
    """
...
class AeroTRN:
    ...
    def __init__(self, terrain_map: TerrainMap, config: AeroTRNConfig | None = None):
        """
        ...
        self.current_state: NavigationState | None = None
    ...
    def update(
        ...
        best_match = self._search_correlation(
            measured_signature, predicted_lat, predicted_lon, predicted_alt, velocity
        )
