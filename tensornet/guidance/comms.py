...
class BlackoutMap:
    ...
    def ranked_antennas(self) -> List[Tuple[str, float]]:
        ...
        return [
            (self.antenna_ids[int(i)], self.attenuation_dB[i].item())
            for i in indices
        ]
...
def compute_blackout_map(
    ...
    """
    from tensornet.cfd.plasma import plasma_frequency, rf_attenuation, is_blackout
    ...
    # Compute attenuation
    atten = rf_attenuation(
        torch.tensor(omega_signal),
        omega_pe
    )
...
class CognitiveComms:
    ...
    def frequency_hop(
        ...
        # Compute margin
        margin_dB = 20 * math.log10(f_target / max(f_critical, 1e6)) if above_cutoff else -100
    ...
    def recommend_maneuver(
        self,
        blackout_map: BlackoutMap,
        target_antenna_id: Optional[str] = None
        ...
        pitch_deg = math.degrees(math.atan2(-normal[0], math.sqrt(normal[1]**2 + normal[2]**2)))
...
def estimate_blackout_duration(
    trajectory_time: Tensor,
    blackout_flags: Tensor
