...
class VehicleState:
    ...
    def to_tensor(self) -> torch.Tensor:
        ...
        return torch.tensor([
            self.latitude_rad, self.longitude_rad, self.altitude_m,
            self.u_m_s, self.v_m_s, self.w_m_s,
            self.q0, self.q1, self.q2, self.q3,
            self.p_rad_s, self.q_rad_s, self.r_rad_s,
            self.mass_kg
        ])
...
def isa_atmosphere(altitude_m: float) -> AtmosphericModel:
    ...
    if h <= TROPOPAUSE_ALT_M:
        ...
        p = SEA_LEVEL_PRESSURE_PA * (T / SEA_LEVEL_TEMP_K) ** (9.80665 / (LAPSE_RATE_K_M * GAS_CONSTANT_J_KG_K))
    elif h <= 20000:
        # Lower stratosphere (isothermal)
        T = 216.65  # K
        p_trop = 22632.0  # Pa at tropopause
        p = p_trop * math.exp(-9.80665 * (h - TROPOPAUSE_ALT_M) / (GAS_CONSTANT_J_KG_K * T))
    elif h <= 32000:
...
class TrajectorySolver:
    ...
    def __init__(
        ...
        self.cfd_lookup: Optional[Callable] = None
    
    def set_cfd_lookup(self, lookup_fn: Callable[[float, float, float], AeroCoefficients]):
        """
    ...
    def state_derivative(
        ...
        p_dot = (Izz * L + Ixz * N - (Izz * (Izz - Iyy) + Ixz**2) * q * r +
                 Ixz * (Ixx - Iyy + Izz) * p * q) / Gamma
        q_dot = (M - (Ixx - Izz) * p * r - Ixz * (p**2 - r**2)) / Iyy
        r_dot = (Ixz * L + Ixx * N + (Ixx * (Ixx - Iyy) + Ixz**2) * p * q -
        ...
        V_N = (ct*cpsi)*u + (sp*st*cpsi - cp*spsi)*v + (cp*st*cpsi + sp*spsi)*w
        V_E = (ct*spsi)*u + (sp*st*spsi + cp*cpsi)*v + (cp*st*spsi - sp*cpsi)*w
    ...
    def step_rk4(
        ...
        q_norm = math.sqrt(y_new[6]**2 + y_new[7]**2 + y_new[8]**2 + y_new[9]**2)
        if q_norm > 0:
            y_new[6:10] /= q_norm
    ...
    def propagate(
        ...
        duration_s: float,
        controls_fn: Optional[Callable[[float, VehicleState], Dict[str, float]]] = None
    ) -> List[VehicleState]:
        """
...
def validate_trajectory_module():
    ...
    print(f"10 km:     T={atm_10k.temperature_K:.1f}K, ρ={atm_10k.density_kg_m3:.4f} kg/m³")
    print(f"30 km:     T={atm_30k.temperature_K:.1f}K, ρ={atm_30k.density_kg_m3:.6f} kg/m³")
    ...
    print(f"After:  alt={new_state.altitude_m:.1f}m, V={new_state.velocity_magnitude:.1f}m/s")
    ...
    traj = create_reentry_trajectory(
        entry_altitude_m=60000,
        entry_velocity_m_s=5000,
        duration_s=10.0
    )
    
    print(f"Entry: alt={traj[0].altitude_m/1000:.1f}km, V={traj[0].velocity_magnitude:.0f}m/s")
    print(f"Final: alt={traj[-1].altitude_m/1000:.1f}km, V={traj[-1].velocity_magnitude:.0f}m/s")
