...
class LatencyProfile:
    ...
    def from_measurements(
        ...
        return cls(
            ...
            p99_ms=(
                sorted_measurements[int(n * 0.99)]
                if n >= 100
    ...
    def profile_precision(
        ...
        for policy in [PrecisionPolicy.FULL, PrecisionPolicy.MIXED_FP16]:
            ...
            if reference_output is not None:
                ...
                accuracy = 1.0 - float(
                    torch.mean(diff / (torch.abs(reference_output) + 1e-8))
                )
    ...
    def optimize_batch_size(
        ...
        for batch_size in range(min_batch, max_batch + 1):
            ...
            if sample_input.dim() == 0:
                ...
                batched = sample_input.unsqueeze(0).expand(
                    batch_size, *sample_input.shape
                )
...
class LatencyOptimizer:
    ...
    def get_recommendations(
        ...
        if profile.mean_ms > self.target.target_ms:
            ...
            recommendations.append("Profile individual layers to identify bottlenecks")
