...
class OperatorGraph:
    ...
    def get_node(self, node_id: int) -> OperatorNode | None:
        """Get a node by ID.
...
class KernelFuser:
    ...
    ) -> list[tuple[list[int], FusionPattern]]:
    ...
    def fuse_sequence(
        ...
        for succ_id in last_node.outputs:
            if succ_id in graph.nodes:
                ...
                succ.inputs = [
                    fused_id if i == node_ids[-1] else i for i in succ.inputs
                ]
