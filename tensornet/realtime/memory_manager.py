...
class StreamingBuffer:
    ...
    def synchronize(self, buffer_idx: int | None = None) -> None:
...
class MemoryPool:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        """
        total_free = sum(
            sum(h.size_bytes for h in handles) for handles in self._free_list.values()
        )
...
def optimize_memory(
    ...
    lifetimes: list[tuple[int, int]] | None = None,
    ...
    Returns:
        Optimization result
    """
    specs = [{"shape": tuple(t.shape), "dtype": t.dtype} for t in tensors]
