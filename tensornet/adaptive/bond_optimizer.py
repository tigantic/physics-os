...
class EntropyMonitor:
    ...
    def update(
        ...
        self.peak_entropies[bond_index] = max(self.peak_entropies[bond_index], entropy)
        return entropy
...
class TruncationScheduler:
    ...
    def _error_based_target(self) -> tuple[int, str]:
        ...
        if error_rate > target_error * 10:
            ...
            new_chi = min(
                int(current_chi * self.config.growth_factor), self.config.chi_max
            )
            ...
            new_chi = max(
                int(current_chi * self.config.shrink_factor), self.config.chi_min
            )
...
class AdaptiveTruncator:
    ...
    def truncate(
        ...
        if min(m, n) > 100:
            U, S, V = torch.svd_lowrank(
                tensor.reshape(m, -1), q=min(target_chi + 10, min(m, n))
            )
            Vh = V.T
        else:
    ...
    def step(self) -> AdaptationEvent | None:
...
def estimate_optimal_chi(
    ...
    # Combine estimates
    chi_estimate = int(
        math.ceil(safety_factor * chi_entropy * math.log(error_factor + 1))
    )
...
def adapt_during_evolution(
    ...
    for step in range(num_steps):
        ...
        for bond_idx in range(len(tensors) - 1):
            ...
            chi_right = (
                tensors[bond_idx + 1].shape[0] if tensors[bond_idx + 1].dim() > 1 else 4
            )
