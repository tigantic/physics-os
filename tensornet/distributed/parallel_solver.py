...
from .domain_decomp import DomainDecomposition, SubdomainInfo, DomainConfig
from .communication import Communicator, all_reduce, AllReduceOp
...
class DomainSolver:
    ...
    def __init__(self, subdomain: SubdomainInfo):
        ...
        self.A_local: Optional[Callable] = None
        
        # Preconditioner
        self.M_inv: Optional[Callable] = None
    ...
    def solve_local(self, b: torch.Tensor, x0: Optional[torch.Tensor] = None,
...
class ParallelCGSolver:
    ...
    def solve(self, A: Callable[[torch.Tensor], torch.Tensor],
             b: torch.Tensor,
        ...
        for iteration in range(config.max_iter):
            ...
            if config.verbose and (iteration + 1) % config.log_interval == 0:
                print(f"  CG iteration {iteration+1}: "
        ...
        for outer in range(config.max_iter // self.restart + 1):
            ...
            for j in range(self.restart):
                ...
                for i in range(j):
                    ...
                    H[i + 1, j] = -self._givens_s[i] * temp + self._givens_c[i] * H[i + 1, j]
                ...
                if config.verbose and len(self.history) % config.log_interval == 0:
                    print(f"  GMRES iteration {len(self.history)}: "
...
def parallel_solve(A: Callable[[torch.Tensor], torch.Tensor],
                  b: torch.Tensor,
                  config: Optional[ParallelConfig] = None,
                  method: str = 'cg',
                  decomp: Optional[DomainDecomposition] = None,
                  comm: Optional[Communicator] = None) -> torch.Tensor:
    """
