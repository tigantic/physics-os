...
class SubdomainInfo:
    ...
    # Neighbors
    neighbor_left: Optional[int] = None
    neighbor_right: Optional[int] = None
    neighbor_bottom: Optional[int] = None
    neighbor_top: Optional[int] = None
    neighbor_back: Optional[int] = None
    neighbor_front: Optional[int] = None
    
    # Physical coordinates
    x_local: Optional[torch.Tensor] = None
    y_local: Optional[torch.Tensor] = None
...
class DomainDecomposition:
    ...
    def _create_local_coords(self, offset: int, size: int,
                            ghost_left: int, ghost_right: int,
                            x_min: float, x_max: float, n_global: int
                            ) -> torch.Tensor:
    ...
    def global_to_local(self, rank: int, i: int, j: int, k: int = 0
                       ) -> Tuple[int, int, int]:
    ...
    def local_to_global(self, rank: int, i: int, j: int, k: int = 0
                       ) -> Tuple[int, int, int]:
...
def compute_ghost_zones(data: torch.Tensor, subdomain: SubdomainInfo,
...
def exchange_ghost_data(data: torch.Tensor, subdomain: SubdomainInfo,
                       received: Dict[str, torch.Tensor], n_ghost: int
                       ) -> torch.Tensor:
    """
...
def test_domain_decomposition():
    ...
    for rank in range(4):
        ...
        print(f"    Rank {rank}: global[{sub.i_start}:{sub.i_end}, {sub.j_start}:{sub.j_end}] "
    ...
    print(f"    Ghost exchange completed")
    ...
    for rank in range(8):
        ...
        cells = (sub.i_end - sub.i_start) * (sub.j_end - sub.j_start) * (sub.k_end - sub.k_start)
        total_cells += cells
