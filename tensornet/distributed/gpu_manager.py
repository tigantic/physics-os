...
class GPUManager:
    ...
    def get_pool(self, device_id: Optional[int] = None) -> Optional[MemoryPool]:
...
def select_optimal_device(workload_size: int,
                         memory_required: int,
                         prefer_compute: bool = True) -> int:
    """
...
def distribute_workload(n_elements: int, n_devices: int,
                       balance_strategy: str = 'equal') -> Dict[int, Tuple[int, int]]:
    """
...
def test_gpu_manager():
    ...
    if torch.cuda.is_available():
        ...
        if pool:
            ...
            print(f"    Allocated {stats['n_blocks']} blocks, "
