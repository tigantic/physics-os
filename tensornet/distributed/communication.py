...
class Communicator:
    ...
    def send(self, data: torch.Tensor, dest: int, tag: int = 0):
        ...
        """
        message = Message(
            source=self.rank, destination=dest, tag=tag, data=data.clone()
        )
    ...
    def wait(self, request_id: int) -> torch.Tensor | None:
        """
...
def async_send(comm: Communicator, data: torch.Tensor, dest: int, tag: int = 0) -> int:
    """
...
def all_reduce(
    comm: Communicator, data: torch.Tensor, op: AllReduceOp = AllReduceOp.SUM
) -> torch.Tensor:
    """
...
def broadcast(comm: Communicator, data: torch.Tensor, root: int = 0) -> torch.Tensor:
    """
...
def scatter(
    comm: Communicator, data: list[torch.Tensor] | None, root: int = 0
) -> torch.Tensor:
    """
...
def gather(
    comm: Communicator, data: torch.Tensor, root: int = 0
...
class DistributedTensor:
    ...
    def __init__(
        self, local_data: torch.Tensor, comm: Communicator, distribution: str = "block"
    ):
    ...
    def to_global(self) -> torch.Tensor | None:
        """
