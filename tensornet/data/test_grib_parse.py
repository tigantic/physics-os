...
def try_open_grib(filepath: Path, filter_config: dict) -> "xr.Dataset":
    ...
    return xr.open_dataset(
        filepath,
        engine='cfgrib',
        backend_kwargs={'filter_by_keys': filter_config}
    )
...
def inspect_and_extract(filepath: Path) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    if u_var is None or v_var is None:
        print(f"[ERROR] Could not find U/V wind components")
    ...
    if nan_count > 0:
        print(f"[WARNING] Found {nan_count} NaN values ({100*nan_count/u_data.size:.1f}%)")
        print(f"[FIX] Replacing NaNs with 0.0...")
