...
from .health_monitor import HealthConfig, HealthMonitor, HealthState, HealthStatus
...
class TwinTelemetry:
    ...
    # State
    physical_state: StateVector | None = None
    digital_state: StateVector | None = None
    ...
    # Predictions
    predicted_state: StateVector | None = None
...
class DigitalTwin:
    ...
    def __init__(self, config: DigitalTwinConfig):
        ...
        # State
        self.current_physical_state: StateVector | None = None
        self.current_digital_state: StateVector | None = None
        ...
        # Callbacks
        self.on_divergence: Callable[[float], None] | None = None
        self.on_health_warning: Callable[[HealthState], None] | None = None
        self.on_maintenance_due: Callable[[MaintenanceSchedule], None] | None = None
    ...
    def _init_subsystems(self):
        ...
        self.rom: ReducedOrderModel | None = None

    def initialize(self, training_data: torch.Tensor | None = None):
        """
        ...
        if training_data is not None and self.config.use_rom:
            ...
            self.rom = create_rom_from_snapshots(
                training_data, self.config.rom_type, rom_config
            )
        ...
        if self.maintenance:
            ...
            self.maintenance.register_component(
                "ELEV-LEFT", ComponentType.CONTROL_SURFACE
            )
            self.maintenance.register_component(
                "ELEV-RIGHT", ComponentType.CONTROL_SURFACE
            )
    ...
    def start(self, mode: TwinMode = TwinMode.SHADOW):
        ...
        if mode in [TwinMode.SHADOW, TwinMode.PREDICTIVE, TwinMode.CONTROL]:
            sync_mode = (
                SyncMode.REAL_TIME if mode != TwinMode.MONITORING else SyncMode.BATCH
            )
    ...
    def get_current_state(self) -> StateVector | None:
        """Get best current state estimate."""
        return self.state_sync.get_synchronized_state()

    def predict(
        self, horizon: float, control_sequence: np.ndarray | None = None
    ) -> StateVector | None:
        """
        ...
        if self.rom is not None:
            ...
            return StateVector.from_vector(
                pred_tensor.cpu().tolist(), current.timestamp + horizon
            )
    ...
    def get_health_status(self) -> HealthState | None:
    ...
    def _check_divergence(self):
        ...
        divergence = compute_state_divergence(
            self.current_physical_state, self.current_digital_state
        )
    ...
    def _update_health(self):
        ...
        temps = (
            ...
            else np.array([state.mach * 200 + 1000])
        )
    ...
    def _log_telemetry(self):
        ...
        telemetry = TwinTelemetry(
            ...
            divergence=(
                self.state_sync.divergence_history[-1]["total"]
                if self.state_sync.divergence_history
                else 0.0
            ),
...
def create_vehicle_twin(
    vehicle_id: str, config: DigitalTwinConfig | None = None
) -> DigitalTwin:
    """
...
def validate_twin_fidelity(
    twin: DigitalTwin, test_states: list[StateVector], tolerance: float = 0.1
) -> dict[str, Any]:
    """
