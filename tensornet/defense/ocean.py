...
class SoundSpeedProfile:
    ...
    thermocline_depth: float | None = None  # Depth of sharp gradient (m)
...
class OceanDomain:
    ...
    def __init__(
        ...
        range_km: float = 50.0,
        grid_res: float = 10.0,
        ssp: SoundSpeedProfile | None = None,
        ...
        self.bottom_depth = torch.full(
            (self.nx,), float(self.nz - 1), device=self.device
        )
    ...
    def _generate_sound_speed(self) -> torch.Tensor:
        ...
        # c(z) = c₀ * (1 + ε * (η + e^(-η) - 1))
        c_profile = self.ssp.c0 * (
            1.0 + self.ssp.epsilon * (eta + torch.exp(-eta) - 1.0)
        )
    ...
    def add_seamount(
        ...
        print(
            f"[OCEAN] Seamount: x={x_pos_km}km, height={height_m}m, peak at {peak_depth}m depth"
        )
...
def create_deep_ocean(
    range_km: float = 50.0, with_seamount: bool = True
) -> OceanDomain:
    """
