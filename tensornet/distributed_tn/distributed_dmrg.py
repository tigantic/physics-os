...
class DMRGPartition:
    ...
    left_boundary: torch.Tensor | None = None
    right_boundary: torch.Tensor | None = None
    energy: float = 0.0
            ...
            if i + 1 < len(self.partition.tensors):
                ...
                if min(m, n) > 100:
                    U, S, V = torch.svd_lowrank(
                        combined, q=min(self.chi_max + 10, min(m, n))
                    )
                    Vh = V.T
                else:
                ...
                if direction == "right":
                    ...
                    self.partition.tensors[i + 1] = (torch.diag(S) @ Vh).reshape(
                        chi_new, d, chi_right
                    )
                else:
                    self.partition.tensors[i] = (U @ torch.diag(S)).reshape(
                        chi_left, d, chi_new
                    )
    ...
    def partition_system(
        ...
        if self.config.strategy == PartitionStrategy.EQUAL:
            ...
            partition_points = sorted(
                [e[0] + 1 for e in entropies[: num_partitions - 1]]
            )
