...
class ArterySimulation:
    ...
    def __init__(
        ...
        self.velocity = torch.zeros(
            (3, *self.shape), device=self.device, dtype=torch.float64
        )
        ...
        print("[MEDICAL] Artery Simulation Initialized")
    ...
    def compute_viscosity(self, shear_rate: torch.Tensor) -> torch.Tensor:
        ...
        viscosity = self.mu_inf + (self.mu_0 - self.mu_inf) * torch.pow(
            term, (n - 1) / 2
        )
    ...
    def solve_blood_flow(
        ...
        for t in range(steps):
            ...
            if verbose and t % 50 == 0:
                print(
                    f"   Heartbeat {t:3d}: Peak Vel={current_peak_v:.1f} cm/s | "
                    f"Wall Shear={current_peak_shear:.2f} Pa"
                )

                if current_peak_shear > 15.0:
                    print("   ⚠️  [ALERT] HIGH RUPTURE RISK at stenosis!")
