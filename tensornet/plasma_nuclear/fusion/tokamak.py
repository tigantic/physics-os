...
class TokamakReactor:
    ...
    def __init__(
        ...
        print("[TOKAMAK] Reactor initialized")
        print(
            f"[TOKAMAK] R₀={major_radius}m, a={minor_radius}m, A={self.aspect_ratio:.2f}"
        )
    ...
    def create_plasma(
        ...
        phi = (
            torch.rand(num_particles, device=self.device, dtype=torch.float64)
            * 2
            * np.pi
        )
        ...
        r_offset = torch.randn(
            num_particles, device=self.device, dtype=torch.float64
        ) * (0.1 * self.a)
        z_offset = torch.randn(
            num_particles, device=self.device, dtype=torch.float64
        ) * (0.1 * self.a)
        ...
        vx = (
            -torch.sin(phi) * toroidal_flow
            + torch.randn(num_particles, device=self.device, dtype=torch.float64)
            * temperature
        )
        vy = (
            torch.cos(phi) * toroidal_flow
            + torch.randn(num_particles, device=self.device, dtype=torch.float64)
            * temperature
        )
        vz = (
