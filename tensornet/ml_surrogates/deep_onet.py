...
def create_deeponet(branch_dim: int = 100,
                   trunk_dim: int = 3,
                   latent_dim: int = 64,
                   n_outputs: int = 1,
                   **kwargs) -> DeepONet:
...
def train_deeponet(model: DeepONet,
...
def test_deep_onet():
    ...
    history = train_deeponet(
        small_onet, u_train, y_train, s_train,
        n_epochs=50, lr=1e-3, batch_size=32, verbose=False
    )
