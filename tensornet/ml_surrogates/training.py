...
from .surrogate_base import CFDSurrogate, SurrogateMetrics
...
class SurrogateTrainer:
    ...
    def train(self, x_train: torch.Tensor, y_train: torch.Tensor,
             ...
             y_val: Optional[torch.Tensor] = None) -> Dict[str, List[float]]:
        """
        ...
        for epoch in range(config.n_epochs):
            ...
            # Early stopping
            if config.early_stopping and self.epochs_without_improvement >= config.patience:
                if config.verbose:
...
class DataAugmentor:
    ...
    def augment_batch(self, x: torch.Tensor, y: torch.Tensor,
                     n_augmentations: int = 1) -> Tuple[torch.Tensor, torch.Tensor]:
...
class ActiveLearner:
    ...
    def __init__(self, model: CFDSurrogate,
...
def train_surrogate(model: CFDSurrogate,
...
def cross_validate(model_class: type,
                  model_config: Any,
                  x: torch.Tensor,
                  y: torch.Tensor,
                  n_folds: int = 5,
...
def test_training():
    ...
    model = MLPSurrogate(SurrogateConfig(input_dim=4, output_dim=1, hidden_dims=[32, 32]))
    ...
    model2 = MLPSurrogate(SurrogateConfig(input_dim=4, output_dim=1, hidden_dims=[16, 16]))
    ...
    model3 = MLPSurrogate(SurrogateConfig(input_dim=4, output_dim=1, hidden_dims=[16, 16]))
