...
class SliceSpec:
    ...
    # LOD / quality
    max_rank: int | None = None  # Rank cap for this slice
    error_budget: float | None = None  # Max truncation error
...
class StepControls:
    ...
    # Budget constraints
    max_ms: float | None = None  # Frame budget in milliseconds
    max_rank: int | None = None  # Rank cap
    max_error: float | None = None  # Error cap
...
class Field:
    ...
    @staticmethod
    def _init_taylor_green_core(
        i: int, n_cores: int, r_left: int, r_right: int
    ) -> torch.Tensor:
    ...
    def slice(
        ...
        if spec.plane == "xy":
            x_coords = torch.linspace(
                spec.x_range[0], spec.x_range[1], res[0], device=self.device
            )
            y_coords = torch.linspace(
                spec.y_range[0], spec.y_range[1], res[1], device=self.device
            )
            z_coord = spec.depth
            ...
            z_coords = torch.linspace(
                spec.z_range[0], spec.z_range[1], res[1], device=self.device
            )
            y_coord = spec.depth
            ...
            y_coords = torch.linspace(
                spec.y_range[0], spec.y_range[1], res[0], device=self.device
            )
            z_coords = torch.linspace(
                spec.z_range[0], spec.z_range[1], res[1], device=self.device
            )
    ...
    def step(
        ...
        new_field._energy_history = self._energy_history + [
            self._compute_energy(new_cores)
        ]
    ...
    def stats(self) -> FieldStats:
        ...
        ranks = [c.shape[0] for c in self.cores[1:]] + [
            c.shape[2] for c in self.cores[:-1]
        ]
        ...
        energy = (
            ...
            else self._compute_energy(self.cores)
        )
    ...
    def serialize(self) -> FieldBundle:
        ...
        """
        from .bundle import BundleMetadata, FieldBundle
    ...
    def _slice_3d(self, spec: SliceSpec) -> torch.Tensor:
        ...
        if len(res) != 3:
            res = (
                (res[0], res[0], res[0]) if len(res) == 1 else (res[0], res[1], res[0])
            )
        ...
        z_coords = torch.linspace(
            spec.z_range[0], spec.z_range[1], res[2], device=self.device
        )
    ...
    def _apply_advection(
        self, cores: list[torch.Tensor], controls: StepControls
    ) -> list[torch.Tensor]:
    ...
    def _compute_energy_drift(self) -> float:
        ...
        if len(self._energy_history) < 2:
            return 0.0
        return (self._energy_history[-1] - self._energy_history[0]) / max(
            self._energy_history[0], 1e-10
        )
