...
class OperatorLog:
    ...
    def add(self, op_name: str, params: Dict[str, Any], hash_before: str, hash_after: str):
...
class FieldBundle:
    ...
    def load(cls, path: str) -> 'FieldBundle':
        ...
        for shape, dtype_str in zip(header['core_shapes'], header['core_dtypes']):
            ...
            arr = np.frombuffer(core_bytes[offset:offset + size], dtype=dtype).reshape(shape)
    ...
    def replay_info(self) -> str:
        ...
        for i, entry in enumerate(self.operator_log.entries[-5:]):  # Last 5
            lines.append(f"  [{i}] {entry['op']}: {entry['hash_before'][:8]} -> {entry['hash_after'][:8]}")
