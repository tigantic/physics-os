...
class TensorSlicer:
    ...
    def __init__(self, cores: List[np.ndarray], dtype=np.float64):
        ...
        for i, core in enumerate(self.cores):
            ...
            if core.shape[1] != 2:
                raise ValueError(f"Core {i} physical dim must be 2, got {core.shape[1]}")
    ...
    def get_element(self, index: Union[int, str]) -> float:
        """
    ...
    def render_slice_2d(self,
                        x_cores: List[int],
                        y_cores: List[int],
                        fixed_indices: Optional[dict] = None,
                        resolution: Tuple[int, int] = (256, 256)) -> np.ndarray:
        """
        ...
        if len(all_specified) != self.n_cores:
            raise ValueError("Must specify all cores via x_cores, y_cores, or fixed_indices")
        ...
        for py in range(height):
            for px in range(width):
                ...
                x_bits = format(int(px * (2**len(x_cores) - 1) / max(1, width-1)), 
                               f'0{len(x_cores)}b')
                for i, core_idx in enumerate(x_cores):
                    binary[core_idx] = x_bits[i]
                
                # Map Y pixel to y_cores  
                y_bits = format(int(py * (2**len(y_cores) - 1) / max(1, height-1)),
                               f'0{len(y_cores)}b')
    ...
    def render_slice_2d_vectorized(self,
                                    x_cores: List[int],
                                    y_cores: List[int], 
                                    fixed_indices: Optional[dict] = None,
                                    resolution: Tuple[int, int] = (256, 256)) -> np.ndarray:
    ...
    def render_zoomed(self,
                      center: Tuple[float, float],
                      zoom_level: int,
                      resolution: Tuple[int, int] = (256, 256),
                      x_cores: Optional[List[int]] = None,
                      y_cores: Optional[List[int]] = None) -> np.ndarray:
        """
    ...
    def benchmark_render(self, resolution: Tuple[int, int] = (256, 256)) -> dict:
        ...
        for _ in range(1000):
            self.get_element(0)
        results['single_point_us'] = (time.perf_counter() - t0) * 1000  # microseconds per call
...
if __name__ == "__main__":
    ...
    for zoom in [1, 4, 16, 64]:
        ...
        print(f"  Zoom {zoom:2d}x: shape={img.shape}, range=[{img.min():.2f}, {img.max():.2f}]")
