...
Usage::
    ...
    python -m hypertensor.cli run     --domain burgers --n-bits 8
    python -m hypertensor.cli validate envelope.json
    python -m hypertensor.cli attest  envelope.json
    python -m hypertensor.cli verify  certificate.json
    python -m hypertensor.cli serve   --host 0.0.0.0 --port 8000

Every command writes structured JSON to stdout.  Human-readable
summaries go to stderr.  This makes it easy to pipe results::

    hypertensor run --domain burgers | jq '.conservation'
"""
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    from hypertensor.core.evidence import generate_validation_report, generate_claims
    from hypertensor.core.certificates import issue_certificate
    from hypertensor.core.hasher import content_hash
    from hypertensor.core.registry import DOMAINS
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    from hypertensor.core.evidence import generate_validation_report
    from hypertensor.core.hasher import content_hash, verify_hash
    from hypertensor.core.certificates import verify_certificate
...
def cmd_attest(args: argparse.Namespace) -> None:
    ...
    from hypertensor.core.evidence import generate_claims
    from hypertensor.core.certificates import issue_certificate
    from hypertensor.core.hasher import content_hash
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        version=f"hypertensor {hypertensor.__version__}",
    )
