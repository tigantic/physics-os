...
#include "OnticFieldActor.h"
#include "ProceduralMeshComponent.h"
#include "Materials/MaterialInstanceDynamic.h"
#include "DrawDebugHelpers.h"

AOnticFieldActor::AOnticFieldActor()
{
    ...
    FieldComponent = CreateDefaultSubobject<UOnticFieldComponent>(TEXT("FieldComponent"));
...
void AOnticFieldActor::BeginPlay()
{
    Super::BeginPlay();
...
void AOnticFieldActor::Tick(float DeltaTime)
{
...
void AOnticFieldActor::ApplyImpulseAtHit(const FHitResult& HitResult, float Strength, float Radius)
{
...
FVector4 AOnticFieldActor::GetFieldValueAt(const FVector& WorldPosition) const
{
    if (!FieldComponent->IsInitialized())
    {
...
void AOnticFieldActor::UpdateVisualization()
{
...
void AOnticFieldActor::CreateBoundingBoxMesh()
{
    if (!FieldComponent->IsInitialized())
    {
        return;
...
void AOnticFieldActor::CreateSliceMesh()
{
    if (!FieldComponent->IsInitialized())
    {
        return;
