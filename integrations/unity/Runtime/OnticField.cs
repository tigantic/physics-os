...
namespace Tigantic.Ontic
{
    ...
    public class OnticField : MonoBehaviour
    {
        ...
        public bool Initialize(int sizeX, int sizeY, int sizeZ, FieldType type = FieldType.Vector)
        {
            if (isInitialized)
            {
                Debug.LogWarning("OnticField already initialized");
                return false;
            }
            ...
            isInitialized = true;
            Debug.Log($"OnticField initialized: {sizeX}x{sizeY}x{sizeZ}, Type={type}");
        ...
        private INativeBridge CreateNativeBridge()
        {
            ...
            try
            {
                ...
                {
                    Debug.Log("Connected to Ontic Python backend via ZMQ");
                    return bridge;
                }
            }
            catch (Exception e)
