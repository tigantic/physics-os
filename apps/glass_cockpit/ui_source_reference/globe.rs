...
impl GlobeCamera {
    ...
    pub fn update(&mut self, delta_time: f32) {
        // [TUNING] INERTIA:
        // 0.90 = Stops fast (Plastic)
        // 0.99 = Drifts forever (Heavy Satellite) -> ICE PHYSICS
        let inertia = 0.99;
        ...
        if !self.is_dragging {
            ...
            // Lowered the "stop" threshold so it drifts longer
            if self.velocity_azimuth.abs() < 0.0001 { self.velocity_azimuth = 0.0; }
            if self.velocity_polar.abs() < 0.0001 { self.velocity_polar = 0.0; }
        }
    ...
    /// Pan camera - RAW POWER MODE
    pub fn pan(&mut self, delta_x: f32, delta_y: f32) {
        // [TUNING] SENSITIVITY: Cranked way up for responsiveness
        let sensitivity = 3.0; 
        
        // Raw power - no extra 0.001 multiplier, just 0.01 to keep it sane
