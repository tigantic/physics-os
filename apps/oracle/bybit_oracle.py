"""
Bybit Live Oracle Engine
========================
High-volume Bybit WebSocket streaming with Triton kernel processing.

Bybit is one of the highest-volume exchanges, great for regime detection.
"""
...
class ZeroLoopSlicer:
    def __init__(self, batch_size: int, window_size: int, bond_dim: int, assets: int):
...
# =============================================================================
# BYBIT CONFIG - Using Linear Perpetuals for maximum volume
# =============================================================================
...
# Bybit has spot and linear (perpetuals) - linear has 100x more volume
WEBSOCKET_URL = "wss://stream.bybit.com/v5/public/linear"  # Linear perpetuals
...
class BybitOracle:
    ...
    def _display_status(self):
        now = time.time()
        if now - self.last_display < 1.0:  # Update every 1 second for terminal visibility
            return
        ...
        # Use newline for terminal capture compatibility
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {' '.join(prices_parts)} | "
              f"{color}{regime:8s}{reset} H={entropy:.1f} | "
              f"{self.trade_count:>7,} trades {tps:>6,.0f}/s", flush=True)
    ...
    async def run(self):
        ...
        print()
        print("=" * 75)
        print("  BYBIT LIVE ORACLE ENGINE")
        print("=" * 75)
        ...
        subscribe_msg = {
            "op": "subscribe",
            "args": [f"publicTrade.{sym}" for sym in SYMBOLS.keys()]
        }
            ...
            async with websockets.connect(
                ...
                async for message in ws:
                    ...
                    try:
                        ...
                        # Bybit trade format: {"topic": "publicTrade.BTCUSDT", "data": [{"p": "...", ...}]}
...
async def main():
    oracle = BybitOracle(batch_size=64)
    await oracle.run()
