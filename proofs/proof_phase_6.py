...
def gate_energy_enstrophy_balance():
    ...
    relative_errors = np.abs(
        dE_dt[mid_start:mid_end] - dissipation[mid_start:mid_end]
...
def gate_regularity_certificate():
    ...
    certificate = {
        ...
        "evolution": {"T_final": T_final, "n_steps": n_steps, "dt": dt},
        "regularity_analysis": {
    ...
    certificate["certificate_valid"] = (
        chi_bounded and assessment_smooth and gradient_bounded
    )
    ...
    print(
        f"  Gradient ratio: {certificate['regularity_analysis']['gradient_ratio']:.4f}"
    )
...
def gate_scaling_law():
    ...
    for N in resolutions:
        ...
        print(
            f"N={N:3d}: ||nablau||_smooth={grad_smooth[-1]:.4f}, ||nablau||_rough={grad_rough[-1]:.4f}"
        )
