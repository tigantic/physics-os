...
def gate_velocity_to_qtt():
    ...
    print(
        f"u field: max chi = {max_chi_u}, compression = {result_u.compression_ratio:.1f}x"
    )
    print(
        f"v field: max chi = {max_chi_v}, compression = {result_v.compression_ratio:.1f}x"
    )
...
def gate_compression_ratio():
    ...
    k_modes = [1, 2, 3, 5, 8]
    u_multi = sum(torch.sin(k * X) * torch.cos(k * Y) / k**2 for k in k_modes)
    ...
    print(
        f"Single mode: chi_max = {max(result_smooth.bond_dimensions)}, "
        f"compression = {result_smooth.compression_ratio:.1f}x"
    )
    print(
        f"Multi-mode:  chi_max = {max(result_multi.bond_dimensions)}, "
        f"compression = {result_multi.compression_ratio:.1f}x"
    )
    print(
        f"Random:      chi_max = {max(result_random.bond_dimensions)}, "
    ...
    chi_ordering = (
        max(result_smooth.bond_dimensions)
...
def gate_tt_poisson():
    ...
    rel_error2 = (
        phi_computed2_zm - phi_exact2_zm
    ).abs().max().item() / phi_exact2_zm.abs().max().item()
