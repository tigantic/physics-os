...
def gate_blowup_indicator_sensitivity():
    ...
    spectral_variation = (spectral_values.max() - spectral_values.min()) / (
        spectral_values.mean() + 1e-10
    )
...
def gate_adaptive_resolution_response():
    ...
    for step in range(n_steps + 1):
        ...
        chi_state = compute_chi_state_2d(
            state.u, state.v, t=t, dx=dx, dy=dy, chi_target=chi_target_low
        )
    ...
    for step in range(n_steps + 1):
        t = step * dt
        chi_state = compute_chi_state_2d(
            state.u, state.v, t=t, dx=dx, dy=dy, chi_target=chi_target_high
        )
...
def gate_prevention_via_regularization():
    ...
    for nu in [0.01, 0.02, 0.05, 0.1]:  # More reasonable range
        ...
        chi_state_final = compute_chi_state_2d(
            state.u, state.v, t=n_steps * dt, dx=dx, dy=dy
        )
...
def gate_warning_system_timing():
    ...
    for step in range(n_steps + 1):
        t = step * dt
        chi_state = compute_chi_state_2d(
            state.u, state.v, t=t, dx=dx, dy=dy, chi_target=16  # Moderate target
        )
        trajectory.add(chi_state)
    ...
    warning_is_early = (
        (warning_step is None)
        or (enstrophy_peak_step == 0)
        or (warning_step <= enstrophy_peak_step + 10)
    )
    ...
    has_warning_capability = (warning_step is not None) or (
        chi_ratios.max() > warning_threshold
    )
