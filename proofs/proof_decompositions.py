...
def proof_1_1_svd_truncation_optimality() -> ProofResult:
    ...
    for k in [5, 10, 20, 40]:
        ...
        measurements.append(
            {"k": k, "our_error": our_error, "ref_error": ref_error, "diff": diff}
        )
...
def proof_1_2_svd_orthogonality() -> ProofResult:
    ...
    # Check U^T @ U = I
    U_ortho_error = torch.linalg.norm(
        U.T @ U - torch.eye(U.shape[1], dtype=torch.float64)
    ).item()

    # Check V @ V^T = I (Vh^T @ Vh)
    V_ortho_error = torch.linalg.norm(
        Vh @ Vh.T - torch.eye(Vh.shape[0], dtype=torch.float64)
    ).item()
...
def proof_1_3_qr_reconstruction() -> ProofResult:
    ...
    return ProofResult(
        ...
        measurements=[{"property": "||A - Q @ R||_F", "value": reconstruction_error}],
        tolerance=NUMERICAL_STABILITY,
...
def proof_1_4_qr_orthogonality() -> ProofResult:
    ...
    ortho_error = torch.linalg.norm(
        Q.T @ Q - torch.eye(Q.shape[1], dtype=torch.float64)
    ).item()
    ...
    return ProofResult(
        ...
        measurements=[{"property": "||Q^T @ Q - I||_F", "value": ortho_error}],
        tolerance=NUMERICAL_STABILITY,
