...
from tensornet.cfd.weno import (ReconstructionSide, WENOConfig,
                                convergence_order, smoothness_indicators,
                                teno5, weno5_js, weno5_z)
...
def test_weno_convergence(
    weno_fn, test_fn, N_values: list, domain: tuple = (0.0, 1.0), name: str = "WENO"
) -> dict:
    """
    ...
    for N in N_values:
        ...
        u_avg = (torch.cos(2 * np.pi * x_left) - torch.cos(2 * np.pi * x_right)) / (
            2 * np.pi * dx
        )
...
def main():
    ...
    results["tests"].append({"name": "smoothness_indicators", **beta_results})
