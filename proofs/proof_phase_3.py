...
def gate_chi_growth_physics():
    ...
    solver2 = NS2DSolver(
        Nx=N, Ny=N, Lx=L, Ly=L, nu=0.001, dtype=torch.float64
    )  # Lower viscosity
    ...
    print(
        f"Smooth flow: chi growth = {analysis_smooth['chi_growth_rate']:.4f}, "
        f"assessment = {analysis_smooth['regularity_assessment']}"
    )
    print(
        f"Perturbed:   chi growth = {analysis_pert['chi_growth_rate']:.4f}, "
