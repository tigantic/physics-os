...
def gate_enstrophy_chi_relationship():
    ...
    monotonic_decay = all(
        enstrophy[i] >= enstrophy[i + 1] * 0.999 for i in range(len(enstrophy) - 1)
    )
...
def gate_spectral_radius_tracking():
    ...
    print(
        f"Smooth: lambda_max(0)={lambda_smooth[0]:.4f}, lambda_max(T)={lambda_smooth[-1]:.4f}"
    )
    print(
        f"Perturbed: lambda_max(0)={lambda_perturbed[0]:.4f}, lambda_max(T)={lambda_perturbed[-1]:.4f}"
    )
...
def gate_regularity_assessment_consistency():
    ...
    print(
        f"  (smooth chi_max={smooth_max_chi:.2f}, perturbed chi_max={perturbed_max_chi:.2f})"
    )
