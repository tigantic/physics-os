...
def compare_with_naive_interpolation(N: int = 100):
    ...
    for i in range(n_recon):
        ...
        if idx - 2 >= 0 and idx + 2 < N:
            um2, um1, u0, up1, up2 = u[idx-2], u[idx-1], u[idx], u[idx+1], u[idx+2]
    ...
    return {
        ...
        'weno_is_better': (osc_weno < osc_linear) or \
                          (overshoot_weno['total_violation'] < overshoot_linear['total_violation']),
    }
...
def main():
    ...
    result1 = test_eno_property(
        weno5_js, step_function, N=100,
        u_bounds=(1.0, 2.0), name="WENO5-JS step"
    )
    ...
    result2 = test_eno_property(
        weno5_z, step_function, N=100,
        u_bounds=(1.0, 2.0), name="WENO5-Z step"
    )
    ...
    result3 = test_eno_property(
        teno5, step_function, N=100,
        u_bounds=(1.0, 2.0), name="TENO5 step"
    )
    ...
    result4 = test_eno_property(
        weno5_z, two_step_function, N=100,
    ...
    result5 = test_eno_property(
        weno5_z, sod_density_profile, N=200,
    ...
    for N in [50, 100, 200, 400]:
        result = test_eno_property(
            weno5_z, step_function, N=N,
            u_bounds=(1.0, 2.0), name=f"WENO5-Z N={N}"
        )
