...
def gate_re1000_chi_tracking() -> Dict:
    ...
    passed = (
        chi_max < 500  # Reasonable bound for Re=1000
        and analysis.get("regularity_assessment") != "potential_blowup"
    )
...
def gate_re10000_chi_tracking() -> Dict:
    ...
    passed = (
        chi_max < 2000 and analysis.get("regularity_assessment") != "potential_blowup"
    )
...
def gate_reynolds_scaling_law() -> Dict:
    ...
    for Re in Re_values:
        ...
        solver = NS3DSolver(
            Nx=N, Ny=N, Nz=N, Lx=L, Ly=L, Lz=L, nu=nu, dtype=torch.float64
        )
        state = solver.create_taylor_green_3d()
        ...
        try:
            for step in range(n_steps):
                ...
                if step % max(1, n_steps // 10) == 0:
                    chi_state = compute_chi_state_3d(
                        state.u, state.v, state.w, t=step * dt, dx=dx, dy=dy, dz=dz
                    )
    ...
    if alpha < 0.5:
        interpretation = (
            "SUBLINEAR: chi grows slowly with Re (favorable for regularity)"
        )
    elif alpha < 1.0:
    ...
    passed = (
        len(chi_max_values) >= 3
        and np.isfinite(alpha)
        and r_squared > 0.5  # Reasonable fit
    )
...
def gate_blowup_detection() -> Dict:
    ...
    try:
        for step in range(n_steps + 1):
            ...
            if step % sample_interval == 0:
                chi_state = compute_chi_state_3d(
                    state.u, state.v, state.w, t=t, dx=dx, dy=dy, dz=dz, chi_target=512
                )
                trajectory.add(chi_state)
    ...
    first_half_growth = (
        (chi_values[mid] - chi_values[0]) / (times[mid] - times[0]) if mid > 0 else 0
    )
    second_half_growth = (
        (chi_values[-1] - chi_values[mid]) / (times[-1] - times[mid])
        if times[-1] > times[mid]
        else 0
    )
