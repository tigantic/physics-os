...
def compute_vorticity_field(u: torch.Tensor, v: torch.Tensor, w: torch.Tensor,
    ...
    omega_x = (torch.roll(w, -1, dims=1) - torch.roll(w, 1, dims=1)) / (2*dy) - \
              (torch.roll(v, -1, dims=2) - torch.roll(v, 1, dims=2)) / (2*dz)
    
    # ω_y = ∂u/∂z - ∂w/∂x
    omega_y = (torch.roll(u, -1, dims=2) - torch.roll(u, 1, dims=2)) / (2*dz) - \
              (torch.roll(w, -1, dims=0) - torch.roll(w, 1, dims=0)) / (2*dx)
    
    # ω_z = ∂v/∂x - ∂u/∂y
    omega_z = (torch.roll(v, -1, dims=0) - torch.roll(v, 1, dims=0)) / (2*dx) - \
              (torch.roll(u, -1, dims=1) - torch.roll(u, 1, dims=1)) / (2*dy)
    
    return omega_x, omega_y, omega_z


def compute_max_vorticity(u: torch.Tensor, v: torch.Tensor, w: torch.Tensor,
...
def compute_enstrophy(u: torch.Tensor, v: torch.Tensor, w: torch.Tensor,
                      dx: float, dy: float, dz: float) -> float:
    """
...
def proof_5_2_bkm_convergence() -> BKMResult:
    ...
    for r in results:
        print(f"  {r['N']:>6} | {r['max_vort_peak']:>12.4f} | {r['bkm_integral']:>12.4f}")
...
def proof_5_3_enstrophy_evolution() -> BKMResult:
    ...
    for step in range(n_steps):
        ...
        if step % (n_steps // 5) == 0:
            print(f"  t={t:.3f}: E = {E:.4f} ({E/E0*100:.1f}%), Ω = {Omega:.4f} ({Omega/Omega0*100:.1f}%)")
    ...
    print(f"  Final Enstrophy: {Omega_final:.4f} ({Omega_final/Omega0*100:.1f}% of initial)")
...
def proof_5_4_high_reynolds() -> BKMResult:
    ...
    for Re in reynolds_numbers:
        ...
        print(f"    {status}: BKM = {bkm_integral:.4f}, Peak ‖ω‖_∞ = {max_vort_peak:.4f}")
    ...
    for r in results:
        ...
        print(f"  {r['Re']:>8} | {r['max_vort_peak']:>12.4f} | {r['bkm_integral']:>12.4f} | {status:>10}")
    ...
    print("  " + "-"*55)
    print(f"  BKM ratio (Re={reynolds_numbers[-1]}/Re={reynolds_numbers[0]}): {bkm_ratio:.4f}")
    print(f"\n  VERDICT: {verdict}")
    print(f"  VERIFIED: {verified}")
