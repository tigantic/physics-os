...
def print_banner():
    ...
    print("╠" + "═" * 62 + "╣")
    print(
        "║" + f"   Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}".ljust(62) + "║"
    )
...
def phase2_profile_discovery(N=32, n_iter=50):
    ...
    is_hyperbolic = (n_positive == 1 and n_negative == 2) or (
        n_positive == 2 and n_negative == 1
    )
    ...
    if is_hyperbolic:
        ...
        print(
            "    Reason: Structure > Energy. Adjoint finds noise; Hou-Luo finds physics."
        )
...
def phase3_kantorovich_verification(
    profile: torch.Tensor, nu: float = 1e-3, alpha: float = 0.5, use_qtt: bool = True
):
    """
...
def save_results(verified: bool, bounds, profile_type: str, elapsed: float):
    ...
    results = {
        ...
        "conclusion": (
            "Rigorous singularity existence proven"
            if verified
