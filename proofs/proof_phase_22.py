...
def proof_22_1_saha_equilibrium():
    ...
    from tensornet.cfd.plasma import saha_ionization, Species
    ...
    record_proof("Saha Ionization Equilibrium", passed, {
...
def proof_22_2_plasma_frequency():
    ...
    record_proof("Plasma Frequency Physics", passed, {
...
def proof_22_3_blackout_geometry():
    ...
    freq_monotonic = all(omega_pe[i] <= omega_pe[i+1] for i in range(len(omega_pe)-1))
    ...
    record_proof("Blackout Geometry Consistency", passed, {
...
def proof_22_4_fads_sensitivity():
    ...
    from tensornet.simulation.sensors import FADSSensor, SensorNoiseConfig, NoiseModel
    ...
    record_proof("FADS Sensor Sensitivity", passed, {
...
def proof_22_5_differentiable_cfd():
    ...
    # Advance one step
    rho_new, rhou_new, E_new = solver.step(rho_c.detach(), rhou_c.detach(), E_c.detach())
    ...
    record_proof("Differentiable CFD Conservation", passed, {
...
def proof_22_6_aerotrn_drift():
    ...
    from tensornet.guidance.aero_trn import (
        TerrainMap, AeroTRN, AeroTRNConfig, compute_aero_signature
    )
    ...
    for i in range(ny):
        for j in range(nx):
            # Mountain range
            elevation[i, j] = 1000 + 500 * math.sin(10 * lat_grid[i]) * math.cos(10 * lon_grid[j])
    ...
    record_proof("Aero-TRN Navigation Drift", passed, {
...
def proof_22_7_jet_penetration():
    ...
    from tensornet.cfd.jet_interaction import (
        jet_penetration_height, mach_disk_location, barrel_shock_radius
    )
    ...
    record_proof("Jet Penetration Correlations", passed, {
        "penetration_J5": y1,
        "penetration_J20": y2,
...
def proof_22_8_ji_forces():
    ...
    from tensornet.cfd.jet_interaction import (
        JetConfig, UnderexpandedJet, JetInteractionCorrector
    )
    ...
    record_proof("Jet Interaction Forces", passed, {
...
def proof_22_9_divert_accuracy():
    ...
    from tensornet.guidance.divert import (
        VehicleState, TargetState, DivertThruster, DivertGuidance,
        ThrusterConfig, zero_effort_miss, time_to_go
    )
    ...
    record_proof("Divert Guidance Accuracy", passed, {
        "time_to_go": tgo,
