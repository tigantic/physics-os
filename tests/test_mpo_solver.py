...
@pytest.mark.skip(
    reason="einsum optimize parameter not supported in this torch version"
)
def test_laplacian_operator():
    ...
    num_modes = 12
    laplacian = LaplacianMPO(
        num_modes=num_modes, viscosity=1e-4, dx=1.0 / 64, device=device
    )
    ...
    print(
        f"  Target: <0.2 ms | Status: {'✓ PASS' if elapsed_ms < 0.2 else '✗ FAIL (but expected for non-optimized)'}"
    )
...
def test_projection_operator():
    ...
    print(f"  ⏱ Time: {elapsed_ms:.3f} ms")
    print(
        f"  Target: <0.3 ms | Status: {'✓ PASS' if elapsed_ms < 0.3 else '✗ FAIL (but expected for non-optimized)'}"
    )
...
def test_atmospheric_solver_step():
    ...
    print(
        f"  Target: <0.65 ms | Status: {'✓ PASS' if mean_time_ms < 0.65 else '✗ FAIL (but expected for initial implementation)'}"
    )
    ...
    if stats:
        ...
        for key, timing in stats.items():
            print(
                f"    {key:12s}: {timing['mean_ms']:6.3f} ± {timing['std_ms']:5.3f} ms"
            )
...
def run_all_tests():
    ...
    if passed_count == total_count:
        ...
        print(
            "Note: Some failures expected for initial implementation (performance targets)"
        )
        return 1
