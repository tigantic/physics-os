...
def analytical_sine_3d(N: int, device: torch.device) -> torch.Tensor:
    ...
    return (
        torch.sin(2 * np.pi * xx)
        * torch.sin(2 * np.pi * yy)
        * torch.sin(2 * np.pi * zz)
    )
...
def analytical_sine_z_slice(N: int, z_index: int, device: torch.device) -> torch.Tensor:
    ...
    return (
        torch.sin(2 * np.pi * xx)
        * torch.sin(2 * np.pi * yy)
        * np.sin(2 * np.pi * z_val)
    )


def analytical_gaussian_3d(
    N: int, device: torch.device, sigma: float = 0.1
) -> torch.Tensor:
    """
...
def analytical_gaussian_z_slice(
    N: int, z_index: int, device: torch.device, sigma: float = 0.1
) -> torch.Tensor:
...
def analytical_linear_z_slice(
    N: int, z_index: int, device: torch.device
) -> torch.Tensor:
...
def contract_2d_qtt_to_dense(
    cores: List[torch.Tensor], bits_per_dim: int
) -> torch.Tensor:
    """
...
def run_full_audit(
    ...
    result = run_audit_test(
        "Sine Wave", dense_3d, gt_slice, z_index, max_rank, tolerance
    )
    results.append(result)
    print(
        f"  {'✅ PASS' if result.passed else '❌ FAIL'} | max_err={result.max_error:.2e} | time={result.time_ms:.1f}ms"
    )
    ...
    result = run_audit_test(
        "Gaussian", dense_3d, gt_slice, z_index, max_rank, tolerance
    )
    results.append(result)
    print(
        f"  {'✅ PASS' if result.passed else '❌ FAIL'} | max_err={result.max_error:.2e} | time={result.time_ms:.1f}ms"
    ...
    result = run_audit_test(
        "Constant", dense_3d, gt_slice, z_index, max_rank, tolerance
    )
    results.append(result)
    print(
        f"  {'✅ PASS' if result.passed else '❌ FAIL'} | max_err={result.max_error:.2e} | time={result.time_ms:.1f}ms"
...
def run_benchmark(bits_per_dim: int = 4, max_rank: int = 32) -> Dict[str, Any]:
    ...
    print(
        f"│   Complexity: O(L × r²) = O({d} × {r}²) = O({d * r * r:,})                   │"
    )
    print("├────────────────────────────────────────────────────────────────┤")
    ...
    print("├────────────────────────────────────────────────────────────────┤")
    print(
        f"│ Theoretical Speedup: {(N*N * d * r * r) / (d * r * r):,.0f}× (for pure contraction)         │"
    )
    print("└────────────────────────────────────────────────────────────────┘")
