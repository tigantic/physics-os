...
class Euler3DMMSSolution:
    ...
    def rho(
        ...
        return self.rho_0 + self.eps_rho * torch.sin(self.kx * x) * torch.sin(
            self.ky * y
        ) * torch.sin(self.kz * z) * math.cos(self.omega * t)
    ...
    def w(
        ...
        return self.w_0 + self.eps_w * torch.sin(self.kx * x) * torch.sin(
            self.ky * y
        ) * torch.cos(self.kz * z) * math.cos(self.omega * t)
...
class Euler3DMMS:
    ...
    def step_rk4(
        self, U: torch.Tensor, dt: float, mms: Euler3DMMSSolution, t: float
    ) -> torch.Tensor:
    ...
    def solve(
        self, mms: Euler3DMMSSolution, t_final: float, cfl: float = 0.3
    ) -> torch.Tensor:
        ...
        # Estimate stable dt
        u_max = max(
            abs(mms.u_0) + mms.eps_u, abs(mms.v_0) + mms.eps_v, abs(mms.w_0) + mms.eps_w
        )
...
class TestEuler3DMMS:
    ...
    def test_short_time_accuracy(self, euler3d_mms: Euler3DMMSSolution):
        ...
        assert (
            errors["L2_rho"] < 1e-2
        ), f"Short-time error too large: {errors['L2_rho']}"
    ...
    def test_spatial_convergence_order(self, euler3d_mms: Euler3DMMSSolution):
        ...
        assert (
            avg_rate > 0.3
        ), f"Convergence rate {avg_rate:.2f} below expected threshold"
