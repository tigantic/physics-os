...
class LidDrivenCavitySolver:
        ...
        for iteration in range(max_iter):
            ...
            for i in range(1, self.N-1):
                for j in range(1, self.N-1):
                    # Laplacian
                    lap_u = (u[i+1,j] + u[i-1,j] + u[i,j+1] + u[i,j-1] - 4*u[i,j]) / self.dx**2
                    lap_v = (v[i+1,j] + v[i-1,j] + v[i,j+1] + v[i,j-1] - 4*v[i,j]) / self.dx**2
                    
                    # Advection (upwind)
                    dudx = (u[i,j] - u[i-1,j]) / self.dx if u[i,j] > 0 else (u[i+1,j] - u[i,j]) / self.dx
                    dudy = (u[i,j] - u[i,j-1]) / self.dy if v[i,j] > 0 else (u[i,j+1] - u[i,j]) / self.dy
                    dvdx = (v[i,j] - v[i-1,j]) / self.dx if u[i,j] > 0 else (v[i+1,j] - v[i,j]) / self.dx
                    dvdy = (v[i,j] - v[i,j-1]) / self.dy if v[i,j] > 0 else (v[i,j+1] - v[i,j]) / self.dy
                    
                    # Update
                    u_new[i,j] = u[i,j] + dt * (self.nu * lap_u - u[i,j]*dudx - v[i,j]*dudy)
                    v_new[i,j] = v[i,j] + dt * (self.nu * lap_v - u[i,j]*dvdx - v[i,j]*dvdy)
            ...
            # Check convergence
            residual = max(
                (u - u_old).abs().max().item(),
                (v - v_old).abs().max().item()
            )
    ...
    def interpolate_to_ghia_points(
        ...
        f_u = interp1d(y_positions, u_centerline, kind='cubic', fill_value='extrapolate')
        u_at_ghia = f_u(GHIA_Y_POSITIONS)
        
        # Interpolate v to Ghia x-positions
        f_v = interp1d(x_positions, v_centerline, kind='cubic', fill_value='extrapolate')
...
class TestLidDrivenCavityBenchmark:
    ...
    def test_re100_u_centerline(self, solver_re100):
        ...
        # Extract centerlines
        y_pos, u_center, x_pos, v_center = solver_re100.extract_centerline_profiles(u, v)
        ...
        assert rms_error_norm < self.RMS_ERROR_TOL, \
