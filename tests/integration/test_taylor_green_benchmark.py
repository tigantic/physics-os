...
def taylor_green_kinetic_energy_exact(t: float, nu: float, A: float = 1.0, L: float = 2*math.pi) -> float:
    """
...
def taylor_green_enstrophy_exact(t: float, nu: float, A: float = 1.0, L: float = 2*math.pi) -> float:
    """
...
class TestTaylorGreenBenchmark:
    ...
    def solver_64(self) -> NS2DSolver:
        ...
        return NS2DSolver(
            Nx=64, Ny=64,
            Lx=self.DOMAIN_SIZE, Ly=self.DOMAIN_SIZE,
            nu=self.VISCOSITY,
            dtype=torch.float64,
            device='cpu'
        )
    ...
    def test_initial_condition_matches_exact(self, solver_64):
        ...
        u_exact, v_exact = taylor_green_exact(
            solver_64.X, solver_64.Y, 
            t=0.0, nu=self.VISCOSITY, A=self.AMPLITUDE
        )
        
        u_error = torch.abs(state0.u - u_exact).max().item()
        v_error = torch.abs(state0.v - v_exact).max().item()
        
        assert u_error < 1e-14, f"Initial u error {u_error:.2e} exceeds machine precision"
        assert v_error < 1e-14, f"Initial v error {v_error:.2e} exceeds machine precision"
    ...
    def test_velocity_decay_matches_analytical(self, solver_64):
        ...
        u_exact, v_exact = taylor_green_exact(
            solver_64.X, solver_64.Y,
            t=self.T_FINAL, nu=self.VISCOSITY, A=self.AMPLITUDE
        )
        ...
        assert max_rel_error < self.VELOCITY_ERROR_TOL, \
    ...
    def test_energy_decay_rate(self, solver_64):
        ...
        if result.diagnostics_history:
            ...
            ke_final = 0.5 * (result.final_state.u**2 + result.final_state.v**2).sum().item() * dx * dy
        
        # Expected decay
        ke_exact = taylor_green_kinetic_energy_exact(self.T_FINAL, self.VISCOSITY, self.AMPLITUDE)
        ...
        decay_error = abs(decay_factor_numerical - decay_factor_exact) / decay_factor_exact
        ...
        assert decay_error < self.ENERGY_DECAY_TOL, \
    ...
    def test_divergence_free(self, solver_64):
        ...
        div_u = compute_divergence_2d(
            result.final_state.u, result.final_state.v,
            solver_64.dx, solver_64.dy,
            method='spectral'
        )
        ...
        assert max_div < self.DIVERGENCE_TOL, \
...
class TestTaylorGreenConvergence:
    ...
    def test_spatial_convergence_order(self):
        ...
        for N in grids:
            ...
            u_exact, v_exact = taylor_green_exact(
                solver.X, solver.Y,
                t=self.T_FINAL, nu=self.VISCOSITY
            )
            ...
            if len(errors) > 1:
                ...
                print(f"{N:<10} {dx:<12.4e} {error:<14.6e} {ratio:<10.2f} {order:<10.2f}")
            else:
