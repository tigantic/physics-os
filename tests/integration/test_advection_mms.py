...
class AdvectionDiffusionSolver:
    ...
    def diffusion_rhs(self, u: torch.Tensor) -> torch.Tensor:
        ...
        d2u_dx2 = (torch.roll(u, -1, dims=0) - 2*u + torch.roll(u, 1, dims=0)) / self.dx**2
        d2u_dy2 = (torch.roll(u, -1, dims=1) - 2*u + torch.roll(u, 1, dims=1)) / self.dy**2
    ...
    def step_rk4(self, u: torch.Tensor, dt: float, mms: AdvectionMMSSolution, t: float) -> torch.Tensor:
        """RK4 time step."""
    ...
    def solve(self, mms: AdvectionMMSSolution, t_final: float, cfl: float = 0.4) -> torch.Tensor:
        ...
        dt_diff = cfl * min(self.dx, self.dy)**2 / (4 * self.D) if self.D > 0 else float('inf')
    ...
    def compute_error(self, u: torch.Tensor, mms: AdvectionMMSSolution, t: float) -> dict:
...
class TestAdvectionDiffusionMMS:
    ...
    def test_short_time_accuracy(self, advection_mms: AdvectionMMSSolution):
        ...
        solver = AdvectionDiffusionSolver(
            nx=64, ny=64,
            vx=advection_mms.vx, vy=advection_mms.vy,
            D=advection_mms.D
        )
        t_final = 0.01
...
if __name__ == "__main__":
    ...
    for n in grids:
        solver = AdvectionDiffusionSolver(
            nx=n, ny=n, vx=mms.vx, vy=mms.vy, D=mms.D
        )
