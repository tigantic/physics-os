...
class ShuOsherConfig:
    ...
    def initial_condition(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        u = torch.where(
            x < self.x_shock,
            torch.full_like(x, self.u_L),
            torch.full_like(x, self.u_R)
        )
        
        p = torch.where(
            x < self.x_shock,
            torch.full_like(x, self.p_L),
            torch.full_like(x, self.p_R)
        )
...
REFERENCE_X_POINTS = np.array([
    -3.0, -2.0, -1.0, 0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0
])
...
class ShuOsherSolver:
    ...
    def __init__(
        ...
        self.state = EulerState(
            rho=rho, rho_u=rho_u, E=E, gamma=self.config.gamma
        )
        self.t = 0.0
    ...
    def step(self, dt: float) -> None:
        ...
        for i in range(self.N + 1):
            ...
            F[i] = hllc_flux(U_L.unsqueeze(0), U_R.unsqueeze(0), self.config.gamma).squeeze(0)
    ...
    def solve(self, t_final: float = None, cfl: float = 0.5, verbose: bool = False) -> Dict[str, Any]:
        """
        Solve to final time.
...
class TestShuOsherBenchmark:
    ...
    def test_initial_condition_correct(self, solver_200, config):
        ...
        assert torch.allclose(rho_left, torch.full_like(rho_left, config.rho_L), atol=1e-6), \
...
def generate_validation_report() -> Dict[str, Any]:
    ...
    try:
        ...
        report["tests"].append({
            "name": "positivity",
            "status": "PASS" if positivity_pass else "FAIL"
        })
        ...
        report["overall_status"] = "PASS" if (positivity_pass and shock_pass) else "FAIL"
