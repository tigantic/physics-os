...
from tensornet.distributed.domain_decomp import (DecompType, DomainConfig,
                                                 DomainDecomposition,
                                                 SubdomainInfo)
...
class TestSingleDomain:
    ...
    def test_single_domain_no_neighbors(self):
        ...
        config = DomainConfig(
            nx=64, ny=64, n_procs=1, periodic_x=False, periodic_y=False
        )
        decomp = DomainDecomposition(config)
    ...
    def test_single_domain_periodic_self_neighbor(self):
        ...
        config = DomainConfig(nx=64, ny=64, n_procs=1, periodic_x=True, periodic_y=True)
        decomp = DomainDecomposition(config)
    ...
    def test_single_domain_no_ghost_zones_non_periodic(self):
        ...
        config = DomainConfig(
            nx=64, ny=64, n_procs=1, n_ghost=2, periodic_x=False, periodic_y=False
        )
        decomp = DomainDecomposition(config)
...
class TestBoundaryConditions:
    ...
    def test_periodic_x_neighbors(self):
        ...
        config = DomainConfig(
            nx=64, ny=64, n_procs=4, periodic_x=True, periodic_y=False
        )
        decomp = DomainDecomposition(config)
    ...
    def test_non_periodic_boundary_no_neighbor(self):
        ...
        config = DomainConfig(
            nx=64, ny=64, n_procs=4, periodic_x=False, periodic_y=False
        )
        decomp = DomainDecomposition(config)
        ...
        for rank in range(4):
            subdomain = decomp.get_subdomain(rank)
            if (
                subdomain.neighbor_left is None
                or subdomain.neighbor_right is None
                or subdomain.neighbor_bottom is None
                or subdomain.neighbor_top is None
            ):
...
class TestGhostZones:
    ...
    def test_interior_domain_full_ghost_zones(self):
        ...
        config = DomainConfig(nx=64, ny=64, n_procs=9, n_ghost=2)  # 3x3 grid
        decomp = DomainDecomposition(config)
    ...
    def test_ghost_zone_size_consistent(self):
        ...
        n_ghost = 3
        config = DomainConfig(nx=64, ny=64, n_procs=4, n_ghost=n_ghost)
        decomp = DomainDecomposition(config)
...
class TestDecompTypes:
    ...
    def test_slab_decomposition(self):
        ...
        config = DomainConfig(nx=64, ny=64, n_procs=4, decomp_type=DecompType.SLAB)
        decomp = DomainDecomposition(config)
    ...
    def test_block_decomposition(self):
        ...
        config = DomainConfig(nx=64, ny=64, n_procs=4, decomp_type=DecompType.BLOCK)
        decomp = DomainDecomposition(config)
...
class TestCoveragee:
    ...
    def test_all_cells_covered(self):
        ...
        for rank in range(4):
            subdomain = decomp.get_subdomain(rank)
            covered[
                subdomain.i_start : subdomain.i_end, subdomain.j_start : subdomain.j_end
            ] += 1
