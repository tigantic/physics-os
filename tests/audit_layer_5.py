...
def create_linear_vector_field_dense(N: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Create F(x,y,z) = (x, y, z)
...
def analytical_gradient_of_quadratic(N: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def fd_gradient_3d(f: torch.Tensor, dx: float = 1.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
...
def fd_divergence_3d(fx: torch.Tensor, fy: torch.Tensor, fz: torch.Tensor, dx: float = 1.0) -> torch.Tensor:
    """
...
def run_diffusion_test(bits: int, rank: int, device: torch.device) -> TestResult:
    ...
    xx, yy, zz = torch.meshgrid(coords, coords, coords, indexing='ij')
    sigma_init = 0.05
    f = torch.exp(-((xx - 0.5)**2 + (yy - 0.5)**2 + (zz - 0.5)**2) / (2 * sigma_init**2))
...
def run_qtt_operator_test(bits: int, rank: int, device: torch.device) -> TestResult:
    ...
    field = Field.create(dims=3, bits_per_dim=bits, rank=rank, init='random', device=device)
...
def run_full_audit(bits: int = 4, rank: int = 32) -> Dict[str, Any]:
    ...
    print(f"  {'✅ PASS' if result.passed else '❌ FAIL'} | max_err={result.max_error:.2e} | {result.details}")
