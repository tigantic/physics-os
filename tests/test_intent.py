...
class TestGoalDirector:
    ...
    def test_achieve_goal(self):
        ...
        director.register_action(
            "clamp",
            lambda x, lower=None, upper=None: np.clip(x, lower, upper)
        )
...
class TestIntentEngine:
    ...
    def test_execute_nl_query(self, velocity_field):
        ...
        assert result.status in [ResultStatus.SUCCESS, ResultStatus.PARTIAL, ResultStatus.FAILED]
    ...
    def test_execute_help(self):
        ...
        assert result.status == ResultStatus.SUCCESS
        assert "commands" in result.message.lower() or "available" in result.message.lower()
