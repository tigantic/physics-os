...
def test_intent_parser():
    ...
    assert result.intent_type == IntentType.QUERY_VALUE, f"Expected QUERY_VALUE, got {result.intent_type}"
    
    result = parser.parse("set pressure to 100")
    assert result.intent_type == IntentType.ACTION_SET, f"Expected ACTION_SET, got {result.intent_type}"
    
    result = parser.parse("minimize drag")
    assert result.intent_type == IntentType.ACTION_OPTIMIZE, f"Expected ACTION_OPTIMIZE, got {result.intent_type}"
...
def test_query_on_physics():
    ...
    from tensornet.intent import FieldQuery, Aggregator, Predicate
    ...
    assert abs(result.value - expected_max) < 0.1, f"Max: {result.value} != {expected_max}"
    
    # Query: mean of hot region (>340)
    query = FieldQuery("temperature").where(Predicate.gt(340)).aggregate(Aggregator.mean())
    result = query.execute(temperature)
...
def test_constraint_solver():
    ...
    from tensornet.intent import ConstraintSet, BoundConstraint, ConstraintSolver
...
def test_intent_engine_integration():
    ...
    from tensornet.intent import IntentEngine, FieldQuery, Aggregator, ResultStatus
