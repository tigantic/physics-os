...
class TestMultiGPUTN:
    def test_compute_partition_contiguous(self):
        from tensornet.engine.gpu.multi_gpu_tn import compute_partition, PartitionStrategy
    ...
    def test_distributed_mps_construct(self):
        from tensornet.engine.gpu.multi_gpu_tn import DistributedMPS, MultiGPUConfig
    ...
    def test_halo_exchange(self):
        from tensornet.engine.gpu.multi_gpu_tn import DistributedMPS, MultiGPUConfig, halo_exchange
        cfg = MultiGPUConfig(n_devices=2)
    ...
    def test_distributed_expectation(self):
        from tensornet.engine.gpu.multi_gpu_tn import DistributedMPS, MultiGPUConfig, distributed_expectation
...
class TestROCmHIP:
    def test_import_and_registry(self):
        from tensornet.engine.hardware import BackendKind
        import tensornet.engine.hardware.rocm_hip  # noqa: F401
        assert BackendKind.ROCM is not None

    def test_hip_tensor(self):
        from tensornet.engine.hardware.rocm_hip import HIPTensor, ROCmBackend
...
class TestOneAPI:
    def test_import(self):
        import tensornet.engine.hardware.oneapi_sycl  # noqa: F401

    def test_xpu_tensor(self):
        from tensornet.engine.hardware.oneapi_sycl import XPUTensor
        arr = np.eye(3)
...
class TestMetalMPS:
    def test_import(self):
        import tensornet.engine.hardware.metal_mps  # noqa: F401

    def test_mps_tensor(self):
        from tensornet.engine.hardware.metal_mps import MPSTensor
...
class TestFPGA:
    def test_fixed_point_format(self):
        from tensornet.engine.hardware.fpga import FixedPointFormat
    ...
    def test_fixed_point_array(self):
        from tensornet.engine.hardware.fpga import FixedPointArray, FixedPointFormat
    ...
    def test_fixed_matmul(self):
        from tensornet.engine.hardware.fpga import fixed_matmul, FixedPointArray, FixedPointFormat
    ...
    def test_tmr_vote(self):
        from tensornet.engine.hardware.fpga import tmr_vote
        a = np.array([1.0, 2.0, 3.0])
        result = tmr_vote(a, a, a)
        np.testing.assert_array_equal(result, a)

    def test_bitstream_registry(self):
        from tensornet.engine.hardware.fpga import Bitstream, BitstreamRegistry
...
class TestNeuromorphic:
    def test_lif_simulate(self):
        from tensornet.engine.hardware.neuromorphic import LIFParams, lif_simulate
        params = LIFParams()
    ...
    def test_rate_encode_decode(self):
        from tensornet.engine.hardware.neuromorphic import rate_encode, rate_decode
    ...
    def test_energy_profile(self):
        from tensornet.engine.hardware.neuromorphic import EnergyProfile
...
class TestPhotonic:
    def test_clements_roundtrip(self):
        from tensornet.engine.hardware.photonic import _clements_decompose, _clements_reconstruct
        n = 4
    ...
    def test_matrix_to_photonic(self):
        from tensornet.engine.hardware.photonic import matrix_to_photonic
    ...
    def test_photonic_backend_throughput(self):
        from tensornet.engine.hardware.photonic import PhotonicBackend
...
class TestQuantumBackend:
    def test_statevector_h_gate(self):
        from tensornet.engine.hardware.quantum_backend import apply_single_qubit, H_GATE
    ...
    def test_vqe_via_backend(self):
        from tensornet.engine.hardware.quantum_backend import QuantumBackend, VQEConfig
    ...
    def test_amplitude_encode(self):
        from tensornet.engine.hardware.quantum_backend import amplitude_encode
    ...
    def test_state_to_tt(self):
        from tensornet.engine.hardware.quantum_backend import state_to_tt
...
class TestMixedPrecision:
    def test_precision_policy(self):
        from tensornet.engine.gpu.mixed_precision import PrecisionPolicy, Precision, OpCategory
    ...
    def test_mixed_precision_svd(self):
        from tensornet.engine.gpu.mixed_precision import mixed_precision_svd, Precision
    ...
    def test_conservation_monitor(self):
        from tensornet.engine.gpu.mixed_precision import ConservationMonitor
...
class TestBlackwellOpt:
    def test_nvarch_enum(self):
        from tensornet.engine.gpu.blackwell_opt import NVArch
        assert NVArch.BLACKWELL is not None

    def test_fp8_matmul_fallback(self):
        from tensornet.engine.gpu.blackwell_opt import fp8_matmul, FP8Config
    ...
    def test_select_kernel(self):
        from tensornet.engine.gpu.blackwell_opt import select_kernel, GPUArchInfo, NVArch, KernelChoice
...
class TestARMSVE:
    def test_sve_tiled_matmul(self):
        from tensornet.engine.hardware.arm_sve import sve_tiled_matmul
    ...
    def test_qtt_evaluate(self):
        from tensornet.engine.hardware.arm_sve import qtt_evaluate_sve
...
class TestNVLinkTopology:
    def test_ring_schedule(self):
        from tensornet.engine.gpu.nvlink_topology import ring_schedule, CommSchedule
    ...
    def test_tree_schedule(self):
        from tensornet.engine.gpu.nvlink_topology import tree_schedule, CommSchedule
        sched = tree_schedule(8)
        assert isinstance(sched, CommSchedule)
        assert len(sched.steps) > 0

    def test_select_collective(self):
        from tensornet.engine.gpu.nvlink_topology import select_collective, GPUTopology, CollectiveAlgo
...
class TestPersistentKernel:
    def test_persistent_solve_jacobi(self):
        from tensornet.engine.gpu.persistent_kernel import persistent_solve, jacobi_iterate, l2_residual, PersistentConfig
        n = 2
    ...
    def test_batched_persistent(self):
        from tensornet.engine.gpu.persistent_kernel import batched_persistent_solve, PersistentConfig
        n = 2
...
class TestHILRealtime:
    def test_circular_buffer(self):
        from tensornet.engine.gpu.hil_realtime import CircularBuffer
    ...
    def test_timing_budget(self):
        from tensornet.engine.gpu.hil_realtime import TimingBudget
        tb = TimingBudget()
    ...
    def test_hil_controller_single_tick(self):
        from tensornet.engine.gpu.hil_realtime import HILController, HILConfig
    ...
    def test_telemetry_jitter(self):
        from tensornet.engine.gpu.hil_realtime import TelemetryRing, TelemetryEntry
        ring = TelemetryRing()
        for i in range(10):
...
class TestTensorCore:
    def test_wmma_tiles(self):
        from tensornet.engine.gpu.tensor_core import WMMA_TILES, TCPrecision
        tile = WMMA_TILES[TCPrecision.FP16_FP32]
        assert tile.m == 16 and tile.n == 16 and tile.k == 16

    def test_fragment_mma(self):
        from tensornet.engine.gpu.tensor_core import Fragment, wmma_mma, WMMA_TILES, TCPrecision
        tile = WMMA_TILES[TCPrecision.FP16_FP32]
    ...
    def test_tc_gemm_cpu(self):
        from tensornet.engine.gpu.tensor_core import tc_gemm, TCGEMMConfig, TCPrecision
    ...
    def test_tt_contract_tc(self):
        from tensornet.engine.gpu.tensor_core import tt_contract_tc, TCPrecision
    ...
    def test_throughput_estimate(self):
        from tensornet.engine.gpu.tensor_core import estimate_throughput, TCPrecision
    ...
    def test_fusion_analysis(self):
        from tensornet.engine.gpu.tensor_core import analyze_fusion
...
class TestManagedMemory:
    def test_mmap_tensor_basic(self):
        from tensornet.engine.gpu.managed_memory import MMapTensor
    ...
    def test_page_table(self):
        from tensornet.engine.gpu.managed_memory import PageTable, MemLocation
    ...
    def test_access_tracker_predict(self):
        from tensornet.engine.gpu.managed_memory import AccessTracker
    ...
    def test_managed_tensor_backend(self):
        from tensornet.engine.gpu.managed_memory import ManagedTensor
    ...
    def test_tt_core_stream(self):
        from tensornet.engine.gpu.managed_memory import TTCoreStream, ooc_tt_contract
        with tempfile.TemporaryDirectory() as d:
...
class TestHardwareRegistry:
    ...
    def test_available_backends(self):
        from tensornet.engine.hardware import available_backends
        backends = available_backends()
        assert isinstance(backends, list)

    def test_best_backend_returns_cpu(self):
        from tensornet.engine.hardware import best_backend
        result = best_backend()
