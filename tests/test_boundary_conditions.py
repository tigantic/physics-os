...
def apply_robin_1d(
    ...
    if abs(alpha_right + beta_right / dx) > 1e-10:
        u[-1] = (gamma_right + beta_right * u[-2] / dx) / (alpha_right + beta_right / dx)
...
class TestNeumannBC:
    ...
    def test_neumann_1d(self, deterministic_seed, bc_params):
        ...
        u = apply_neumann_1d(u, left_gradient=0.0, right_gradient=0.0, dx=bc_params['dx'])
...
class TestRobinBC:
    ...
    def test_robin_reduces_to_dirichlet(self, deterministic_seed, bc_params):
        ...
        u = apply_robin_1d(
            u, 
            alpha_left=1.0, beta_left=0.0, gamma_left=2.0,
            alpha_right=1.0, beta_right=0.0, gamma_right=3.0,
            dx=bc_params['dx']
        )
    ...
    def test_robin_reduces_to_neumann(self, deterministic_seed, bc_params):
        ...
        u = apply_robin_1d(
            u,
            alpha_left=0.0, beta_left=1.0, gamma_left=0.0,  # Zero gradient
            alpha_right=0.0, beta_right=1.0, gamma_right=0.0,
            dx=dx
        )
