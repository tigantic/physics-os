...
class TestLBM:
    def test_import_and_config(self):
        from ontic.cfd.lbm import LatticeModel, CollisionModel, LBMState
        assert LatticeModel.D2Q9 is not None
        assert CollisionModel.BGK is not None

    def test_solver_exists(self):
        from ontic.cfd.lbm import LatticeBoltzmannSolver
...
class TestSPH:
    def test_import(self):
        from ontic.cfd.sph import KernelType, SPHState, SPHSolver, TaitEOS

    def test_tait_eos(self):
        from ontic.cfd.sph import TaitEOS
...
class TestDG:
    def test_import(self):
        from ontic.cfd.dg import DGSolver1D, DGState, FluxType

    def test_gauss_legendre(self):
        from ontic.cfd.dg import gauss_legendre
...
class TestSEM:
    def test_import(self):
        from ontic.cfd.sem import SEMSolver1D, SEMState

    def test_gll_points(self):
        from ontic.cfd.sem import gll_points
        x, w = gll_points(6)
        assert len(x) == 6
...
class TestSpaceTimeDG:
    def test_import(self):
        from ontic.cfd.space_time_dg import SpaceTimeDGSolver, STDGState


# ── Peridynamics ───────────────────────────────────────────────────
class TestPeridynamics:
    def test_import(self):
        from ontic.materials.mechanics.peridynamics import (
            PeridynamicsModel,
            PeridynamicsSolver,
            Material,
            PeridynamicState,
...
class TestMPM:
    def test_import(self):
        from ontic.materials.mechanics.mpm import (
            MPMSolver,
            MPMParticleState,
            ShapeFunction,
            ConstitutiveModel,
...
class TestPFC:
    def test_import(self):
        from ontic.fluids.phase_field.pfc import PFCState, PFCSolver


# ── XFEM ────────────────────────────────────────────────────────────
class TestXFEM:
    def test_import(self):
        from ontic.materials.mechanics.xfem import XFEMSolver, XFEMState, CrackGeometry

    def test_heaviside(self):
        from ontic.materials.mechanics.xfem import heaviside
...
class TestIGA:
    def test_import(self):
        from ontic.materials.mechanics.iga import IGASolver1D, IGAState

    def test_bspline_partition_of_unity(self):
        from ontic.materials.mechanics.iga import basis_funs, find_span
...
class TestVEM:
    def test_import(self):
        from ontic.materials.mechanics.vem import VEMSolver, VEMState


# ── Mimetic FD ──────────────────────────────────────────────────────
class TestMimeticFD:
    def test_import(self):
        from ontic.materials.mechanics.mimetic import (
            MimeticFDSolver,
            MFDMesh2D,
    ...
    def test_mesh_build(self):
        from ontic.materials.mechanics.mimetic import build_cartesian_mfd_mesh
...
class TestHHO:
    def test_import(self):
        from ontic.materials.mechanics.hho import HHOSolver, HHOState, HHOMesh2D


# ── LES/ILES ────────────────────────────────────────────────────────
class TestILES:
    def test_iles_mode_exists(self):
        from ontic.cfd.les import LESModel
