...
from tensornet.cfd.qtt_eval import (
    ...
    dense_to_qtt_cores,
    qtt_to_dense,
    verify_qtt_evaluation,
    QTTContiguous,
    qtt_eval_multi_field_batch,
)
from tensornet.cfd.tci_flux import (
    rusanov_flux,
...
def test_rusanov_flux_sod():
    ...
    F_rho, F_rho_u, F_E = rusanov_flux(
        rho_L, rho_u_L, E_L,
        rho_R, rho_u_R, E_R,
        gamma
    )
    ...
    print(f"  Fluxes: F_ρ={F_rho.item():.4f}, F_ρu={F_rho_u.item():.4f}, F_E={F_E.item():.4f}")
    ...
    assert abs(F_rho.item() - expected_F_rho) < 1e-6, f"F_rho mismatch: {F_rho.item()} vs {expected_F_rho}"
...
def run_all_tests():
    ...
    print("  2. python -c 'from tci_core import TCISampler; print(\"Rust TCI loaded!\")'")
    print()
