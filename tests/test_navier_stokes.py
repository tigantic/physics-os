...
def divergence_2d(
    u: torch.Tensor, v: torch.Tensor, dx: float, dy: float
) -> torch.Tensor:
...
def curl_2d(u: torch.Tensor, v: torch.Tensor, dx: float, dy: float) -> torch.Tensor:
...
class TestFiniteDifferenceOperators:
    ...
    def test_central_diff_linear(self, deterministic_seed):
        ...
        assert torch.allclose(
            dfdx[2:-2, 2:-2], torch.ones(nx - 4, ny - 4, dtype=torch.float64), atol=0.01
        )
