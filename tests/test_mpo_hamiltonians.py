...
def create_mpo_heisenberg(
    n_sites: int, J: float = 1.0, h: float = 0.0, dtype=torch.float64
) -> List[torch.Tensor]:
...
def create_identity_mpo(
    n_sites: int, phys_dim: int = 2, dtype=torch.float64
) -> List[torch.Tensor]:
    """Create identity MPO."""
...
class TestPauliMatrices:
    ...
    def test_pauli_z_properties(self, deterministic_seed):
        ...
        assert torch.allclose(
            eigvals.sort()[0], torch.tensor([-1.0, 1.0], dtype=torch.float64)
        )
...
class TestSpinOperators:
    ...
    def test_spin_half_sz(self, deterministic_seed):
        ...
        assert torch.allclose(
            eigvals.sort()[0], torch.tensor([-0.5, 0.5], dtype=torch.float64)
        )
...
class TestMPOMPSApplication:
    ...
    def test_mpo_mps_contraction_shape(self, deterministic_seed):
        ...
        mps = [
            torch.randn(
                1 if i == 0 else mps_bond,
                phys_dim,
                1 if i == n_sites - 1 else mps_bond,
                dtype=torch.float64,
            )
            for i in range(n_sites)
        ]
        ...
        mpo = [
            torch.randn(
                1 if i == 0 else mpo_bond,
                phys_dim,
                phys_dim,
                1 if i == n_sites - 1 else mpo_bond,
                dtype=torch.float64,
            )
            for i in range(n_sites)
        ]
...
class TestExpectationValues:
    ...
    def test_identity_expectation(self, deterministic_seed):
        ...
        mps = [
            torch.tensor([[[1.0, 0.0]]], dtype=torch.float64).permute(0, 2, 1)
        ]  # |0>
