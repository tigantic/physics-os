...
def full_grid_shock_demo():
    ...
    for i, (node, score) in enumerate(critical):
        role = ["Core Router", "Distribution Hub", "Edge Gateway", 
                "Backup Link", "Secondary Path"][i]
...
def run_cascade_threshold_study():
    ...
    for rate in [100, 200, 300, 400, 500, 750, 1000]:
        ...
        print(f"  Attack Rate {rate:4d}: {sim.cascade_report.failed_nodes:2d} failures - {cascade}")
    ...
    for i, (rate, failed, status) in enumerate(results):
        if status == "CASCADE":
            print(f"Cascade threshold: ~{results[i-1][0] if i > 0 else rate} - {rate} units/step")
            break
