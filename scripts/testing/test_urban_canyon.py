...
def scan_city_safety():
    ...
    print(
        f"   Domain: {city.physical_size[0]:.0f}m x "
    ...
    print(
        f"   🟢 GREEN (Safe):      {report.safe_volume:>8,} voxels ({safe_pct:>5.1f}%)"
    )
    print(
        f"   🟡 YELLOW (Caution):  {report.caution_volume:>8,} voxels ({caution_pct:>5.1f}%)"
    )
    print(
        f"   🔴 RED (Danger):      {report.danger_volume:>8,} voxels ({danger_pct:>5.1f}%)"
    )
    print()
    ...
    # Find safe altitude
    print(
        f"   Minimum safe altitude: {report.safe_altitude_min * city.voxel_size:.0f}m AGL"
    )
    ...
    print(
        f"   Safe voxels at {mid_height * city.voxel_size:.0f}m altitude: {connected_safe}"
    )
    print()
...
def run_multi_scenario():
    ...
    for name, speed in scenarios:
        ...
        print(
            f"  {name:<15} | {speed:>5.1f} m/s | {report.max_updraft:>5.1f} m/s | {safe_pct:>5.1f}%  | {status}"
        )
...
def run_building_height_study():
    ...
    for max_h in [10, 20, 30, 40, 50]:
        ...
        print(
            f"  {height_m:>5.0f}m       | {report.max_updraft:>6.1f} m/s | {safe_pct:>6.1f}%   | {rec}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multi", action="store_true", help="Run multi-scenario analysis"
    )
    parser.add_argument(
        "--heights", action="store_true", help="Run building height study"
    )
