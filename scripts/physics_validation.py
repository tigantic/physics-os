...
def validate_rankine_hugoniot() -> ValidationResult:
    ...
    try:
        ...
        # Exact solutions
        M2_exact = np.sqrt((1 + (gamma - 1) / 2 * M1**2) / 
...
def main():
    ...
    for result in results:
        ...
        print(f"       {result.metric}: {result.value:.6g} (threshold: {result.threshold})")
    ...
    if args.json:
        ...
        output_path = Path(__file__).parent.parent / 'artifacts' / 'physics_validation.json'
