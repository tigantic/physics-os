...
def run_bandit(project_dir: Path) -> Tuple[bool, List[Dict[str, Any]]]:
    ...
    try:
        ...
        high_issues = [i for i in issues if i.get('issue_severity', '').upper() in ('HIGH', 'MEDIUM')]
...
def main():
    ...
    high_issues = [i for i in bandit_issues if i.get('issue_severity', '').upper() in ('HIGH', 'MEDIUM')]
