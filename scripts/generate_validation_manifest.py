...
def main():
    ...
    parser.add_argument('--output', type=Path, default=Path('validation_manifest.json'), help='Output file')
    ...
    for cat, results in manifest['validation'].items():
        status_icon = '✅' if results['status'] == 'PASS' else '❌' if results['status'] == 'FAIL' else '❓'
        print(f"  {cat}: {status_icon} {results['passed']} passed, {results['failed']} failed")
