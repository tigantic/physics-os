...
def profile_euler_solver():
    ...
    for N in sizes:
        ...
        def make_sod_state():
            x = torch.linspace(0, 1, N, dtype=torch.float64)
            rho = torch.where(x < 0.5, torch.ones(N, dtype=torch.float64), torch.full((N,), 0.125, dtype=torch.float64))
            u = torch.zeros(N, dtype=torch.float64)
            p = torch.where(x < 0.5, torch.ones(N, dtype=torch.float64), torch.full((N,), 0.1, dtype=torch.float64))
            rho_u = rho * u
...
def generate_report(all_results: Dict) -> str:
    ...
    if 'svd' in all_results:
        ...
        for shape, metrics in all_results['svd'].items():
            lines.append(f"  {shape:12s}: {metrics['time_ms']:8.3f} ms, {metrics['gflops']:.2f} GFLOPS")
        lines.append("")
    ...
    if 'weno' in all_results:
        ...
        for size, metrics in all_results['weno'].items():
            lines.append(f"  {size:12s}: {metrics['time_ms']:8.3f} ms, {metrics['points_per_sec']/1e6:.2f}M pts/s")
        lines.append("")
    ...
    if 'euler' in all_results:
        ...
        for size, metrics in all_results['euler'].items():
            lines.append(f"  {size:12s}: {metrics['step_ms']:8.3f} ms/step, {metrics['cells_per_sec']/1e6:.2f}M cells/s")
        lines.append("")
    ...
    if 'mps' in all_results:
        ...
        for config, metrics in sorted(all_results['mps'].items()):
            lines.append(f"  {config:20s}: create={metrics['creation_ms']:.2f}ms, roundtrip={metrics['roundtrip_ms']:.2f}ms")
        lines.append("")
