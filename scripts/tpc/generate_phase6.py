...
Generates TPC certificates for all 25 Tier 2A domains.
    ...
    python scripts/tpc/generate_phase6.py --domain all
    python scripts/tpc/generate_phase6.py --domain turbulence
    python scripts/tpc/generate_phase6.py --domain ideal_mhd --output my_cert.tpc
...
def _run_shallow_water() -> dict[str, Any]:
    ...
    try:
        from tensornet.geophysics.oceanography import ShallowWaterEquations
...
def _run_non_equilibrium() -> dict[str, Any]:
    from tensornet.statmech.trace_adapters.non_equilibrium_adapter import NonEquilibriumTraceAdapter

    try:
        from tensornet.statmech.kinetics import GillespieSSA
        ssa = GillespieSSA(
            species=["A", "B", "C"],
            reactions=[
...
def _run_md() -> dict[str, Any]:
    from tensornet.statmech.trace_adapters.md_adapter import MDTraceAdapter

    try:
        from tensornet.md.engine import MDSimulation
...
def _run_lattice_spin() -> dict[str, Any]:
    from tensornet.statmech.trace_adapters.lattice_spin_adapter import LatticeSpinTraceAdapter
...
def _run_ideal_mhd() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.ideal_mhd_adapter import IdealMHDTraceAdapter
...
def _run_resistive_mhd() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.resistive_mhd_adapter import ResistiveMHDTraceAdapter
    from tensornet.plasma.extended_mhd import HallMHDSolver1D
...
def _run_gyrokinetics() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.gyrokinetics_adapter import GyrokineticsTraceAdapter
...
def _run_reconnection() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.reconnection_adapter import ReconnectionTraceAdapter
    from tensornet.plasma.magnetic_reconnection import SweetParkerReconnection
...
def _run_laser_plasma() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.laser_plasma_adapter import LaserPlasmaTraceAdapter
    from tensornet.plasma.laser_plasma import LaserPlasmaParams, StimulatedRamanScattering
...
def _run_dusty_plasma() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.dusty_plasma_adapter import DustyPlasmaTraceAdapter
    from tensornet.plasma.dusty_plasmas import DustyPlasmaParams
...
def _run_space_plasma() -> dict[str, Any]:
    from tensornet.plasma.trace_adapters.space_plasma_adapter import SpacePlasmaTraceAdapter
