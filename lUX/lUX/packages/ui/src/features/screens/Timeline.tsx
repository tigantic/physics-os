    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {proof.timeline.steps.map((s) => (
          <div
            key={s.step_index}
            className="grid grid-cols-1 gap-1 rounded-[var(--radius-inner)] border bg-[var(--color-bg-surface)] px-3 py-2 md:grid-cols-12 md:gap-2"
          >
            <div className="font-mono text-xs text-[var(--color-text-tertiary)] md:col-span-2">Step {s.step_index}</div>
            <div className="hidden truncate font-mono text-xs text-[var(--color-text-tertiary)] md:col-span-4 md:block">
              {s.state_hash}
            </div>
