...
class FluidState:
    ...
    def __init__(self, grid_bits: int = 30, rank: int = 8, viscosity: float = 0.01):
        ...
        print(f"  Grid: {self.grid_size:,} x {self.grid_size:,} = {self.total_points:,} points")
        print(f"  Cores: {self.n_cores}")
        print(f"  Rank: {rank}")
        print(f"  QTT Memory: {self.qtt_memory_bytes:,} bytes ({self.qtt_memory_bytes/1024:.1f} KB)")
