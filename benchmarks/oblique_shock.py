...
def run_oblique_shock_benchmark(
    ...
    if verbose:
        ...
        print(f"  Pressure ratio: {p_ratio_sim:.4f} (exact: {exact['p2_p1']:.4f}, error: {p_error:.2f}%)")
        print(f"  Density ratio:  {rho_ratio_sim:.4f} (exact: {exact['rho2_rho1']:.4f}, error: {rho_error:.2f}%)")
        print(f"  Downstream Mach: {M_2:.4f} (exact: {exact['M2']:.4f}, error: {M_error:.2f}%)")
        print()
...
def parameter_study(
    ...
    for M in Mach_numbers:
        ...
        if verbose:
            print(f"{M:6.1f} {math.degrees(exact['beta']):8.2f} "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Oblique Shock Validation Benchmark"
    )
