...
def benchmark_dense(N: int, n_steps: int = 10):
    ...
    # Warmup
    _ = rusanov_flux(rho, rhou, E, torch.roll(rho, -1), torch.roll(rhou, -1), torch.roll(E, -1), gamma)
...
def benchmark_hybrid(N: int, n_qubits: int, max_rank: int = 64, n_steps: int = 5):
    ...
    for _ in range(n_steps):
        ...
        F_rho, F_rhou, F_E = rusanov_flux(
            rho_d, rhou_d, E_d,
            torch.roll(rho_d, -1), torch.roll(rhou_d, -1), torch.roll(E_d, -1),
            gamma
        )
...
def main():
    ...
    for n_qubits in [10, 12, 14]:
        ...
        print(f"  TCI:    {r_tci['time_per_step']*1000:.2f} ms/step ({r_tci['evals_per_step']:.0f} evals, {r_tci['compression']:.1f}x compression)")
