...
def compute_separation_metrics(
    ...
    return {
        ...
        'bubble_length_normalized': bubble_length / setup['delta_99'] if setup['delta_99'] > 0 else 0,
    }


def compute_wall_distributions(
    state: Euler2DState,
    setup: dict
) -> dict:
    """
...
def run_sbli_simulation(
    ...
    while t < t_final:
        ...
        if verbose and step % 100 == 0:
            M_max = torch.sqrt(state.u**2 + state.v**2).max() / \
                    torch.sqrt(setup['config'].gamma * state.p / state.rho).max()
            print(f"Step {step:5d}, t = {t:.2e} s, dt = {dt:.2e} s, M_max = {M_max:.2f}")
...
def plot_sbli_results(results: dict, save_path: str = None):
    ...
    ax = axes[0, 0]
    M = torch.sqrt(state.u**2 + state.v**2) / torch.sqrt(config.gamma * state.p / state.rho)
    ...
    if sep['separated'] and sep['x_sep'] is not None:
        ...
        if sep['x_reat'] is not None:
            axes[1, 1].axvline(x=sep['x_reat'], color='c', linestyle=':', label='Reattachment')
        axes[1, 1].legend()
