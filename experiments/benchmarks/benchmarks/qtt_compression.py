...
def benchmark_oblique_shock(
    ...
    analysis = compression_analysis(
        state, chi_values=[4, 8, 16, 32, 64], verbose=verbose
    )
    ...
    if verbose:
        ...
        print(
            f"Scaling behavior: {scaling['interpretation']} (slope={scaling['slope']:.3f})"
        )
        print(f"Status: {'✓ PASS' if results['pass'] else '✗ FAIL'}")
...
def benchmark_smooth_function(Nx: int = 256, verbose: bool = True) -> dict:
    ...
    for chi in chi_values:
        ...
        if verbose:
            print(
                f"χ={chi:3d}: CR={result.compression_ratio:8.2f}x, error={error.item():.2e}"
            )
...
def benchmark_resolution_scaling(
    ...
    if verbose:
        ...
        print(
            f"Interpretation: {'Sublinear (efficient)' if scaling_exponent < 0.5 else 'Linear or worse'}"
        )
        print(f"Status: {'✓ PASS' if results['pass'] else '✗ FAIL'}")
...
def run_all_benchmarks(verbose: bool = True, save_results: bool = True) -> dict:
    ...
    results["benchmarks"]["smooth_function"] = benchmark_smooth_function(
        verbose=verbose
    )
    results["benchmarks"]["resolution_scaling"] = benchmark_resolution_scaling(
        verbose=verbose
    )
