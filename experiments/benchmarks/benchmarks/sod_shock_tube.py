...
def convergence_study(
    ...
    for r in results:
        print(
            f"{r['N']:<8} {r['dx']:<12.4e} {r['L1_rho']:<12.4e} {r['L1_u']:<12.4e} {r['L1_p']:<12.4e}"
        )
    ...
    for i in range(1, len(results)):
        rate_rho = (
            torch.log(torch.tensor(results[i - 1]["L1_rho"] / results[i]["L1_rho"]))
            / torch.log(torch.tensor(2.0))
        ).item()
        rate_u = (
            torch.log(
                torch.tensor(results[i - 1]["L1_u"] / (results[i]["L1_u"] + 1e-15))
            )
            / torch.log(torch.tensor(2.0))
        ).item()
        rate_p = (
            torch.log(torch.tensor(results[i - 1]["L1_p"] / results[i]["L1_p"]))
            / torch.log(torch.tensor(2.0))
        ).item()
