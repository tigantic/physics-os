...
def validate_sod():
    ...
    if RUST_AVAILABLE and qtt_rust_time is not None:
        ...
        results["speedup_rust_vs_python"] = (
            qtt_time / qtt_rust_time if qtt_rust_time > 0 else 0
        )
        results["eval_reduction_rust"] = (
            total_evals / total_evals_rust if total_evals_rust > 0 else 0
        )
