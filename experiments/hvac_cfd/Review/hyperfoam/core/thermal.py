...
class ThermalMultiPhysicsSolver:
    ...
    def __init__(
        ...
        # ─────────────────────────────────────────────────────────────────
        # SDF-based wall enhancement factor
        # ─────────────────────────────────────────────────────────────────
        # Near walls, heat transfer is enhanced due to boundary layer effects
        # This factor increases thermal diffusivity near walls
        # Reference: Incropera & DeWitt, "Fundamentals of Heat and Mass Transfer"
        self.sdf = grid.sdf
        wall_enhancement_scale = 0.1  # meters (enhance within 10cm of walls)
        wall_proximity = torch.clamp(self.sdf, 0, 1e6) / wall_enhancement_scale
        # Enhancement factor: 2.0 at wall, 1.0 far from wall (smooth blend)
        self.wall_heat_factor = 1.0 + torch.exp(-wall_proximity)
        # Clamp to valid SDF regions only
        self.wall_heat_factor = torch.where(
            self.sdf < 1e6,
            self.wall_heat_factor,
            torch.ones_like(self.wall_heat_factor)
        )
