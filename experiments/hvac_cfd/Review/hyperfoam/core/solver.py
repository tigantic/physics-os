...
# Fix torch.compile breaking on paths with spaces
# Redirect inductor cache to secure temp directory (no spaces)
_temp_base = tempfile.gettempdir()
os.environ.setdefault("TORCHINDUCTOR_CACHE_DIR", os.path.join(_temp_base, "torchinductor_cache"))
os.environ.setdefault("TORCH_EXTENSIONS_DIR", os.path.join(_temp_base, "torch_extensions"))


# Numerical stability constants
CG_EPSILON: float = 1e-12       # Division-by-zero guard in CG solver
CLAMP_BOUND: float = 1e6        # Bounds for pressure/divergence clamping
PRESSURE_GRAD_MAX: float = 100.0  # Maximum pressure gradient correction
FLUID_THRESHOLD: float = 0.01  # Minimum vol_frac to consider as fluid
...
def _apply_laplacian(p, area_x, area_y, area_z, idx2, idy2, idz2):
...
def _cg_iterations(x, b, mask, area_x, area_y, area_z, 
    ...
    """
    # Initial residual
    Ax = _apply_laplacian(x, area_x, area_y, area_z, idx2, idy2, idz2) * mask
    r = (b - Ax) * mask
    p = r.clone()
    rsold = torch.sum(r * r)
    ...
    for _ in range(n_iter):
        Ap = _apply_laplacian(p, area_x, area_y, area_z, idx2, idy2, idz2) * mask
...
# Disable torch.compile for now - C++ linker fails on paths with spaces
# TODO: Re-enable when running from a path without spaces
_COMPILED_CG_KERNEL = _cg_iterations
...
class GeometricPressureSolver:
    ...
    def __init__(self, grid, dt, n_iter: int = 20):
        ...
        self.mask = (grid.geo[0] > FLUID_THRESHOLD).float()
        
        # Use module-level compiled kernel
        self._cg_kernel = _COMPILED_CG_KERNEL
        self._compiled = False
    
    def solve(self, divergence, p_init=None):
        """Solve pressure Poisson equation.
        
        Args:
            divergence: Divergence field to solve for.
            p_init: Initial pressure guess for warm-start (uses zeros if None).
        """
        # Initialize pressure guess
        if p_init is None:
            x = torch.zeros_like(divergence)
        else:
            x = p_init
        
        # Call CG kernel: _cg_iterations(x, b, mask, area_x, area_y, area_z, idx2, idy2, idz2, n_iter)
...
class HyperFoamSolver:
    ...
    # Maximum velocity magnitude for stability clamping
    ...
    def __init__(self, grid, config):
        ...
        # Geometry access via HyperGrid API (not raw tensor indices)
        # This ensures consistent interpretation of geometry channels
        self.vol_frac = grid.vol_frac  # Channel 0: volume fraction
        self.fluid_mask = (self.vol_frac > FLUID_THRESHOLD).float()

        # Cache area fractions for flux computation
        # These are face porosities that modulate advective fluxes
        self.area_x = grid.area_x  # Channel 1: X-face openings
        self.area_y = grid.area_y  # Channel 2: Y-face openings
        self.area_z = grid.area_z  # Channel 3: Z-face openings

        # SDF for wall-distance-based treatments (near-wall physics)
        self.sdf = grid.sdf  # Channel 4: signed distance field
    ...
    def step(self):
        ...
        # --- 0. Geometry (cached from HyperGrid) ---
        area_x, area_y, area_z = self.area_x, self.area_y, self.area_z
        ...
        # Van Driest-style damping: reduce effective viscosity near walls
        # This mimics the viscous sublayer without resolving it explicitly
        # Reference: Van Driest (1956), y+ scaling for turbulent BL
        wall_dist = self.sdf
        # Characteristic length scale for damping (cells within ~5cm of walls)
        wall_damping_scale = 0.05  # meters
        # Damping factor: 1 far from walls, 0 at walls
        # Uses smooth tanh profile to avoid discontinuities
        wall_damping = torch.tanh(torch.clamp(wall_dist, 0, 1) / wall_damping_scale)
        wall_damping = torch.where(wall_dist < 1e6, wall_damping, torch.ones_like(wall_damping))

        # Apply wall damping to advection (reduces momentum transport near walls)
