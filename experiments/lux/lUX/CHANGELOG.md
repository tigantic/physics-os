...
- **Phase 7 — Deployment, Auth & Production Operations**:
  ...
  - **Kubernetes manifests** (`deployment/k8s/`): 7 files — namespace, configmap, secret, deployment (2 replicas, rolling update, securityContext, probes, resource limits), service (ClusterIP), ingress (nginx + TLS for `lux.physics-os.io`), HPA (2–10 replicas, CPU 70% + memory 80% targets, 300s scale-down stabilization).
