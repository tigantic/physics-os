...
def main():
    ...
    for i in range(1000):
        ...
        if i % 100 == 0:
            print(
                f"  Frame {i:4d}: {frame_time_ms:6.2f}ms ({1000/frame_time_ms:5.1f} FPS)"
            )
    ...
    with open(output_file, "w") as f:
        ...
        for i in range(min(100, len(frame_times))):
            f.write(
                f"Frame {i:4d}: {frame_times[i]:6.2f}ms ({1000/frame_times[i]:6.1f} FPS)\n"
            )
    ...
    if max_time < mean_time * 1.5:
        ...
        print(
            f"⚠ Frame time spikes detected (max/mean ratio: {max_time/mean_time:.2f})"
        )
