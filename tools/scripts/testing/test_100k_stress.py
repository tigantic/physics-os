...
def main():
    ...
    with open(output_file, "w") as log:
        ...
        log.write(
            f"Total time: {(end_total - start_total):.2f}s ({(end_total - start_total)/60:.1f} min)\n"
        )
        ...
        for seg in range(10):
            ...
            seg_std = segment.std()
            log.write(
                f"Frames {start_idx:6d}-{end_idx:6d}: {seg_mean:.3f}ms ± {seg_std:.3f}ms ({1000/seg_mean:.1f} FPS)\n"
            )
        ...
        if mean_fps >= 60:
            ...
            log.write(
                "✗ Performance: FAIL ({:.1f} FPS < 60 FPS target)\n".format(mean_fps)
            )
        ...
        if stability_ratio < 2.0:
            log.write(
                "✓ Stability: PASS (99.9th/mean = {:.2f} < 2.0, max/mean = {:.2f})\n".format(
                    stability_ratio, max_time / mean_time
                )
            )
            passed_tests += 1
        else:
            log.write(
                "✗ Stability: FAIL (99.9th/mean = {:.2f} >= 2.0, max/mean = {:.2f})\n".format(
                    stability_ratio, max_time / mean_time
                )
            )
        ...
        if abs(total_vram_growth) < 0.1:
            log.write(
                "✓ Memory: PASS (growth = {:.3f} GB < 0.1 GB)\n".format(
                    total_vram_growth
                )
            )
            passed_tests += 1
            ...
            log.write(
                "✗ Memory: FAIL (growth = {:.3f} GB >= 0.1 GB)\n".format(
                    total_vram_growth
                )
            )
