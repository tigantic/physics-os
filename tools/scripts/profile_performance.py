...
Usage:
    python tools/scripts/profile_performance.py [--dmrg|--tebd|--cfd1d|--cfd2d] [--gpu]
...
def profile_tebd(device: str = "cpu", chi: int = 64, L: int = 20):
    ...
    psi = MPS.random(
        L, d=2, chi=chi, dtype=torch.float64, device=device, normalize=True
    )
    ...
    gate = U.reshape(
        2, 2, 2, 2
    )  # (s1, s2, s1', s2') -> transpose to (s1', s2', s1, s2)
