...
def parse_junit_xml(xml_path: Path) -> CategoryResults:
    ...
    results = CategoryResults(passed=0, failed=0, skipped=0, errors=0, tests=[])
...
def detect_regressions(baseline: dict, current: dict) -> RegressionReport:
        ...
        if isinstance(cat_results, dict) and "tests" in cat_results:
            for test in cat_results["tests"]:
                ...
                if key in baseline_tests:
                    ...
                    if baseline_test["status"] == "pass" and test["status"] in (
                        "fail",
                        "error",
                    ):
                        new_failures.append(
                    ...
                    if (
                        baseline_test["status"] in ("fail", "error")
                    ...
                    if (
                        baseline_test["time"] > 0
                        and test["time"] > baseline_test["time"] * 1.5
                    ):
    ...
    # Calculate summary
    total_current_passed = sum(
        c.get("passed", 0) for c in current.values() if isinstance(c, dict)
    )
    total_current_failed = sum(
        c.get("failed", 0) for c in current.values() if isinstance(c, dict)
    )
    total_baseline_passed = sum(
        c.get("passed", 0) for c in baseline.values() if isinstance(c, dict)
    )
    total_baseline_failed = sum(
        c.get("failed", 0) for c in baseline.values() if isinstance(c, dict)
    )
...
def main():
    ...
    parser.add_argument(
        "--baseline", type=Path, default=Path(".vv_baseline.json"), help="Baseline file"
    )
    parser.add_argument(
        "--current",
        type=Path,
        default=Path("artifacts"),
        help="Current artifacts directory",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print(f"\nSummary:")
    print(
        f"  Current: {report.summary['current_passed']} passed, {report.summary['current_failed']} failed"
    )
    if baseline:
        print(
            f"  Baseline: {report.summary['baseline_passed']} passed, {report.summary['baseline_failed']} failed"
        )
