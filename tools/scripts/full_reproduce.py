...
Usage:
    python tools/scripts/full_reproduce.py --quick   # Smoke test (5 min)
    python tools/scripts/full_reproduce.py --full    # Full reproduction (30+ min)
...
def run_benchmark(
    name: str, script: str, timeout: int = 300
...
def quick_smoke_test(project_root: Path) -> Dict:
    ...
    # Quick benchmarks
    benchmarks = [
        ("Sod Shock Tube", "experiments/benchmarks/benchmarks/sod_shock_tube.py"),
        ("TFIM Ground State", "experiments/benchmarks/benchmarks/tfim_ground_state.py"),
        ("QTT Compression", "experiments/benchmarks/benchmarks/qtt_compression.py"),
    ]
...
def full_reproduction(project_root: Path) -> Dict:
    ...
    # All benchmarks
    benchmarks = [
        ("Sod Shock Tube", "experiments/benchmarks/benchmarks/sod_shock_tube.py", 120),
        ("TFIM Ground State", "experiments/benchmarks/benchmarks/tfim_ground_state.py", 180),
        ("Heisenberg Ground State", "experiments/benchmarks/benchmarks/heisenberg_ground_state.py", 300),
        ("QTT Compression", "experiments/benchmarks/benchmarks/qtt_compression.py", 120),
        ("Blasius Validation", "experiments/benchmarks/benchmarks/blasius_validation.py", 180),
        ("Double Mach Reflection", "experiments/benchmarks/benchmarks/double_mach_reflection.py", 600),
        ("Oblique Shock", "experiments/benchmarks/benchmarks/oblique_shock.py", 180),
        ("SBLI Benchmark", "experiments/benchmarks/benchmarks/sbli_benchmark.py", 600),
    ]
    ...
    if physics_script.exists():
        ...
        passed, output, runtime = run_benchmark(
            "Physics Validation", str(physics_script), timeout=300
        )
        status = "PASS" if passed else "FAIL"
        print(f"{status} ({runtime:.1f}s)")
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick smoke test (~5 min)"
    )
    parser.add_argument(
        "--full", action="store_true", help="Full reproduction (~30+ min)"
    )
    ...
    all_benchmarks_passed = all(
        b["passed"] for b in results.get("benchmarks", []) if results.get("benchmarks")
    )
    all_proofs_passed = all(
        p["passed"] for p in results.get("proofs", []) if results.get("proofs")
    )
    all_physics_passed = all(
        p["passed"] for p in results.get("physics", []) if results.get("physics")
    )
    ...
    results["summary"] = {
        ...
        "benchmarks_passed": sum(
            1 for b in results.get("benchmarks", []) if b["passed"]
        ),
    ...
    print()
    print(
        f"  Benchmarks: {results['summary']['benchmarks_passed']}/{results['summary']['benchmarks_total']} passed"
    )
    print(
        f"  Proofs: {results['summary']['proofs_passed']}/{results['summary']['proofs_total']} passed"
    )
    print()
