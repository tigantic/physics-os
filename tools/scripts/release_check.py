    ...
    try:
        ...
        if "[Unreleased]" in content:
            ...
            if unreleased_section.strip() and len(unreleased_section.strip()) > 50:
                issues.append(
                    "Unreleased section has content - should be moved to version section"
                )
    ...
    try:
        ...
        if match:
            ...
            if version and cff_version != version:
                issues.append(
                    f"Version mismatch: CITATION.cff has {cff_version}, expected {version}"
                )
