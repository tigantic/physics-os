...
Usage:
    python tools/scripts/benchmark_tt_cfd.py
...
def benchmark_conservation_check(n_cells: int = 128, n_steps: int = 100) -> dict:
    ...
    return {
        ...
        "conservation_verified": (
            abs(final_mass - initial_mass) < 1e-10
            and abs(final_energy - initial_energy) < 1e-6
        ),
    }
...
def main():
    ...
    print("-" * 70)
    print(
        f"{'Solver':<25} {'N_cells':<10} {'Time (s)':<12} {'Steps/s':<12} {'Memory':<12}"
    )
    print("-" * 70)
    ...
    for n_cells in n_cells_list:
        ...
        try:
            ...
            print(
                f"{result['solver']:<25} {n_cells:<10} {result['elapsed_sec']:<12.4f} "
    ...
    try:
        ...
        print(
            f"Conservation:   {'✓ VERIFIED' if cons_result['conservation_verified'] else '✗ FAILED'}"
        )
    except Exception as e:
