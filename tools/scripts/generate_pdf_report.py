...
<table>
    ...
    <tr><td>tools/scripts/gauntlet_vs_nvidia.py</td><td class="num">~380</td>
        <td>Multi-resolution benchmark harness</td></tr>
    <tr><td>tools/scripts/ahmed_body_spectrum.py</td><td class="num">~410</td>
        <td>Energy spectrum analysis and plotting</td></tr>
    <tr><td>tools/scripts/trustless_physics.py</td><td class="num">~1,600</td>
        <td>Trustless physics proof engine v2.0 (Merkle, hash-chain, Ed25519, invariants)</td></tr>
    <tr><td>tools/scripts/run_trustless_ahmed.py</td><td class="num">~203</td>
        <td>Trustless proof runner &amp; offline verifier CLI (RK2, projection, incremental)</td></tr>
    <tr><td>proofs/proof_engine/proof_carrying.py</td><td class="num">425</td>
...
# Single resolution
PYTHONPATH="$PWD:$PYTHONPATH" python3 tools/tools/scripts/ahmed_body_ib_solver.py \\
    --n-bits 9 --max-rank 48 --steps 400 --cfl 0.08

# Spectrum analysis (requires completed solver run)
PYTHONPATH="$PWD:$PYTHONPATH" python3 tools/tools/scripts/ahmed_body_spectrum.py \\
