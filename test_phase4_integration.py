...
from tensornet.cfd.fast_euler_3d import FastEuler3D, Euler3DConfig
...
def create_taylor_green_vortex(config: Euler3DConfig):
    ...
    p = 100.0 + (rho[0, 0, 0] / 16.0) * (torch.cos(2*X) + torch.cos(2*Y)) * (torch.cos(2*Z) + 2)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="density",
        choices=["density", "velocity_x", "velocity_y", "velocity_z", "pressure", "energy"],
