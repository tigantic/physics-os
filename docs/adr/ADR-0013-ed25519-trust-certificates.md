...
The Trustless Physics Certificate (TPC) system requires cryptographic signatures to attest that a simulation result was produced by a verified HyperTensor pipeline — bit-exact inputs, deterministic solver, verified conservation laws. The signature must be:
...
2. Signing is performed by `proof_bridge` (Rust, via `ed25519-dalek`) or `hypertensor.tpc` (Python, via `PyNaCl`).
