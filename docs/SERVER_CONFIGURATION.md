...
1. Copy the example file to create your local configuration:
   ```bash
   cp sdk/server/.env.example sdk/server/.env
   ```
...
3. Load environment variables before starting the server:
   ```bash
   # Linux/Mac
   source sdk/server/.env && uvicorn sdk.server.main:app
   
   # Windows PowerShell
   Get-Content sdk/server/.env | ForEach-Object { 
       if ($_ -match '^([^#][^=]+)=(.*)$') { 
...
- [.env.example](../../sdk/server/.env.example) - Example configuration
- [main.py](../../sdk/server/main.py) - Server implementation
