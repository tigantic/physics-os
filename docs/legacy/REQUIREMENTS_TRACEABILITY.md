...
|----|-------------|----------------|-------|
| MPS-01 | MPS can represent arbitrary quantum states | `ontic/core/mps.py` | `test_mps_random_creation_*` |
| MPS-02 | MPS normalization preserves quantum norm | `MPS.normalize()` | `test_mps_normalization_*` |
| MPS-03 | MPO encodes local Hamiltonians | `ontic/core/mpo.py` | `test_mpo_heisenberg_*` |
...
| ID | Requirement | Implementation | Tests |
|----|-------------|----------------|-------|
| ALG-01 | DMRG converges to ground state | `ontic/algorithms/dmrg.py` | `test_dmrg_heisenberg_ground_state_*` |
| ALG-02 | TEBD performs unitary time evolution | `ontic/algorithms/tebd.py` | `test_tebd_*` |
| ALG-03 | TDVP preserves MPS manifold | `ontic/algorithms/tdvp.py` | `test_tdvp_*` |
| ALG-04 | Lanczos finds lowest eigenvalue | `ontic/algorithms/lanczos.py` | `test_lanczos_*` |
...
| ID | Requirement | Implementation | Tests |
|----|-------------|----------------|-------|
| CFD-01 | Euler1D conserves mass/momentum/energy | `ontic/cfd/euler_1d.py` | `test_euler1d_conservation_*` |
| CFD-02 | Shock tube matches exact Riemann | `ontic/cfd/godunov.py` | `test_cfd_sod_shock_tube_*` |
| CFD-03 | Euler2D uses Strang splitting | `ontic/cfd/euler_2d.py` | `test_euler2d_*` |
...
| QUA-03 | API documentation generated | `ontic/docs/api_reference.py` | 86 markdown files |
