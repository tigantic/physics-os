...
class StateVector:
    ...
    def from_vector(
        ...
        position = vector[idx : idx + 3]
        idx += 3
        velocity = vector[idx : idx + 3]
        idx += 3
        quaternion = vector[idx : idx + 4]
        idx += 4
        angular_rates = vector[idx : idx + 3]
        idx += 3
        accelerations = vector[idx : idx + 3]
        idx += 3
        control_surfaces = vector[idx : idx + n_control]
        idx += n_control

        thrust = vector[idx]
        idx += 1
        fuel_mass = vector[idx]
        idx += 1
        mach = vector[idx]
        idx += 1
        dynamic_pressure = vector[idx]
        idx += 1
        altitude = vector[idx]
        idx += 1
        ...
        if n_temps > 0:
            temperatures = vector[idx : idx + n_temps]
            idx += n_temps
        if n_loads > 0:
    ...
    def copy(self) -> "StateVector":
        ...
        return StateVector(
            ...
            temperatures=(
                self.temperatures.copy() if self.temperatures is not None else None
            ),
...
class StateBuffer:
    ...
    def get_at_time(self, t: float) -> StateVector | None:
    ...
    def get_range(self, t_start: float, t_end: float) -> list[StateVector]:
        ...
        with self.lock:
            return [s for s in self.buffer if t_start <= s.timestamp <= t_end]
...
def interpolate_state(
    state1: StateVector, state2: StateVector, t: float
) -> StateVector:
    """
    ...
    return StateVector(
        ...
        temperatures=(
            lerp(state1.temperatures, state2.temperatures, alpha)
            if state1.temperatures is not None and state2.temperatures is not None
            else None
        ),
        loads=(
            lerp(state1.loads, state2.loads, alpha)
            if state1.loads is not None and state2.loads is not None
            else None
        ),
    )
...
def extrapolate_state(
    states: list[StateVector], t: float, order: int = 2
) -> StateVector:
    """
...
def compute_state_divergence(
    physical: StateVector, digital: StateVector
) -> dict[str, float]:
    """
    ...
    weighted_sum = sum(
        weights.get(k, 1.0) * v**2 for k, v in divergence.items() if k in weights
    )
...
class StateSync:
    ...
    def __init__(self, config: SyncConfig):
        ...
        self.divergence_start_time: float | None = None
        ...
        # Callbacks
        self.on_state_received: Callable[[StateVector], None] | None = None
        self.on_divergence: Callable[[dict[str, float]], None] | None = None
    ...
    def _blend_states(
        self, physical: StateVector, digital: StateVector, alpha: float
    ) -> StateVector:
        """Blend two states with factor alpha (0=physical, 1=digital)."""
        return interpolate_state(
            physical,
            digital,
            physical.timestamp + alpha * (digital.timestamp - physical.timestamp),
        )
    ...
    def _check_divergence(self):
        ...
        # Check thresholds
        exceeded = (
            divergence["position"] > self.config.position_threshold
            or divergence["velocity"] > self.config.velocity_threshold
            or divergence["attitude"] > self.config.attitude_threshold
        )
        ...
        if exceeded:
            ...
            if (
                time.time() - self.divergence_start_time
                > self.config.max_divergence_time
            ):
    ...
    def _attempt_recovery(self):
        ...
        if len(digital_states) > 0:
            ...
            corrected = StateVector(
                ...
                position=digital.position
                + gain * (physical.position - digital.position),
                velocity=digital.velocity
                + gain * (physical.velocity - digital.velocity),
...
class StateSynchronizer:
    ...
    def get_state(self, t: float | None = None) -> StateVector | None:
