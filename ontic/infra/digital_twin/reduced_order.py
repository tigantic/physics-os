...
class ROMMetrics:
    ...
    prediction_error: float | None = None  # Prediction error (if dynamics)
    ...
    def compute_metrics(self, snapshots: torch.Tensor) -> ROMMetrics:
        ...
        with torch.no_grad():
            ...
            if hasattr(self, "singular_values"):
                ...
                captured = torch.sum(
                    self.singular_values[: self.latent_dim] ** 2
                ).item()
...
class PODModel(ReducedOrderModel):
    ...
    def train_from_snapshots(self, snapshots: torch.Tensor):
        ...
        if self.config.energy_threshold < 1.0:
            ...
            n_modes = (
                torch.searchsorted(cumulative, self.config.energy_threshold).item() + 1
            )
...
class DMDModel(ReducedOrderModel):
    ...
    def predict(self, x0: torch.Tensor, n_steps: int) -> torch.Tensor:
        ...
        a0 = torch.linalg.lstsq(self.modes, x0_norm.unsqueeze(1).to(self.modes.dtype))[
            0
        ].squeeze()
...
def create_rom_from_snapshots(
    snapshots: torch.Tensor,
    rom_type: ROMType = ROMType.POD,
    config: ROMConfig | None = None,
) -> ReducedOrderModel:
    """
...
def validate_rom_accuracy(
    model: ReducedOrderModel, test_snapshots: torch.Tensor
) -> ROMMetrics:
    """
...
def test_reduced_order():
    ...
    # Two-mode system
    snapshots = (
        torch.sin(t).unsqueeze(1) * torch.sin(np.pi * x).unsqueeze(0)
    ...
    # Test Autoencoder
    print("\n  Testing Autoencoder...")
    ae_config = ROMConfig(n_modes=5, hidden_dims=[64, 32], dropout=0.0)
