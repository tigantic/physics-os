...
class Navigation:
    ...
    def add_item(self, item: NavItem, parent_path: str | None = None):
        """Add navigation item."""
    ...
    def _add_to_parent(
        self, items: list[NavItem], parent_path: str, item: NavItem
    ) -> bool:
...
class MarkdownRenderer:
    ...
    def _render_math(self, html: str) -> str:
        ...
        # Block math
        html = re.sub(
            r"\$\$(.+?)\$\$", r'<div class="math-block">\1</div>', html, flags=re.DOTALL
        )
        # Inline math
...
<body class="api-page">
    ...
    def render(self, template_name: str, context: dict[str, Any]) -> str:
        ...
        result = re.sub(
            r"{%\s*if\s+\w+\s*%}.*?{%\s*endif\s*%}", "", result, flags=re.DOTALL
        )
...
class SiteBuilder:
    ...
    def add_pages_from_directory(self, directory: str | Path):
        """
