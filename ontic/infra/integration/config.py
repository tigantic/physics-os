...
class Configuration:
    ...
    def merge(
        self, other: "Configuration", source: ConfigSource = ConfigSource.OVERRIDE
    ):
...
class ConfigManager:
    ...
    def _load_defaults(self):
        ...
        # Deployment defaults
        deploy = ConfigSection(
            name="deployment", description="Deployment configuration"
        )
...
def load_config(path: str | Path) -> bool:
    """
    Load configuration from file.
...
def validate_config(config: Configuration | None = None) -> list[str]:
    """
    Validate configuration.
