...
class QTTSliceExtractor:
    ...
    def morton_encode_3d_gpu(
        self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor, n_bits: int
    ) -> torch.Tensor:
        """
    ...
    def morton_decode_3d_gpu(
        self, morton_idx: torch.Tensor, n_bits: int
    ...
    def eval_qtt_at_indices_gpu(
        self, qtt: QTT3DState, morton_indices: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if result.shape[1] != 1:
            raise RuntimeError(
                f"QTT contraction failed: final rank should be 1, got {result.shape[1]}"
            )
    ...
    def extract_xy_slice(
        ...
        x_coords = torch.linspace(
            0, qtt.grid_size - 1, width, device=self.device
        ).long()
        y_coords = torch.linspace(
            0, qtt.grid_size - 1, height, device=self.device
        ).long()
