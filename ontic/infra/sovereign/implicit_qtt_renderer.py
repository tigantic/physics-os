...
class ImplicitQTTRenderer:
    ...
    def __init__(
        ...
        self.output_buffer = torch.zeros(
            (height, width, 4), dtype=torch.float32, device=self.device
        )
    ...
    def render_multi_layer(
        ...
        if len(layer_enabled) != len(qtt_layers):
            raise ValueError(
                f"layer_enabled length {len(layer_enabled)} != qtt_layers length {len(qtt_layers)}"
            )
        ...
        enabled_flags = torch.tensor(
            layer_enabled, dtype=torch.int32, device=self.device
        )
