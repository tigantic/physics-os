...
class CUDAHeatmapGenerator:
    ...
    def __init__(
        ...
        print("✓ CUDA Heatmap Generator initialized")
    ...
    def _create_plasma_colormap(self) -> torch.Tensor:
        ...
        r = (
            torch.clamp(1.0 - 2.0 * (t - 0.5).abs(), 0, 1) * 0.5
        ...
        g = torch.clamp(0.5 + 0.5 * torch.sin(math.pi * (t * 2.0 - 1.0)), 0, 1)
        b = 1.0 - 0.9 * t
...
def main() -> None:
    ...
    parser.add_argument(
        "--duration", type=float, default=None, help="Run duration (seconds)"
    )
    parser.add_argument(
        "--resolution", type=str, default="256x128", help="Grid resolution (WxH)"
    )
    parser.add_argument(
        "--qtt", action="store_true", help="Use QTT backend (2900+ FPS)"
    )
