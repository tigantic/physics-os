    ...
    def from_states(
        ...
        return cls(
            ...
            metrics={
                ...
                "max_error": max_error,
                "convergence_fraction": sum(
                    1 for e in errors.values() if e <= threshold
                )
                / max(len(errors), 1),
            },
        )
...
class FormationController:
    ...
    def compute_formation_positions(
        ...
        if self.config.formation_type == FormationType.LINE:
            ...
            for i in range(4, num_vehicles):
                ...
                dx, dy = rotate(
                    ring * spacing * math.cos(angle), ring * spacing * math.sin(angle)
                )
                positions.append(center + np.array([dx, dy, 0]))
            ...
            for i in range(num_vehicles):
                ...
                if offset_key in self.config.custom_offsets:
                    ...
                    dx, dy = rotate(
                        offset[0] * self.config.scale, offset[1] * self.config.scale
                    )
                    positions.append(
                        center + np.array([dx, dy, offset[2] if len(offset) > 2 else 0])
                    )
                else:
    ...
    def get_target_positions(
        ...
        if not self._slot_assignments or set(vehicle_ids) != set(
            self._slot_assignments.keys()
        ):
