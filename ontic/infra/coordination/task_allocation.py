...
class TaskAllocator:
    ...
    def allocate_greedy(
        ...
        assignments = []
        vehicle_task_count = dict.fromkeys(states, 0)
        assigned_tasks = set()
    ...
    def allocate_nearest(
        ...
        for vid, state in states.items():
            if (
                len([a for a in assignments if a.vehicle_id == vid])
                >= max_tasks_per_vehicle
            ):
                continue
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        assigned = sum(
            1 for t in self._tasks.values() if t.status == TaskStatus.ASSIGNED
        )
        completed = sum(
