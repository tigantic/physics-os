...
class NoiseChannel:
    ...
    probability: float
    target_qubits: list[int] | None = None  # None = all qubits
...
class NoiseModel:
    ...
    t1_times: dict[int, float] | None = None  # Relaxation times
    t2_times: dict[int, float] | None = None  # Dephasing times

    def add_depolarizing(self, p: float, qubits: list[int] | None = None):
        """Add depolarizing noise channel."""
        self.channels.append(NoiseChannel(NoiseType.DEPOLARIZING, p, qubits))
        return self

    def add_amplitude_damping(self, gamma: float, qubits: list[int] | None = None):
        """Add amplitude damping (T1 decay)."""
        self.channels.append(NoiseChannel(NoiseType.AMPLITUDE_DAMPING, gamma, qubits))
        return self

    def add_phase_damping(self, gamma: float, qubits: list[int] | None = None):
    ...
    def from_device_params(
        ...
        model.t1_times = dict.fromkeys(range(n_qubits), t1_us)
        model.t2_times = dict.fromkeys(range(n_qubits), t2_us)
...
class KrausChannel:
    ...
    def _validate(self):
        ...
        if not torch.allclose(total, torch.eye(d, dtype=torch.complex128), atol=1e-10):
            raise ValueError(
                "Kraus operators do not satisfy trace-preserving condition"
            )
...
class ZeroNoiseExtrapolator:
    ...
    def __init__(self, config: ZNEConfig | None = None):
        self.config = config or ZNEConfig()

    def fold_circuit(
        self, circuit_executor: Callable[[], float], scale_factor: float
    ) -> Callable[[], float]:
        """
...
class ProbabilisticErrorCancellation:
    ...
    def __init__(self, noise_model: NoiseModel, config: PECConfig | None = None):
    ...
    def mitigate(
        self, circuit_executor: Callable[[list[str]], float], n_gates: int
    ) -> tuple[float, float]:
        """
...
class CliffordDataRegression:
    ...
    def generate_training_circuits(
        self, template_circuit: list, n_circuits: int
...
class PhaseFlipCode(QECCode):
    ...
    def decode(self, physical_state: torch.Tensor) -> torch.Tensor:
        ...
        logical[1] = sum(
            ((-1) ** bin(i).count("1")) * physical_state[i] for i in range(8)
        ) / (2 * math.sqrt(2))
...
class NoiseAwareOptimizer:
    ...
    def __init__(
        ...
        n_params: int,
        config: NoiseAwareVQEConfig | None = None,
    ):
    ...
    def mitigated_objective(
        self, params: torch.Tensor, n_samples: int = 10
    ) -> tuple[float, float]:
        """
...
def create_device_noise_model(device_name: str = "ibm_perth") -> NoiseModel:
    """
