...
class ObjectiveSpec:
    ...
    minimize: bool = True
    weight: float = 1.0
    reference: float | None = None  # For reference point method
    utopia: float | None = None  # Best achievable value
    nadir: float | None = None  # Worst Pareto value
...
def dominates(
    obj_a: dict[str, float], obj_b: dict[str, float], minimize: dict[str, bool]
) -> bool:
    """
...
def fast_non_dominated_sort(
    population: list[ParetoSolution], minimize: dict[str, bool]
) -> list[list[int]]:
    """
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if dominates(population[i].objectives, population[j].objectives, minimize):
                ...
                population[j].objectives, population[i].objectives, minimize
            ):
...
def crowding_distance(
    ...
    for obj_name in obj_names:
        ...
        sorted_indices = sorted(
            front_indices, key=lambda i: population[i].objectives[obj_name]
        )
...
class MultiObjectiveOptimizer:
    ...
    def random_design(self) -> torch.Tensor:
        ...
        return self.lower_bounds + torch.rand(self.n_vars) * (
    ...
    def tournament_selection(self, population: list[ParetoSolution]) -> ParetoSolution:
        """
    ...
    def run_nsga2(self) -> MOOResult:
        ...
        for generation in range(config.n_generations):
            ...
            while (
                len(new_population) + len(fronts[front_idx]) <= config.population_size
            ):
        ...
        utopia = {
            name: min(s.objectives[name] for s in pareto_front) for name in obj_names
        }
        nadir = {
            name: max(s.objectives[name] for s in pareto_front) for name in obj_names
        }
    ...
    def run_weighted_sum(self) -> MOOResult:
        ...
        for i in range(n_weights):
            ...
            w2 = 1.0 - w1
            weights_list.append(
                {self.objectives[0].name: w1, self.objectives[1].name: w2}
            )
        ...
        for i, sol in enumerate(pareto_front):
            ...
            for j, other in enumerate(pareto_front):
                if i != j and dominates(
                    other.objectives, sol.objectives, self.minimize
                ):
                    is_dominated = True
                    break
            if not is_dominated:
        ...
        obj_names = [obj.name for obj in self.objectives]
        utopia = {
            name: min(s.objectives[name] for s in non_dominated) for name in obj_names
        }
        nadir = {
            name: max(s.objectives[name] for s in non_dominated) for name in obj_names
        }
    ...
    def optimize(self) -> MOOResult:
        ...
        if self.config.algorithm == MOOAlgorithm.NSGA_II:
            ...
            from ontic.core.phase_deferred import PhaseDeferredError
...
def create_drag_heating_problem(
    n_vars: int = 10,
) -> tuple[list[ObjectiveSpec], tuple[torch.Tensor, torch.Tensor]]:
    """
    ...
    objectives = [
        ObjectiveSpec(
            name="drag", function=drag_objective, gradient=drag_gradient, minimize=True
        ),
        ObjectiveSpec(
            name="heating",
...
def validate_moo():
    ...
    config = MOOConfig(
        algorithm=MOOAlgorithm.NSGA_II, population_size=20, n_generations=10
    )
