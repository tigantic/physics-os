...
def apply_qtt_filter_3d(
    U: torch.Tensor, chi_max: int = 64, tol: float = 1e-8
) -> torch.Tensor:
    """
...
def enforce_hou_luo_symmetry(U: torch.Tensor) -> torch.Tensor:
    ...
    window_3d = (
        window_1d.view(-1, 1, 1) * window_1d.view(1, -1, 1) * window_1d.view(1, 1, -1)
    )
...
def compute_residual_direct(
    ...
    div_R = 1j * kx * R_hat[..., 0] + 1j * ky * R_hat[..., 1] + 1j * kz * R_hat[..., 2]
...
def stabilized_newton_refinement(config: RefinementConfig = None) -> RefinementResult:
    ...
    for iteration in range(config.max_iter):
        ...
        if config.verbose:
            if iteration % 10 == 0:
                print(
                    f"  Iter {iteration:3d}: ||F|| = {f_norm:.6e} | α = {alpha:.5f} | η = {current_eta:.2e} | best = {best_f:.2e}"
                )
        ...
        for step_scale in [1.0, 0.5, 0.25, 0.1]:
            ...
            if use_qtt:
                U_trial_filtered = apply_qtt_filter_3d(
                    U_trial, chi_max=config.chi_max, tol=config.qtt_tol
                )
            else:
        ...
        if config.alpha_adapt and iteration % 20 == 0 and iteration > 0:
            ...
            for alpha_test in np.linspace(
                max(0.1, alpha - 0.1), min(1.5, alpha + 0.1), 11
            ):
                _, f_test = compute_residual_direct(U, alpha_test, config.nu)
            ...
            if best_alpha_val != alpha:
                ...
                if config.verbose:
                    print(
                        f"         ★ α updated to {alpha:.4f} (||F|| = {best_alpha_f:.2e})"
                    )
...
if __name__ == "__main__":
    ...
    output_path = (
        Path(__file__).parent.parent.parent / "proofs" / "refined_singularity.pt"
    )
    torch.save(
        {
