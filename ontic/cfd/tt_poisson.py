...
class PoissonResult:
    ...
    def __repr__(self) -> str:
        return (
            f"PoissonResult(converged={self.converged}, "
...
class ALSEnvironments:
    ...
    def get_effective_operator(self, site: int) -> tuple[Tensor, Tensor]:
        ...
        tmp_LA = torch.einsum(
            "iak,abcd->ibckd", L[:, :, 0:1].squeeze(-1), A_s
        )  # (χ_l, d_out, d_in, D_r)
...
def _optimize_site_poisson(
    ...
    site: int,
    direction: str,
    envs: ALSEnvironments | None = None,
) -> None:
    """
...
def _qtt_subtract(a: "QTTState", b: "QTTState", max_rank: int) -> "QTTState":
    ...
    from ontic.cfd.pure_qtt_ops import qtt_add, truncate_qtt
...
def test_projection():
    ...
    # Project
    result = project_velocity_2d(
        u_star, v_star, dx, dy, dt, bc="periodic", method="spectral"
    )

    print(f"After projection max|∇·u|: {result.divergence_after:.4e}")
    print(
        f"Reduction factor: {result.divergence_before / max(result.divergence_after, 1e-16):.2e}"
    )
    ...
    u_mag_after = (
        torch.sqrt(result.u_projected**2 + result.v_projected**2).mean().item()
    )
    print(
        f"Mean velocity magnitude: before={u_mag_before:.4f}, after={u_mag_after:.4f}"
    )
...
def test_advection():
    ...
    print("  Shear flow (u=sin(2πy), v=0):")
...
def test_poisson_solver():
    ...
    for N in [16, 32, 64]:
        ...
        interior_error = (
            torch.abs(phi_computed[1:-1, 1:-1] - phi_exact[1:-1, 1:-1]).max().item()
        )
        ...
        print(f"\nN = {N}, dx = {dx:.4f}")
        print(
            f"  Self-consistency error: {self_error:.2e} (should be ~machine epsilon)"
        )
