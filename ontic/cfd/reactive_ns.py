...
from .implicit import AdaptiveImplicit, ImplicitConfig
from .viscous import (
    GAMMA_AIR,
    PR_AIR,
...
class ReactiveState:
    ...
    def to_euler_state(self, gamma: float) -> Euler2DState:
        ...
        return Euler2DState(rho=self.rho, u=self.u, v=self.v, p=self.p, gamma=gamma)
    ...
    def to_chemistry_state(self) -> ChemistryState:
        ...
        return ChemistryState(rho=self.rho, Y=self.Y, T=self.T, p=self.p)
...
class ReactiveNS:
    ...
    def __init__(self, config: ReactiveConfig):
        ...
        self.euler = Euler2D(Nx=config.Nx, Ny=config.Ny, Lx=config.Lx, Ly=config.Ly)
        ...
        for j in range(Ny):
            for i in range(Nx):
                ...
                Y_local = torch.tensor(
                    [state.Y[s][j, i].item() for s in Species], dtype=torch.float64
                )
                ...
                def omega_fn(Y_vec: torch.Tensor, T_val: float) -> torch.Tensor:
                    ...
                    conc = {
                        s: rho_local * Y_vec[s.value].item() / MW[s] for s in Species
                    }
                    ...
                    return torch.tensor(
                        [omega[s].item() for s in Species], dtype=torch.float64
                    )
    ...
    def _diffusion_step(self, state: ReactiveState, dt: float) -> ReactiveState:
        ...
        rhs = compute_viscous_rhs_2d(
            state.rho, state.u, state.v, T, mu, k, config.dx, config.dy
        )
        ...
        for species in Species:
            ...
            lap_Y[1:-1, 1:-1] = (
                Y[1:-1, 2:] - 2 * Y[1:-1, 1:-1] + Y[1:-1, :-2]
            ) / config.dx**2 + (
                Y[2:, 1:-1] - 2 * Y[1:-1, 1:-1] + Y[:-2, 1:-1]
            ) / config.dy**2
        ...
        return ReactiveState(rho=state.rho.clone(), u=u_new, v=v_new, p=p_new, Y=Y_new)
    ...
    def compute_timestep(self, state: ReactiveState) -> float:
        ...
        dt_cfl = config.cfl * min(
            config.dx / (u_max + c_max), config.dy / (v_max + c_max)
        )
...
def reactive_flat_plate_ic(
    ...
    return ReactiveState(rho=rho, u=u, v=v, p=p, Y=Y)
