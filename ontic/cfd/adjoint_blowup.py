...
from ontic.cfd.self_similar import (
    RescaledNSEquations,
    SelfSimilarScaling,
...
class AdjointOptimizer:
    ...
    def __init__(self, config: OptimizationConfig):
        ...
        self.ns = RescaledNSEquations(self.scaling, nu=config.nu, N=config.N)
    ...
    def forward_euler_step(self, U: torch.Tensor, dt: float) -> torch.Tensor:
        """
        ...
        def rhs(u):
            ...
            # Derivatives
            dudx = torch.fft.ifftn(
                1j * self.kx.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
            dudy = torch.fft.ifftn(
                1j * self.ky.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
            dudz = torch.fft.ifftn(
                1j * self.kz.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
    ...
    def evolve_forward(
        self, U0: torch.Tensor, store_trajectory: bool = True
    ...
    def compute_objective_gradient(self, U: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.objective == "enstrophy":
            ...
            U_hat = torch.fft.fftn(U, dim=(0, 1, 2))
            lap_U = torch.fft.ifftn(
                -self.k_sq.unsqueeze(-1) * U_hat, dim=(0, 1, 2)
            ).real
            ...
            curl_adj[..., 0] = (
                1j * self.ky * grad_hat[..., 2] - 1j * self.kz * grad_hat[..., 1]
            )
            curl_adj[..., 1] = (
                1j * self.kz * grad_hat[..., 0] - 1j * self.kx * grad_hat[..., 2]
            )
            curl_adj[..., 2] = (
                1j * self.kx * grad_hat[..., 1] - 1j * self.ky * grad_hat[..., 0]
            )

            return torch.fft.ifftn(curl_adj, dim=(0, 1, 2)).real

    def adjoint_step(
        self, Lambda: torch.Tensor, U: torch.Tensor, dt: float
    ) -> torch.Tensor:
        """
        ...
        def adjoint_rhs(lam, u):
            ...
            dlam_dx = torch.fft.ifftn(
                1j * self.kx.unsqueeze(-1) * lam_hat, dim=(0, 1, 2)
            ).real
            dlam_dy = torch.fft.ifftn(
                1j * self.ky.unsqueeze(-1) * lam_hat, dim=(0, 1, 2)
            ).real
            dlam_dz = torch.fft.ifftn(
                1j * self.kz.unsqueeze(-1) * lam_hat, dim=(0, 1, 2)
            ).real

            # Derivatives of u
            du_dx = torch.fft.ifftn(
                1j * self.kx.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
            du_dy = torch.fft.ifftn(
                1j * self.ky.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
            du_dz = torch.fft.ifftn(
                1j * self.kz.unsqueeze(-1) * u_hat, dim=(0, 1, 2)
            ).real
            ...
            # Laplacian
            lap_lam = torch.fft.ifftn(
                -self.k_sq.unsqueeze(-1) * lam_hat, dim=(0, 1, 2)
            ).real
    ...
    def compute_gradient(self, U0: torch.Tensor) -> tuple[torch.Tensor, float]:
        """
        ...
        if self.config.regularization > 0:
            ...
            reg_grad = torch.fft.ifftn(
                self.k_sq.unsqueeze(-1) * U0_hat, dim=(0, 1, 2)
            ).real
    ...
    def optimize(self, U0_init: torch.Tensor | None = None) -> OptimizationResult:
        """
        ...
        for iteration in range(self.config.n_iter):
            ...
            if self.config.verbose and iteration % 10 == 0:
                ...
                print(
                    f"Iter {iteration:4d}: J = {J:.6f}, ||∇J|| = {grad_norm:.4f}, ||ω||_∞ = {omega_max:.4f}"
                )
...
def find_blowup_candidate(
    N: int = 48, nu: float = 1e-3, n_iter: int = 50, verbose: bool = True
) -> OptimizationResult:
    """
