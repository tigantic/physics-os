...
class BSplineParameterization(GeometryParameterization):
    ...
    def __init__(
        self, n_control_points: int, degree: int = 3, n_eval_points: int = 100
    ):
    ...
    def _basis_function(self, i: int, p: int, u: float) -> float:
        ...
        if denom2 > 1e-10:
            term2 = (
                (knots[i + p + 1] - u) / denom2 * self._basis_function(i + 1, p - 1, u)
            )
...
class ShapeOptimizer:
    ...
    def _smooth_gradient(self, grad: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.config.smoothing_iterations):
            ...
            for i in range(1, n - 1):
                new_smoothed[i] = (1 - w) * smoothed[i] + w * 0.5 * (
    ...
    def optimize_steepest_descent(self, alpha0: torch.Tensor) -> OptimizationResult:
        ...
        for iteration in range(config.max_iterations):
            ...
            # Record history
            self.history.append(
                {"iteration": iteration, "objective": J, "gradient_norm": grad_norm}
            )
    ...
    def optimize_lbfgs(self, alpha0: torch.Tensor) -> OptimizationResult:
        """
        ...
        def closure():
            ...
            self.history.append({"objective": J, "gradient_norm": final_grad_norm})
...
def create_wedge_design_problem(
    n_control: int = 10, Mach: float = 5.0, theta_initial: float = 10.0
...
def validate_optimization():
    ...
    print(
        f"Curve range: x=[{curve[:,0].min():.3f}, {curve[:,0].max():.3f}], "
