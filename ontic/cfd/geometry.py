...
class WedgeGeometry:
    ...
    def distance_to_surface(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        dist = torch.where(dx < 0, d_le, torch.minimum(d_upper, d_lower))
    ...
    def surface_normal(self, x: Tensor, y: Tensor) -> tuple[Tensor, Tensor]:
        ...
        ny = torch.where(
            is_upper, torch.full_like(y, cos_theta), torch.full_like(y, -cos_theta)
        )
    ...
    def _compute_ghost_cells(self) -> Tensor:
        ...
        mask_padded = (
            torch.nn.functional.pad(
                self.mask.unsqueeze(0).float(), (1, 1, 1, 1), mode="constant", value=0
            )
            .squeeze(0)
            .bool()
        )
...
def compute_pressure_coefficient(
    p: Tensor, p_inf: float, rho_inf: float, u_inf: float, gamma: float = gamma_default
) -> Tensor:
    """
