...
class AdjointState:
    ...
    def to_tensor(self) -> torch.Tensor:
        ...
        return torch.stack(
            [self.psi_rho, self.psi_rhou, self.psi_rhov, self.psi_E], dim=0
        )

    @classmethod
    def from_tensor(cls, psi: torch.Tensor) -> "AdjointState":
        """Create from (4, Ny, Nx) tensor."""
        return cls(psi_rho=psi[0], psi_rhou=psi[1], psi_rhov=psi[2], psi_E=psi[3])
...
class HeatFluxObjective(ObjectiveFunction):
    ...
    def gradient(
        ...
        **kwargs,
        ...
        return (dJ_drho, torch.zeros_like(u), torch.zeros_like(v), dJ_dp)
...
class AdjointEuler2D:
    ...
    def flux_jacobian_x(
        self, rho: torch.Tensor, u: torch.Tensor, v: torch.Tensor, p: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def adjoint_rhs(
        ...
        d2psi_dx2[:, :, 1:-1] = (
            psi_vec[:, :, 2:] - 2 * psi_vec[:, :, 1:-1] + psi_vec[:, :, :-2]
        )
