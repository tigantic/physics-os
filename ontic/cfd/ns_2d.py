...
from ontic.cfd.tt_poisson import (
    ProjectionResult,
...
class NS2DSolver:
    ...
    def step_rk4(
        ...
        state2 = NSState(
            u=proj2.u_projected, v=proj2.v_projected, t=state.t + 0.5 * dt, step=0
        )
        ...
        state3 = NSState(
            u=proj3.u_projected, v=proj3.v_projected, t=state.t + 0.5 * dt, step=0
        )
        ...
        state4 = NSState(
            u=proj4.u_projected, v=proj4.v_projected, t=state.t + dt, step=0
        )
        ...
        # Final projection
        proj = project_velocity_2d(
            u_star, v_star, self.dx, self.dy, dt=1.0, method="spectral"
        )
    ...
    def compute_stable_dt(
        ...
        dt_adv = cfl_target * min(self.dx / (max_u + 1e-10), self.dy / (max_v + 1e-10))
    ...
    def solve(
        ...
        if verbose:
            print(
                f"NS2D Solver ({method.upper()}): t_final={t_final:.4f}, dt={dt:.2e}, ν={self.nu:.2e}"
            )
            print(
                f"  Initial: KE={initial_diag.kinetic_energy:.4e}, "
...
def test_taylor_green():
    ...
    result = solver.solve(
        state0, t_final, cfl_target=0.2, diag_interval=10, verbose=True
    )
    ...
    print(
        f"  Max divergence:            {result.diagnostics_history[-1].max_divergence:.2e}"
    )
