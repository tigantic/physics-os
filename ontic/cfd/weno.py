...
def smoothness_indicators(u: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    ...
    beta0 = (13.0 / 12.0) * (um2 - 2.0 * um1 + u0) ** 2 + (1.0 / 4.0) * (
        um2 - 4.0 * um1 + 3.0 * u0
    ) ** 2

    # Smoothness indicator for stencil 1: {i-1, i, i+1}
    beta1 = (13.0 / 12.0) * (um1 - 2.0 * u0 + up1) ** 2 + (1.0 / 4.0) * (um1 - up1) ** 2

    # Smoothness indicator for stencil 2: {i, i+1, i+2}
    beta2 = (13.0 / 12.0) * (u0 - 2.0 * up1 + up2) ** 2 + (1.0 / 4.0) * (
        3.0 * u0 - 4.0 * up1 + up2
    ) ** 2
...
def weno5_js(
    ...
    omega0, omega1, omega2 = nonlinear_weights_js(
        beta0, beta1, beta2, d0, d1, d2, epsilon=config.epsilon, p=config.p
    )
...
def teno_cutoff_function(
    beta0: Tensor, beta1: Tensor, beta2: Tensor, C_T: float = 1e-5
) -> tuple[Tensor, Tensor, Tensor]:
    """
    ...
    if all_cut.any():
        ...
        delta0 = torch.where(
            all_cut & (beta0 <= min_beta + 1e-40), torch.ones_like(delta0), delta0
        )
        delta1 = torch.where(
            all_cut & (beta1 <= min_beta + 1e-40) & (delta0 < 0.5),
            torch.ones_like(delta1),
            delta1,
        )
        delta2 = torch.where(
            all_cut & (delta0 < 0.5) & (delta1 < 0.5), torch.ones_like(delta2), delta2
        )
...
def reconstruct(
    ...
    **kwargs,
    ...
    if method == "weno5-js":
        ...
        raise ValueError(
            f"Unknown method: {method}. Choose from: weno5-js, weno5-z, teno5"
        )
...
def convergence_order(errors: list[float], dx_values: list[float]) -> float:
    """
