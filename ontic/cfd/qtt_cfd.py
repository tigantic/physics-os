...
class QTT_Euler1D:
    ...
    def __init__(
        ...
        self.state: QTTEulerState | None = None
        self.time = 0.0
        self._step_count = 0
    ...
    def initialize_sod(self):
        ...
        # Sod shock tube IC
        rho = torch.where(
            x < 0.5 * self.L, torch.ones_like(x), 0.125 * torch.ones_like(x)
        )
        u = torch.zeros_like(x)
        p = torch.where(x < 0.5 * self.L, torch.ones_like(x), 0.1 * torch.ones_like(x))

        self.state = QTTEulerState.from_primitive(
            rho, u, p, gamma=self.gamma, chi_max=self.chi_max, tol=self.config.tol
        )
        self.time = 0.0
        self._step_count = 0
    ...
    def step(self, dt: float | None = None):
        ...
        F_rho, F_rhou, F_E = self._rusanov_flux(rho_L, rhou_L, E_L, rho_R, rhou_R, E_R)
    ...
    def solve(self, t_final: float, dt: float | None = None):
        """Solve until t_final."""
        if dt is None:
            dt = self.config.dt
...
__all__ = ["QTTCFDConfig", "QTTEulerState", "QTT_Euler1D", "complexity_comparison"]
