...
class KantorovichBounds:
    ...
    error_bound: float | None  # ||u* - ū|| if proof succeeds
    ...
    def verify_profile(
        ...
        if discriminant < 0.5:
            ...
            if verbose:
                print("\n★ PROOF SUCCESSFUL ★")
                print(
                    f"  A true singularity exists within ||u* - ū|| ≤ {error_bound:.6e}"
                )
            ...
            if verbose:
                print("\n⚠ INCONCLUSIVE")
                print("  Discriminant is between 0.5 and 1.0")
                print("  Need finer grid or better profile")
        else:
            status = VerificationStatus.PROOF_FAILURE
            error_bound = None
            if verbose:
                print("\n✗ PROOF FAILED")
                print("  Discriminant ≥ 1.0 - profile may not be near a singularity")
...
if __name__ == "__main__":
    from ontic.cfd.self_similar import create_candidate_profile
