...
class EnstrophyObjective:
    ...
    def evaluate(self, u: Tensor, v: Tensor, w: Tensor) -> Tensor:
        ...
        from ontic.cfd.ns_3d import compute_vorticity_3d
    ...
    def gradient(
        ...
        return (-curl_omega_x * scale, -curl_omega_y * scale, -curl_omega_z * scale)
...
class SingularityHunter:
    ...
    def __init__(
        ...
        self.solver = NS3DSolver(
            Nx=Nx, Ny=Ny, Nz=Nz, Lx=Lx, Ly=Ly, Lz=Lz, nu=nu, dtype=dtype
        )
    ...
    def random_smooth_ic(
        self, n_modes: int = 4, amplitude: float = 1.0
    ) -> tuple[Tensor, Tensor, Tensor]:
        """
        ...
        x = torch.linspace(
            0, self.Lx * (1 - 1 / self.Nx), self.Nx, dtype=self.dtype, device=device
        )
        y = torch.linspace(
            0, self.Ly * (1 - 1 / self.Ny), self.Ny, dtype=self.dtype, device=device
        )
        z = torch.linspace(
            0, self.Lz * (1 - 1 / self.Nz), self.Nz, dtype=self.dtype, device=device
        )
        ...
        for _ in range(n_modes):
            ...
            mode = (
                torch.cos(kx * X + phase_x)
                * torch.sin(ky * Y + phase_y)
                * torch.cos(kz * Z + phase_z)
            )
        ...
        proj = project_velocity_3d(
            u, v, w, self.dx, self.dy, self.dz, dt=1.0, method="spectral"
        )
    ...
    def forward_integrate(
        ...
        """
        from ontic.cfd.chi_diagnostic import compute_chi_state_3d
    ...
    def hunt(
        self,
        u0_init: Tensor | None = None,
        v0_init: Tensor | None = None,
        w0_init: Tensor | None = None,
        ...
        for iteration in range(self.config.max_iterations):
            ...
            print(
                f"  Iter {iteration}: chi_max={chi_max:.1f}, enstrophy_max={enstrophy_max:.4f}"
            )
            ...
            if iteration % self.config.projection_interval == 0:
                proj = project_velocity_3d(
                    u0, v0, w0, self.dx, self.dy, self.dz, dt=1.0
                )
                u0 = proj.u_projected
                v0 = proj.v_projected
                w0 = proj.w_projected
