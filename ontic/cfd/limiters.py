...
def mc_limiter(r: Tensor) -> Tensor:
    ...
    return torch.maximum(
        torch.zeros_like(r),
        torch.minimum(torch.minimum(2 * r, (1 + r) / 2), 2 * torch.ones_like(r)),
    )
...
def koren(r: Tensor) -> Tensor:
    ...
    return torch.maximum(
        torch.zeros_like(r),
        torch.minimum(torch.minimum(2 * r, (2 + r) / 3), 2 * torch.ones_like(r)),
    )
...
def compute_slope_ratio(u: Tensor, i: int) -> Tensor:
    ...
    denom = torch.where(
        torch.abs(delta_plus) < 1e-10, torch.sign(delta_plus) * 1e-10, delta_plus
    )
...
def apply_limiter(
    ...
    if limiter not in limiters:
        raise ValueError(
            f"Unknown limiter: {limiter}. Options: {list(limiters.keys())}"
        )
