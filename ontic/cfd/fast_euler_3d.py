...
from ontic.cfd.pure_qtt_ops import QTTState, dense_to_qtt, qtt_add, qtt_to_dense
...
def dense_to_qtt_3d(field: torch.Tensor, max_bond: int = 32) -> QTT3DState:
    ...
    if not (2**nx == Nx and 2**ny == Ny and 2**nz == Nz):
        raise ValueError(
            f"Field dimensions must be powers of 2, got ({Nx}, {Ny}, {Nz})"
        )
...
def qtt_3d_to_dense(state: QTT3DState) -> torch.Tensor:
    ...
    field = torch.zeros(
        Nx, Ny, Nz, dtype=morton_field.dtype, device=morton_field.device
    )
...
if __name__ == "__main__":
    ...
    config = Euler3DConfig(qubits_per_dim=4, max_rank=24, cfl=0.3)  # 16³ = 4096 points
    ...
    for i in range(n_steps):
        ...
        print(
            f"  Step {i+1}: dt={dt:.5f}, rank={state.max_rank()}, time={step_time:.2f}s"
        )
