...
class MPSState:
    ...
    def __init__(
        self, cores: list[Tensor], n_vars: int = 3, canonical_form: str = "none"
    ):
        self.cores = cores
    ...
    @classmethod
    def from_primitive(
        cls, rho: Tensor, u: Tensor, p: Tensor, gamma: float = 1.4, chi_max: int = 32
    ) -> MPSState:
        """
    ...
    def _state_to_tt_cores(
        state: Tensor, chi_max: int, svd_cutoff: float = 1e-12
    ) -> list[Tensor]:
        """
...
class EulerMPO:
    ...
    def __init__(self, n_sites: int, dx: float, gamma: float = 1.4, chi_mpo: int = 8):
    ...
    def apply(self, mps: MPSState) -> MPSState:
        ...
        for i, (mps_core, mpo_core) in enumerate(zip(mps.cores, self.mpo_cores)):
            ...
            result = result.reshape(chi_l_mps * chi_l_mpo, d_out, chi_r_mps * chi_r_mpo)
...
def tdvp_euler_step(
    mps: MPSState, mpo: EulerMPO, dt: float, config: TTCFDConfig | None = None
) -> MPSState:
    """
...
def _tdvp1_step(
    mps: MPSState, mpo: EulerMPO, dt: float, config: TTCFDConfig
) -> MPSState:
    """
...
def _tdvp2_step(
    ...
    for i in range(n_sites - 2, -1, -1):
        ...
        H_eff = _compute_two_site_hamiltonian(new_cores, mpo.mpo_cores, i)
        ...
        # Truncate
        chi_new = min(
            config.chi_max, len(S), (S > config.svd_cutoff * S[0]).sum().item()
        )
...
def _compute_effective_hamiltonian(
    mps_cores: list[Tensor], mpo_cores: list[Tensor], site: int
) -> Tensor:
    """
    ...
    for i in range(site):
        ...
        left_env = torch.einsum(
            "abc,aid,bije,cje->def", left_env, mps_core, mpo_core, mps_core.conj()
        )
    ...
    for i in range(n_sites - 1, site, -1):
        ...
        right_env = torch.einsum(
            "abc,dia,bijd,cjc->def", right_env, mps_core, mpo_core, mps_core.conj()
        )
...
class TT_Euler1D:
    ...
    def initialize(self, rho: Tensor, u: Tensor, p: Tensor) -> None:
        """
        ...
        """
        self.state = MPSState.from_primitive(rho, u, p, self.gamma, self.chi_max)
        self.time = 0.0
    ...
    def initialize_sod(self) -> None:
        ...
        rho = torch.where(
            x < self.L / 2, torch.ones_like(x), 0.125 * torch.ones_like(x)
        )
        u = torch.zeros_like(x)
        p = torch.where(x < self.L / 2, torch.ones_like(x), 0.1 * torch.ones_like(x))
    ...
    def step(self, dt: float | None = None) -> None:
        ...
        # TDVP time step
        self.state = tdvp_euler_step(self.state, self.mpo, dt, self.config)
    ...
    def solve(
        self,
        t_final: float,
        callback: Callable[[float, MPSState], None] | None = None,
...
class TT_Euler2D:
    ...
    def __init__(
        ...
        self.state: Tensor | None = None  # (Nx, Ny, 3)
        self.time = 0.0
    ...
    def initialize(self, rho: Tensor, u: Tensor, v: Tensor, p: Tensor) -> None:
        """
        ...
        self.state = torch.stack([rho, rho * u, rho * v, E], dim=-1)  # (Nx, Ny, 4)
...
def check_conservation(
    state_initial: MPSState, state_final: MPSState, gamma: float = 1.4
) -> dict:
    """
