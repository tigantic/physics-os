...
class HybridLESState:
    ...
    f_d: torch.Tensor | None = None  # DDES delay function
    f_e: torch.Tensor | None = None  # IDDES elevation function
    f_b: torch.Tensor | None = None  # IDDES blending function
...
def compute_wall_distance_scale(
    d_wall: torch.Tensor, kappa: float = KAPPA
) -> torch.Tensor:
    """
...
def des_length_scale(
    l_rans: torch.Tensor, delta: torch.Tensor, c_des: float = C_DES
) -> torch.Tensor:
    """
...
def ddes_delay_function(
    r_d: torch.Tensor, c_d1: float = C_D1, c_d2: float = C_D2
) -> torch.Tensor:
    """
...
def ddes_length_scale(
    l_rans: torch.Tensor, delta: torch.Tensor, f_d: torch.Tensor, c_des: float = C_DES
) -> torch.Tensor:
    """
...
def iddes_blending_function(
    ...
    f_b = torch.minimum(2.0 * torch.exp(-9.0 * (r_d**2)), 1.0 - f_step)
...
def iddes_length_scale(
    ...
    l_wmles = torch.minimum(torch.maximum(KAPPA * d_wall, delta), l_les)
...
def run_hybrid_les(
    ...
    if is_3d:
        dudx = torch.gradient(u[0], spacing=(grid_spacing[0][0, 0, 0].item(),), dim=0)[
            0
        ]
        dudy = torch.gradient(u[0], spacing=(grid_spacing[1][0, 0, 0].item(),), dim=1)[
            0
        ]
        dudz = torch.gradient(u[0], spacing=(grid_spacing[2][0, 0, 0].item(),), dim=2)[
            0
        ]
...
def validate_hybrid_les():
    ...
    for model in [HybridModel.DES, HybridModel.DDES, HybridModel.IDDES]:
        ...
        stats = estimate_rans_les_ratio(state)
        print(
            f"{model.value}: RANS {stats['rans_fraction']:.1%}, LES {stats['les_fraction']:.1%}"
        )
