...
def download_hrrr(
    ...
    if local_path.exists() and not force:
        print("[CACHE] Using cached file (use --force to re-download)")
        return local_path
    ...
    # Download
    print("[DOWNLOAD] Fetching from AWS S3...")
    try:
        ...
        if cycle_hour > 0:
            ...
            print("[FATAL] No available HRRR data found")
            sys.exit(1)
...
def list_available_cycles(limit: int = 5):
    ...
    # List date folders
    response = s3.list_objects_v2(Bucket=BUCKET_NAME, Prefix="hrrr.", Delimiter="/")

    prefixes = sorted(
        [p["Prefix"] for p in response.get("CommonPrefixes", [])], reverse=True
    )[:limit]
...
def main():
    ...
    parser.add_argument(
        "--list", action="store_true", help="List available cycles and exit"
    )
    ...
    filepath = download_hrrr(
        cycle_hour=args.hour, forecast_hour=args.forecast, force=args.force
    )
