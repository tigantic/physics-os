...
class PhysicsLoss:
    ...
    def to_dict(self) -> dict[str, float]:
        ...
        return {
            ...
            "momentum_z": (
                self.momentum_z.item() if self.momentum_z is not None else 0.0
            ),
...
class PhysicsInformedNet(CFDSurrogate):
    ...
    def predict_with_gradients(
        ...
        for i in range(self.config.output_dim):
            ...
            grad = torch.autograd.grad(
                u[:, i].sum(), x, create_graph=True, retain_graph=True
            )[0]
    ...
    def compute_euler_residual(self, x: torch.Tensor) -> PhysicsLoss:
        ...
        mom_z_loss = (
            torch.mean(momentum_z**2) if momentum_z is not None else torch.tensor(0.0)
        )
    ...
    def compute_loss(
        ...
        losses["data"] = (
            torch.mean((y_pred - y_target) ** 2) * self.loss_weights["data"]
        )
        ...
        if x_bc is not None and y_bc is not None:
            ...
            losses["boundary"] = (
                torch.mean((y_bc_pred - y_bc_target) ** 2)
        ...
        if x_ic is not None and y_ic is not None:
            ...
            losses["initial"] = (
                torch.mean((y_ic_pred - y_ic_target) ** 2)
                * self.loss_weights["initial"]
            )
...
def create_pinn_for_equation(
    equation: str, input_dim: int = 4, output_dim: int = 5, **kwargs
) -> PhysicsInformedNet:
    """
    ...
    Returns:
        Configured PINN
    """
    config = PINNConfig(input_dim=input_dim, output_dim=output_dim, **kwargs)
...
def compute_physics_residual(
    pinn: PhysicsInformedNet, x: torch.Tensor
) -> dict[str, float]:
    """
