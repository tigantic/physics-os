...
from .surrogate_base import CFDSurrogate, MLPSurrogate, SurrogateConfig
...
class UncertaintyEstimate:
    ...
    samples: torch.Tensor | None = None  # Raw samples (if requested)
...
class UncertaintyQuantifier(nn.Module):
    ...
    def predict_with_uncertainty(
        self, x: torch.Tensor, confidence: float = 0.95
    ) -> UncertaintyEstimate:
        """
...
class EnsembleUQ(UncertaintyQuantifier):
    ...
    def __init__(self, config: UncertaintyConfig, base_config: SurrogateConfig):
        super().__init__(config)
        self.base_config = base_config

        # Create ensemble
        self.models = nn.ModuleList(
            [MLPSurrogate(base_config) for _ in range(config.n_ensemble)]
        )
    ...
    def calibrate_temperature(
        self, val_x: torch.Tensor, val_y: torch.Tensor, n_iter: int = 100
    ):
        ...
        def closure():
            ...
            nll = 0.5 * torch.mean(
                ...
                + (val_y - estimate.mean) ** 2
                / (estimate.std**2 * self.temperature**2 + 1e-8)
            )
            nll.backward()
            return nll
...
class MCDropoutUQ(UncertaintyQuantifier):
    ...
    def __init__(self, config: UncertaintyConfig, model: CFDSurrogate):
        super().__init__(config)
        self.model = model
...
class BayesianUQ(UncertaintyQuantifier):
    ...
    def elbo_loss(
        self, x: torch.Tensor, y: torch.Tensor, n_samples: int = 5
    ) -> torch.Tensor:
        """
...
def compute_prediction_interval(
    mean: torch.Tensor, std: torch.Tensor, confidence: float = 0.95
...
def test_uncertainty():
    ...
    base_model = MLPSurrogate(
        SurrogateConfig(input_dim=4, output_dim=1, hidden_dims=[32, 32], dropout=0.1)
    )
