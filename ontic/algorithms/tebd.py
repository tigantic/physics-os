...
from ontic.core.profiling import memory_profile
...
def build_heisenberg_gates(
    ...
    def h_two_site(include_left_h: bool = True, include_right_h: bool = True):
        h2 = Jx * torch.kron(Sx, Sx) + Jy * torch.kron(Sy, Sy) + Jz * torch.kron(Sz, Sz)
    ...
    for i in range(0, L - 1, 2):
        h2 = h_two_site(
            ...
            include_right_h=(
                i + 1 == L - 1 and (L - 1) % 2 == 1
            ),  # If last site not covered by even
        )
...
def tebd(
    ...
    order: int = 2,
    cutoff: float = 1e-14,
    compute_energy: Callable[[MPS], float] | None = None,
    compute_every: int = 1,
    ...
    for step in range(1, num_steps + 1):
        ...
        if step % compute_every == 0:
            ...
            if verbose:
                ...
                print(
                    f"Step {step}/{num_steps}: t = {step * dt:.4f}{E_str}, "
