...
class MarketDataFeed:
    ...
    def __init__(self, product_id: str = "BTC-USD", grid_size: int = 2048):
        """
        Initialize feed.
    ...
    async def _connect_and_process(self) -> None:
        ...
        async with websockets.connect(
            self.COINBASE_WS_URL, max_size=10 * 1024 * 1024  # 10MB limit
        ) as ws:
            print("[FEED] Connected to Coinbase")
...
async def run_demo():
    ...
    def on_physics(density: torch.Tensor, book: OrderBookFluid):
        ...
        print(
            f"[PHYSICS] ${book.stats.mid_price:,.2f} | "
