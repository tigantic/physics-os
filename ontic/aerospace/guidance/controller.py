...
from ontic.aerospace.guidance.trajectory import (
    AtmosphericModel,
    TrajectoryConfig,
    TrajectorySolver,
    VehicleState,
...
def proportional_navigation(
    vehicle_state: VehicleState, target_state: VehicleState, nav_ratio: float = 3.0
) -> float:
    """
...
def bank_angle_guidance(
    ...
    range_to_target = math.sqrt(
        (dlat * 6371000) ** 2 + (dlon * math.cos(lat) * 6371000) ** 2
    )
    ...
    if V > 100:
        ...
        E_ratio = (V**2 / 2 + 9.81 * alt) / (
            target.velocity_m_s**2 / 2 + 9.81 * target.altitude_m + 1
        )
    ...
    # Limit bank angle
    bank_cmd = max(
        -corridor.max_bank_angle_rad, min(corridor.max_bank_angle_rad, bank_cmd)
    )
...
class GuidanceController:
    ...
    def __init__(
        ...
        self.aero_table: dict | None = None
    ...
    def _initialize_constraints(self):
        ...
        self.constraints = {
            ConstraintType.THERMAL_RATE: TrajectoryConstraint(
                ConstraintType.THERMAL_RATE, self.corridor.max_heat_rate_W_cm2
            ),
            ConstraintType.THERMAL_LOAD: TrajectoryConstraint(
                ConstraintType.THERMAL_LOAD, self.corridor.max_heat_load_J_cm2
            ),
            ConstraintType.STRUCTURAL_G: TrajectoryConstraint(
                ConstraintType.STRUCTURAL_G, self.corridor.max_g_load
            ),
            ConstraintType.ALTITUDE_MIN: TrajectoryConstraint(
                ConstraintType.ALTITUDE_MIN, self.corridor.min_altitude_m
            ),
            ConstraintType.ALTITUDE_MAX: TrajectoryConstraint(
                ConstraintType.ALTITUDE_MAX, self.corridor.max_altitude_m
            ),
        }
    ...
    def lookup_aerodynamics(
        self, mach: float, alpha_deg: float
    ...
    def estimate_heating(self, state: VehicleState, atm: AtmosphericModel) -> float:
        """
    ...
    def estimate_g_load(
        self, state: VehicleState, atm: AtmosphericModel, bank_angle: float, aoa: float
    ) -> float:
        """
    ...
    def update_constraints(self, state: VehicleState, command: GuidanceCommand):
        """
        ...
        self.constraints[ConstraintType.THERMAL_LOAD].current_value = (
            self.heat_load_accumulated
        )

        # G-load
        n = self.estimate_g_load(
            state, atm, command.bank_angle_rad, command.angle_of_attack_rad
        )
    ...
    def apply_constraint_limiting(
        self, command: GuidanceCommand, state: VehicleState
    ) -> GuidanceCommand:
        """
        ...
        thermal_rel = min(
            1.0, self.constraints[ConstraintType.THERMAL_RATE].relative_margin
        )
        ...
        command.angle_of_attack_rad = max(
            0, min(self.corridor.max_aoa_rad, command.angle_of_attack_rad)
        )

        return command

    def predict_trajectory(
        self, state: VehicleState, command: GuidanceCommand, horizon_s: float = 30.0
    ) -> list[VehicleState]:
        """
    ...
    def compute_guidance(
        self, state: VehicleState, target: WaypointTarget | None = None
    ) -> GuidanceCommand:
        """
        ...
        command = bank_angle_guidance(state, target, self.corridor, L_over_D)
...
def validate_guidance_module():
    ...
    constraint = TrajectoryConstraint(
        ConstraintType.THERMAL_RATE, max_value=100.0, current_value=85.0
    )
    ...
    target_state = VehicleState(latitude_rad=0.01, longitude_rad=0.01, u_m_s=0.0)
    ...
    state = VehicleState(
        latitude_rad=0.0, longitude_rad=0.0, altitude_m=50000, u_m_s=3000
    )

    target = WaypointTarget(latitude_rad=0.1, longitude_rad=0.1, altitude_m=0)
    ...
    initial = VehicleState(altitude_m=40000, u_m_s=2000, mass_kg=1500)

    target = WaypointTarget(latitude_rad=0.05, longitude_rad=0.05)
