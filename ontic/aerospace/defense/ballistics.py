...
class BallisticSolver:
    ...
    def __init__(
        ...
        self.gravity = torch.tensor(
            [0.0, -9.81, 0.0], device=self.device, dtype=torch.float64
        )
        ...
        print("[BALLISTICS] Solver initialized")
        print(
            f"[BALLISTICS] Bullet: {bullet_mass_grains}gr @ {muzzle_velocity_fps} fps"
        )
    ...
    def sample_wind_field(
        ...
        wind_field: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """
    ...
    def solve_trajectory(
        ...
        position = torch.tensor(
            [0.0, 2.0, 0.0], device=self.device, dtype=torch.float64
        )
        ...
        while position[2] < target_distance and t < max_time:
            ...
            if t >= max_time or position[1] < -100:
                wind_at_target = (
                    wind
                    if wind is not None
                    else torch.zeros(3, device=self.device, dtype=torch.float64)
                )
                break
        ...
        los_height_at_impact = (
            2.0 + (target_elevation * final_pos[2].item() / target_distance)
        ...
        if verbose:
            print(
                f"   [IMPACT] Target reached at Z={final_pos[2]:.1f}m, Y={final_pos[1]:.1f}m"
            )
            print(
                f"   Slant range: {slant_range_m:.1f}m, Target angle: {np.degrees(target_angle):.2f}°"
            )
...
def solve_shot(
    ...
    print("   ⚠️  Wind shear detected! Muzzle and target winds OPPOSE.")
    print(f"   Total drift: {solution.drift_meters:+.2f}m")
    print()
    print(
        f"   DIAL: {solution.elevation_mils:.2f} Mils UP, "
