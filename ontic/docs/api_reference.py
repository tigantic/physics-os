...
class FunctionDoc:
    ...
    def to_markdown(self, level: int = 3) -> str:
        ...
        if self.source_file and self.line_number:
            lines.append(
                f"*Source: [{self.source_file}:{self.line_number}]({self.source_file}#L{self.line_number})*"
            )
            lines.append("")
...
class DocstringParser:
    ...
    def parse(self, docstring: str) -> dict[str, Any]:
        ...
        style = (
            ...
            else self.detect_style(docstring)
        )
    ...
    def _parse_google_returns(self, section: str) -> ReturnDoc | None:
    ...
    def _parse_numpy(self, docstring: str) -> dict[str, Any]:
        ...
        converted = re.sub(
            r"^(Parameters|Returns|Raises|Examples)\s*\n-+",
            r"\1:",
            converted,
            flags=re.MULTILINE,
        )
    ...
    def _parse_rst(self, docstring: str) -> dict[str, Any]:
        ...
        for match in re.finditer(r":param\s+(\w+):\s*([^\n]+)", remaining):
            ...
            result["parameters"].append(
                ParameterDoc(name=name, description=desc.strip())
            )
...
class APIExtractor:
    ...
    def extract_module(
        self, module: Any, source_file: str | None = None
    ) -> ModuleDoc:
    ...
    if path.is_file():
        ...
        if init_path.exists():
            spec = importlib.util.spec_from_file_location(
                path.name, init_path, submodule_search_locations=[str(path)]
            )
            module = importlib.util.module_from_spec(spec)
            try:
                spec.loader.exec_module(module)
            ...
            if recursive:
                for child in path.iterdir():
                    if (
                        ...
                        try:
                            sub_doc = extract_module_docs(
                                child, recursive=False, include_private=include_private
                            )
                        ...
                        try:
                            sub_doc = extract_module_docs(
                                child, recursive=True, include_private=include_private
                            )
