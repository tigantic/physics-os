...
class ValidationCase:
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "passed": self.passed,
            "status": (
                self.comparison.overall_status.name if self.comparison else "NOT_RUN"
            ),
    ...
    def _generate_markdown(self) -> str:
        ...
        for case in c.cases:
            ...
            if case.comparison:
                ...
                lines.append("")
                lines.append("| Field | Mean Error (%) | RMS Error | Status |")
                lines.append("|-------|----------------|-----------|--------|")

                for name, fc in comp.field_comparisons.items():
                    status = fc.status.name
                    lines.append(
                        f"| {name} | {fc.mean_percent_error:.2f} | {fc.rms_error:.4f} | {status} |"
                    )
                lines.append("")
        ...
        if failed:
            ...
            for case in failed:
                ...
                if case.comparison:
                    ...
                    if worst:
                        lines.append(
                            f"  - Worst field: {worst.field_name} ({worst.mean_percent_error:.1f}% error)"
                        )
            lines.append("")
        ...
        if c.is_successful():
            lines.append(
                "The validation campaign has **successfully demonstrated** that the simulation "
                "results are in acceptable agreement with flight test data across the "
                "evaluated conditions."
            )
        else:
            lines.append(
                "The validation campaign **did not meet** the required pass threshold. "
        ...
        # Footer
        lines.append("---")
        lines.append("*Report generated by Ontic Validation Framework*")
...
"""
        ...
        html += """
    <footer>
        <p><em>Report generated by Ontic Validation Framework</em></p>
    </footer>
</body>
</html>
"""
    ...
    def _generate_json(self) -> str:
        ...
        for case in self.campaign.cases:
            if case.comparison:
                for name, fc in case.comparison.field_comparisons.items():
                    case_data = next(
                        (c for c in data["cases"] if c["case_id"] == case.case_id), None
                    )
                    if case_data:
    ...
    def _generate_text(self) -> str:
        ...
        for case in c.cases:
            ...
            if case.comparison:
                for name, fc in case.comparison.field_comparisons.items():
                    lines.append(
                        f"    {name}: {fc.mean_percent_error:.2f}% error ({fc.status.name})"
                    )
...
\title{Validation Report: """
            + c.name
            + r"""}
\author{Ontic Validation Framework}
\date{"""
            + time.strftime("%Y-%m-%d")
            + r"""}
...
\textbf{Campaign ID:} """
            + c.campaign_id
            + r"""\\
\textbf{Validation Level:} """
            + c.level.name
            + r"""\\
\textbf{Overall Status:} """
            + (
                r"\textcolor{success}{PASSED}"
                if c.is_successful()
                else r"\textcolor{failure}{FAILED}"
            )
            + r"""
...
"""
        ...
        for case in c.cases:
            status = (
                r"\textcolor{success}{PASS}"
                if case.passed
                else r"\textcolor{failure}{FAIL}"
            )
            latex += (
                r"""
\subsection{Case: """
