...
Usage:
    python3 ontic/benchmarks/gpu_integration_benchmark.py
...
from ontic.engine.gpu import advection
...
def sustained_load_test(duration_seconds: float = 5.0, size: int = 512):
    ...
    print("\n┌─────────────────────────────────────────────────────────────┐")
    print(
        f"│  SUSTAINED LOAD TEST: {duration_seconds}s @ {size}x{size}                       │"
    )
    print("│  Watch nvidia-smi for GPU utilization spike!               │")
    print("└─────────────────────────────────────────────────────────────┘")
    ...
    print("  ✓ GPU should show >40% utilization in nvidia-smi")
...
def main():
    ...
    for size in GRID_SIZES:
        ...
        print(
            f"{size:>8}x{size:<3} │ {cuda_result['mean_ms']:>12.4f} │ {pytorch_result['mean_ms']:>14.4f} │ {speedup:>7.1f}x"
        )
    ...
    for size in GRID_SIZES:
        ...
        print(
            f"{size:>8}x{size:<3} │ {result['mean_ms']:>12.4f} │ {result['fps']:>10.0f}"
        )
