...
class FADSMeasurement:
    ...
    timestamps: Tensor
    validity: Tensor
    temperature_K: Tensor | None = None
...
def modified_newtonian_cp(theta: Tensor, gamma: float = 1.4) -> Tensor:
    """
...
def tangent_cone_cp(mach: float, cone_half_angle: float, gamma: float = 1.4) -> float:
    """
...
class FADSSensor:
    ...
    def measure(
        ...
        for i, port in enumerate(self.ports):
            ...
            p_true = compute_surface_pressure(
                mach, alpha_rad, beta_rad, port.position, port.normal, p_inf, self.gamma
            )
    ...
    def measure_from_field(
        self, pressure_field: Tensor, field_positions: Tensor, timestamp: float = 0.0
    ) -> FADSMeasurement:
        """
    ...
    def _apply_noise(self, pressure: float, port_idx: int, timestamp: float) -> float:
    ...
    def estimate_state(
        ...
        # State vector: [M, α, β]
        x = torch.tensor(
            [mach_initial, alpha_initial, beta_initial], dtype=torch.float64
        )
        ...
        p_total = p_inf * (1 + 0.5 * (self.gamma - 1) * mach**2) ** (
