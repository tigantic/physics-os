...
class Interval:
    ...
    def __post_init__(self):
        ...
        if not torch.all(self.lo <= self.hi + 1e-15):
            raise ValueError("Invalid interval: lo > hi detected")

    @classmethod
    def from_value(cls, x: float | torch.Tensor, ulp_pad: int = 1) -> Interval:
        """
    ...
    def contains(self, x: float | torch.Tensor) -> torch.Tensor:
    ...
    def __add__(self, other: Interval | float | torch.Tensor) -> Interval:
        """[a,b] + [c,d] = [a+c, b+d]"""
    ...
    def __sub__(self, other: Interval | float | torch.Tensor) -> Interval:
        """[a,b] - [c,d] = [a-d, b-c]"""
        if isinstance(other, Interval):
    ...
    def __mul__(self, other: Interval | float | torch.Tensor) -> Interval:
        """
    ...
    def __truediv__(self, other: Interval | float | torch.Tensor) -> Interval:
        """
        [a,b] / [c,d]
        ...
        if isinstance(other, Interval):
            ...
            if other.contains(0.0).any():
                raise ValueError(
                    "Division by interval containing zero - result unbounded"
                )
    ...
    def sin(self) -> Interval:
        ...
        if width >= 2 * math.pi:
            return Interval(-torch.ones_like(self.lo), torch.ones_like(self.hi))
...
class IntervalTensor:
    ...
    def norm(self) -> Interval:
        ...
        return Interval(
            self.interval.norm_lower_bound(), self.interval.norm_upper_bound()
        )
...
def validate_interval_arithmetic():
    ...
    print("✓ Test 6: (x * 2 - 1) contains expected values")
    passed += 1
