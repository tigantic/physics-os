...
class LoadBalancer:
    ...
    def complete_work(
        ...
        with self._lock:
            ...
            status.average_time = (status.average_time * (n - 1) + time_taken) / n

            self._update_worker_state(worker_id)

    def get_next_work(self, worker_id: int) -> WorkUnit | None:
    ...
    def _steal_work(self, worker_id: int) -> WorkUnit | None:
        ...
        with self._lock:
            ...
            if self.config.strategy == BalancingStrategy.STATIC:
                ...
                for source_id in overloaded:
                    ...
                    to_move = int(
                        source_status.current_load * self.config.max_steal_fraction
                    )
                    ...
                    for _ in range(to_move):
                        ...
                        if (
                            self.workers[target_id].utilization
