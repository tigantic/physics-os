...
class TaskGraph:
    ...
    def get_critical_path(self) -> list[int]:
        ...
        dist: dict[int, float] = dict.fromkeys(order, 0.0)
...
class DistributedScheduler:
    ...
    def wait_task(self, task_id: int, timeout: float | None = None) -> Any:
    ...
    def wait_graph(
        self, graph_id: int, timeout: float | None = None
    ) -> dict[int, Any]:
...
def execute_parallel(
    funcs: list[Callable], config: TaskConfig | None = None
) -> list[Any]:
    """
