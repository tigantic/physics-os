...
class ContractionCache:
    ...
    def _make_key(
        self, core_indices: tuple[int, ...], bit_pattern: tuple[int, ...]
    ) -> str:
...
class BoundedMode:
    ...
    def _adapt_rank(self, elapsed_ms: float):
        ...
        if elapsed_ms > target * 1.2:
            ...
            self.current_rank = max(
                self.config.min_rank, int(self.current_rank * (1 - rate))
            )
        elif elapsed_ms < target * 0.7:
            # Increase rank (we have headroom)
            self.current_rank = min(
                self.config.max_rank, int(self.current_rank * (1 + rate * 0.5))
            )
    ...
    def precompile_path(self, name: str, n_cores: int, rank: int) -> ContractionPath:
        ...
        try:
            ...
            shapes = [
                (1 if i == 0 else rank, 2, rank if i < n_cores - 1 else 1)
                for i in range(n_cores)
            ]
            path, info = oe.contract_path(
                eq, *[np.ones(s) for s in shapes], optimize="optimal"
            )
    ...
    def stats(self) -> dict[str, Any]:
        ...
        p99_frame = (
            np.percentile(self.frame_times, 99)
        ...
        return {
            ...
            "budget_hit_ratio": self.budget_hits
            / max(1, self.budget_hits + self.budget_misses),
...
class AdaptiveRankController:
    ...
    def update(self, frame_time_ms: float) -> int:
        ...
        self.current_rank = np.clip(
            self.current_rank + rank_delta, self.min_rank, self.max_rank
        )
