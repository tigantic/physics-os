...
class TileCompositor:
    ...
    def __init__(
        ...
        self.validity_mask = torch.zeros(
            (config.grid_rows, config.grid_cols), dtype=torch.bool, device=self.device
        )

        # Stats
        self.stats = {"tiles_loaded": 0, "cache_evictions": 0, "gpu_memory_mb": 0.0}
    ...
    def get_tile(self, row: int, col: int) -> torch.Tensor | None:
        """
    ...
    def sample_world(
        self, lon: torch.Tensor, lat: torch.Tensor, normalize: bool = True
    ) -> torch.Tensor:
        """
...
def demo_compositor():
    ...
    for row in range(config.grid_rows):
        for col in range(config.grid_cols):
            ...
            y = (
                torch.linspace(0, 1, config.tile_size)
                .view(-1, 1)
                .repeat(1, config.tile_size)
            )
            x = (
                torch.linspace(0, 1, config.tile_size)
                .view(1, -1)
                .repeat(config.tile_size, 1)
            )
